C-----------------------------------------------------------------------
      SUBROUTINE EGMPAR ( NP, T, X, Y, CP, WEG, IWEG )
C-----------------------------------------------------------------------
C
...
C        T(NP)        temperature
C        X(NS,NP)     species mole fractions
C        Y(NS,NP)     species mass fractions
C        CP(NS,NP)    species heat capacities at constant pressure
C                     per unit mass
...
C-----------------------------------------------------------------------
      CALL EGMXY ( NP, NS, WEG(IEGWT), WEG(IXTR), WEG(IYTR), 
     &             WEG(IAUX), X, Y, WEG(IWWTR), WEG(ISUMTR) )
C-----------------------------------------------------------------------
      RETURN
      END
C-----------------------------------------------------------------------
C-----------------------------------------------------------------------
      SUBROUTINE LMPAR ( NP, IFLAG, PATMOS, 
...
C-----------------------------------------------------------------------
     ...
     &          DLT5(NP), DLT6(NP), AAA(NP), CROT(NP)
      DIMENSION CPMS(NS,NP), CINT(NP,NS), CXI(NP,NS)
...
C-----------------------------------------------------------------------
      ...
      enddo
C-----------------------------------------------------------------------
      CALL EGMCOFE ( NP, COFE, NS, DLT1, DLT2, DLT3, ETA, ETALG )
      IF ( IFLAG .LE. 1 ) RETURN
C-----------------------------------------------------------------------
      CALL EGMCOFD ( NP, COFD, NS, DLT1, DLT2, DLT3, BIN )
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMCOFD ( NP, COF, NS, DLT, DLT2, DLT3, BIN )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMCOFE ( NP, COF, NS, DLT, DLT2, DLT3, ETA, ETALG )
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMXY ( NP, NS, WT, XTR, YTR, AUX, X, Y, WWTR, SUMTR )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION WT(NS), XTR(NP,NS), YTR(NP,NS), AUX(NP,NS), X(NS,NP), 
     &          Y(NS,NP), WWTR(NP), SUMTR(NP)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      DO I = 1, NS
         do nn = 1, np
            XTR(nn,I) = X(I,nn) + SSS * ( SUMTR(nn)*AAA - X(I,nn) )
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMBIN (NP, T, WEG, BIN)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
...
C     Output
C     ------
C        BIN(NS,NS,NP)  binary diffusion coefficients at atmospheric 
C                       pressure for species pairs (i,j) with i<>j.
C
C-----------------------------------------------------------------------
      INCLUDE 'eg.cmn'
      DIMENSION WEG(*), BIN(NS,NS,NP)
C-----------------------------------------------------------------------
      IND = 0
      DO I = 1, NS
         IND = IND + NP
         DO J = I+1, NS
            do nn = 1, np
               BIN(I,J,nn) = 1.0D0 / WEG ( IBIN + IND + nn - 1 )
               BIN(J,I,nn) = BIN(I,J,nn)
            enddo
            IND = IND + NP
         ENDDO
      ENDDO
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMD1 (NP, PRES, IPRES, T, Y, WW, WEG, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C     Output
C     ------
C        D(NS,NS,NP)  flux diffusion matrix
C
C     
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMD1 ( NP, NS, PRES, IPRES, WEG(IXTR), WEG(IYTR), 
     &              WEG(ISUMTR), WEG(IEGPA), D, WEG(IBIN), WEG(IG), 
     &              WEG(IDMI), WEG(IAAA), WEG(ITEMP), WEG(IAUX) )
      RETURN
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMD1 ( NP, NS, PRES, IPRES, XTR, YTR, SUMTR, PATMOS, 
     &                    D, BIN, G, DMI, AAA, TEMP, AUX )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      CALL EGMEML00 ( NP, NS, XTR, BIN, G )
C-----------------------------------------------------------------------
C        Call the iterative procedures
C-----------------------------------------------------------------------
      ITERMX = 2
      CALL EGMSI1 ( NP, NS, G, DMI, TEMP, YTR, SUMTR, D, ITERMX, 
     &              AUX, AAA )
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      ELSE
         CALL EGMSCAD ( NP, NS, PATMOS, PRES, D, AAA )
      ENDIF
C-----------------------------------------------------------------------
      RETURN
      END
C=======================================================================
C=======================================================================
      SUBROUTINE EGMD2 (NP, PRES, IPRES, T, Y, WW, WEG, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMD2 ( NP, NS, PRES, IPRES, WEG(IXTR), WEG(IYTR), 
     &              WEG(ISUMTR), WEG(IEGPA), D, WEG(IBIN), 
     &              WEG(IG), WEG(IAN), WEG(IAAA), WEG(ITEMP) )
      RETURN
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMD2 ( NP, NS, PRES, IPRES, XTR, YTR, SUMTR, 
     &                    PATMOS, D, BIN, G, AN, AAA, TEMP )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C     Form the linear system
C-----------------------------------------------------------------------
      CALL EGMDDEC ( NP, NS, XTR, YTR, G, BIN, TEMP )
C-----------------------------------------------------------------------
C     Form the matrix D
C-----------------------------------------------------------------------
      CALL EGMEVD ( NP, NS, NS, AN, YTR, G, SUMTR, D, TEMP, AAA )
C-----------------------------------------------------------------------
C     Project the matrix D
C-----------------------------------------------------------------------
      CALL EGMPDP ( NP, NS, YTR, TEMP, SUMTR, D, AAA )
C-----------------------------------------------------------------------
C     Correct for the pressure dependence of D
C-----------------------------------------------------------------------
      IF ( IPRES .EQ. 0 ) THEN
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMDR1 (NP, T, Y, WW, WEG, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C     Output
C     ------
C        D(NS,NS,NP)  rho * flux diffusion matrix
C
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMDR1 ( NP, NS, T, WEG(IXTR), WEG(IYTR), WEG(IWWTR), 
     ...
     &               WEG(IAAA) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMDR1 ( NP, NS, TEMPER, XTR, YTR, WWTR, SUMTR,  
...
C-----------------------------------------------------------------------
     ...
     &              AUX, AAA )
C-----------------------------------------------------------------------
      CALL EGMSCADR ( NP, NS, PATMOS, WWTR, RU, TEMPER, D, AAA )
C-----------------------------------------------------------------------
      RETURN
      END
C=======================================================================
C=======================================================================
      SUBROUTINE EGMDR2 (NP, T, Y, WW, WEG, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMDR2 ( NP, NS, T, WEG(IXTR), WEG(IYTR), WEG(ISUMTR), 
     &               WEG(IWWTR), WEG(IEGRU), WEG(IEGPA), D, WEG(IBIN), 
     &               WEG(IG), WEG(IAN), WEG(ITEMP), WEG(IAAA) )
      RETURN
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMDR2 ( NP, NS, TEMPER, XTR, YTR, SUMTR, WWTR, 
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGME1 ( NP, ALPHA, T, X, WEG, ETA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGME1 ( NP, NS, X, ETA, WEG(IETALG), ALPHA, WEG(IAAA) ) 
      RETURN
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGME1 ( NP, NS, X, ETAMA, ETALG, ALPHA, SUM )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION X(NS,NP), ETALG(NP,NS), ETAMA(NP), SUM(NP)
C-----------------------------------------------------------------------
      call egzero ( np, sum )
      IF ( ALPHA .EQ. 0.0D0 ) THEN
         DO I = 1, NS
            do nn = 1, np
               SUM(nn) = SUM(nn) + X(I,nn) * ETALG(nn,I)
            enddo
         ENDDO
         do nn = 1, np
      ...
      ELSE
         DO I = 1, NS
            do nn = 1, np
               SUM(nn) = SUM(nn) + X(I,nn) * DEXP ( ALPHA*ETALG(nn,I) ) 
            enddo
         ENDDO
         RALPHA = (1.0D0/ALPHA)
         do nn = 1, np
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGME2 ( NP, T, Y, WEG, ETA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGME2 ( NP, NS, T, WEG(IXTR), WEG(IEGWT), ETA, 
     &            WEG(IEGRU), WEG(IEGPA),
     ...
     &            WEG(IBBB), WEG(ICTAIJ), WEG(IETA), WEG(IBIN) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGME2 ( NP, NS, T, X, WT, ETACG, RU, PATMOS, 
     &                  BETA, AN, ZN, RN, G, DMI, TEMP, AAA, BBB, 
     &                  AIJ, ETA, BIN )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      CALL EGMEMHA ( NP, NS, T, X, WT, RU, PATMOS, BETA, G, AIJ, 
     &               ETA, BIN )
C-----------------------------------------------------------------------
      ITERMX = 1
      CALL DCOPY  ( np*NS, BETA, 1, RN, 1 )
      CALL EGMCG1 ( NP, NS, G, DMI, AN, ZN, RN, TEMP, 
     &              AAA, BBB, ITERMX )
C-----------------------------------------------------------------------
      CALL EGMDOT ( NP, NS, ETACG, AN, BETA )
C-----------------------------------------------------------------------
      RETURN
      END
C=======================================================================
C=======================================================================
      SUBROUTINE EGME3 ( NP, T, Y, WEG, ETA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGME3 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2), WEG(IDLT3),
     &           WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     ...
     &           WEG(IETA), WEG(IBIN) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGME3 ( NP, NS, T, DLT1, DLT2, DLT3, DLT4, DLT5, DLT6,
     &                 X, WT, ETACG, RU, PATMOS, 
     &                 BETA, AN, ZN, RN, G, DMI, TEMP, AAA, BBB, 
     &                 AIJ, FITA, ETA, BIN )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      CALL EGMEMH ( NP, NS, T, DLT1, DLT2, DLT3, DLT4, DLT5, DLT6,
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGME4 ( NP, T, Y, WEG, ETA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGME4 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2), WEG(IDLT3),
     &             WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &             WEG(IXTR), WEG(IEGWT), ETA, 
     &             WEG(IEGRU), WEG(IEGPA),
     ...
     &             WEG(IAIJ), WEG(IFITA), WEG(IETA), WEG(IBIN) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGME4 ( NP, NS, T, DLT1, DLT2, DLT3, DLT4, DLT5, DLT6,
     &                 X, WT, ETACG, RU, PATMOS, 
     &                 BETA, AN, G, TEMP, 
     &                 AIJ, FITA, ETA, BIN )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
     ...
     &              ETA, BIN )
C-----------------------------------------------------------------------
      CALL EGMDEC ( NP, NS, G, TEMP, IER )
      CALL DCOPY  ( np*NS, BETA, 1, AN, 1 )
      CALL EGMSOL ( NP, NS, G, AN, TEMP )
C-----------------------------------------------------------------------
      CALL EGMDOT ( NP, NS, ETACG, AN, BETA )
C-----------------------------------------------------------------------
      RETURN
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMK1 ( NP, ALPHA, T, X, WEG, VOLVIS )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMK1 ( NP, NS, X, VOLVIS, WEG(IETALG), WEG(ICXI),
     &              WEG(ICINT), ALPHA, WEG(IAAA), WEG(IBBB) ) 
      RETURN
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMK1 ( NP, NS, X, VOLVIS, ETALG, CXI, CINT, ALPHA,
     &                  SXP, SUM )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION X(NS,NP), ETALG(NP,NS), CXI(NP,NS), CINT(NP,NS)
         ...
         DO I = 1, NS
            IF ( CXI(1,I) .NE. 0.0D0 ) THEN
               do nn = 1, np
                  ...
                  SUM(nn) = SUM(nn) + SXP(nn) * X(I,nn) * VVV
               enddo
            ENDIF
         ENDDO
         do nn = 1, np
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMK2 (NP, T, Y, WEG, VV01)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMK2 ( NP, NS, T, WEG(IXTR), WEG(IEGWT), WEG(IEGRU), 
     &              WEG(IEGPA), VV01, 
     ...
     &              WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMK2 ( NP, NS, TEMPER, XTR, WT, RU, PATMOS, VV01, 
     &                    BIN, AIJ, CINT, ETA, CXI, G, BETA, AAA, BBB )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION G(NP,*), BETA(NP,*), VV01(NP)
C-----------------------------------------------------------------------
      CALL EGMEMKA01 ( NP, NS, TEMPER, XTR, WT, RU, PATMOS,
     &                 BIN, AIJ, CINT, ETA, CXI, G, BETA, AAA, BBB )
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMK3 (NP, T, Y, WEG, VV01)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMK3 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2), WEG(IDLT3),
     &            WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     ...
     &            WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMK3 ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
...
C-----------------------------------------------------------------------
      DIMENSION G(NP,*), BETA(NP,*), VV01(NP)
C-----------------------------------------------------------------------
      CALL EGMEMK01 ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMK4 (NP, T, Y, WEG, VV)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMK4 ( NP, NS, T, WEG(IXTR), WEG(IEGWT), WEG(IEGRU), 
     &              WEG(IEGPA), VV, 
     &              WEG(IBIN), WEG(ICTAIJ), WEG(ICINT),
     &              WEG(IETA),  WEG(ICXI), WEG(IG), WEG(IBETA),
     &              WEG(IAN), WEG(ITEMP), WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMK4 ( NP, NS, TEMPER, XTR, WT, RU, PATMOS, VV, 
     &           BIN, AIJ, CINT, ETA, CXI, G, BETA, AN, TEMP, AAA, BBB )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      CALL EGMEMKA ( NP, NS, TEMPER, XTR, WT, RU, PATMOS,
     &               BIN, AIJ, CINT, ETA, CXI, G, BETA, AAA, BBB )
C-----------------------------------------------------------------------
C        Evaluate the transport coefficient
C-----------------------------------------------------------------------
      CALL EGMEVK ( NP, NS, VV, G, AN, BETA, TEMP, AAA, BBB )
C-----------------------------------------------------------------------
      RETURN
      END
C=======================================================================
C=======================================================================
      SUBROUTINE EGMK5 (NP, T, Y, WEG, VV)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMK5 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2), WEG(IDLT3),
     &           WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &           WEG(IXTR), WEG(IEGWT), WEG(IEGRU), 
     &           WEG(IEGPA), VV, 
     &           WEG(IBIN), WEG(IAIJ), WEG(IFITA), WEG(ICINT), 
     ...
     &           WEG(IAN), WEG(ITEMP), WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMK5 ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
     &           DLT5, DLT6, XTR, WT, RU, PATMOS, VV, 
     &           BIN, AIJ, FITA, CINT, ETA, CXI, G, BETA, AN, TEMP, 
     &           AAA, BBB )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      CALL EGMEMK ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
     &           DLT5, DLT6, XTR, WT, RU, PATMOS,
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMK6 ( NP, T, Y, WEG, VV )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMK6 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2), WEG(IDLT3),
     &           WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &           WEG(IXTR), WEG(IEGWT), WEG(IEGRU), 
     &           WEG(IEGPA), VV, 
     &           WEG(IBIN), WEG(IAIJ), WEG(IFITA), WEG(ICINT), 
     ...
     &           WEG(IAN), WEG(ITEMP), WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMK6 ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
     &           DLT5, DLT6, XTR, WT, RU, PATMOS, VV, 
     &           BIN, AIJ, FITA, CINT, ETA, CXI, G, BETA, AN, TEMP, 
     &           AAA, BBB )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C        Evaluate the transport coefficient
C-----------------------------------------------------------------------
      CALL EGMDEC ( NP, NG, G, TEMP, IER )
      CALL DCOPY  ( np*NG, BETA, 1, AN, 1 )
      CALL EGMSOL ( NP, NG, G, AN, TEMP )
C-----------------------------------------------------------------------
      CALL EGMDOT ( NP, NG, VV, AN, BETA )
C-----------------------------------------------------------------------
      RETURN
      END
C=======================================================================
C=======================================================================
      SUBROUTINE EGML1 ( NP, ALPHA, T, X, WEG, CON )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
     ...
     &              CON, WEG(IEGCFL), ALPHA, WEG(IAAA) ) 
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGML1 ( NP, NS, X, T, DLT, DLT2, DLT3, 
     &                  CONMA, COF, ALPHA, SUM )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION X(NS,NP), COF(4,NS), CONMA(NP)
      ...
      ELSE
         DO I = 1, NS
            ...
            do nn = 1, np
               CONLG = COF1 + COF2*DLT(nn) + COF3*DLT2(nn) 
     &                      + COF4*DLT3(nn) 
               SUM(nn) = SUM(nn) + X(I,nn) * DEXP ( ALPHA*CONLG )
            enddo
         ENDDO
         RALPHA = (1.0D0/ALPHA)
         do nn = 1, np
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGML2 (NP, T, Y, WEG, IWEG, TC)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGML2 ( NP, NS, T, WEG(IXTR), WEG(IEGWT), WEG(IEGRU), 
     ...
     &              WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGML2 ( NP, NS, TEMPER, XTR, WT, RU, PATMOS,
     &           TC, 
     &           BIN, AIJ, BIJ, CINT, ETA, CXI, LIN,
     &           G, DMI, AN, ZN, RN, TEMP, BETA, AAA, BBB )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      CALL EGMEMAAE ( NP, NS, TEMPER, XTR, WT, RU, PATMOS,
...
C.....
      ...
      CALL EGMCG1 ( NP, NG, G, DMI, AN, ZN, RN, TEMP, 
     &              AAA, BBB, ITERMX )
C-----------------------------------------------------------------------
C        Evaluate the transport coefficients
C-----------------------------------------------------------------------
      CALL EGMEVL ( NP, NG, TC, AN, BETA, PATMOS, TEMPER )
C-----------------------------------------------------------------------
      RETURN
      END
C=======================================================================
C=======================================================================
      SUBROUTINE EGML3 (NP, T, Y, WEG, IWEG, TC)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGML3 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2), WEG(IDLT3),
     &             WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &             WEG(IXTR), WEG(IEGWT), WEG(IEGRU), 
     &             WEG(IEGPA), TC, 
     ...
     &             WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGML3 ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
     ...
     &           G, DMI, AN, ZN, RN, TEMP, BETA, AAA, BBB )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      CALL EGMEMAE ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
     &           DLT5, DLT6, XTR, WT, RU, PATMOS,
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGML4 (NP, T, Y, WEG, IWEG, TC)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGML4 ( NP, NS, T, WEG(IXTR), WEG(IEGWT), WEG(IEGRU), 
     &             WEG(IEGPA), TC, WEG(IBIN),
     &             WEG(ICTAIJ), WEG(ICTBIJ), 
     &             WEG(ICINT), WEG(IETA), WEG(ICXI),
     ...
     &             WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGML4 ( NP, NS, TEMPER, XTR, WT, RU, PATMOS, TC, 
     &           BIN, AIJ, BIJ, CINT, ETA, CXI, LIN,
     &           G, DMI, AN, ZN, RN, TEMP, BETA, AAA, BBB )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      CALL EGMEMAA ( NP, NS, TEMPER, XTR, WT, RU, PATMOS,
     &               BIN, AIJ, BIJ, CINT, ETA, CXI,
     &               LIN, G, BETA )
C-----------------------------------------------------------------------
C        Call the iterative procedures
...
C.....
      ITERMX = 1
      CALL DCOPY  ( np*NG, BETA, 1, RN, 1 )
      CALL EGMCG2 ( NP, NS, NG, G, DMI, AN, ZN, RN, TEMP, 
     &              AAA, BBB, ITERMX )
C-----------------------------------------------------------------------
C        Evaluate the transport coefficients
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGML5 (NP, T, Y, WEG, IWEG, TC)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGML5 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2), WEG(IDLT3),
     &            WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &            WEG(IXTR), WEG(IEGWT), WEG(IEGRU), 
     &            WEG(IEGPA), TC, 
     &            WEG(IBIN), WEG(IAIJ), WEG(IBIJ), 
     ...
     &            WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGML5 ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
     &           DLT5, DLT6, XTR, WT, RU, PATMOS, TC, 
     &           BIN, AIJ, BIJ, FITA, FITB, 
     &           CINT, ETA, CXI, LIN,
     &           G, DMI, AN, ZN, RN, TEMP, BETA, AAA, BBB )
...
C-----------------------------------------------------------------------
      CALL EGMEMA ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
     &           DLT5, DLT6, XTR, WT, RU, PATMOS,
     &           BIN, AIJ, BIJ, FITA, FITB, CINT, ETA, CXI,
     &           LIN, G, BETA )
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLCT1 (NP, T, Y, WEG, IWEG, TC, CHIT)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C     ------
C        TC(NP)       thermal conductivity 
C        CHIT(NS,NP)  rescaled thermal diffusion ratios
C
C     
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLCT1 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2), 
     ...
     &         WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLCT1 ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
     &           DLT5, DLT6, XTR, WT, RU, PATMOS,
     &           TC, CHIT,
...
C-----------------------------------------------------------------------
      CALL EGMEVL  ( NP, NG, TC, AN, BETA, PATMOS, TEMPER )
      CALL EGMEVCT ( NP, NS, CHIT, AN, XTR, BIN, CIJ, FITC,
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLCT2 ( NP, T, Y, WEG, IWEG, TC, CHIT )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLCT2 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2),
     &         WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &         WEG(IXTR), WEG(IEGWT), WEG(IEGRU), 
     &         WEG(IEGPA), TC, CHIT,
     &         WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &         WEG(IG), WEG(IAN), WEG(ITEMP), WEG(IBETA) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLCT2 ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
     &           DLT5, DLT6, XTR, WT, RU, PATMOS,
     &           TC, CHIT,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC,
     &           CINT, ETA, CXI, LIN,
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLCT3 (NP, T, Y, WEG, IWEG, TC, CHIT)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLCT3 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2),
     &         WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &         WEG(IXTR), WEG(IEGWT), WEG(IEGRU), 
     &         WEG(IEGPA), TC, CHIT,
     &         WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &         WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLCT3 ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
     &           DLT5, DLT6, XTR, WT, RU, PATMOS,
     &           TC, CHIT,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, 
     &           CINT, ETA, CXI, LIN,
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLCT4 (NP, T, Y, WEG, IWEG, TC, CHIT)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLCT4 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2),
     &         WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &         WEG(IXTR), WEG(IEGWT), WEG(IEGRU), 
     &         WEG(IEGPA), TC, CHIT,
     &         WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &         WEG(IG), WEG(IAN), WEG(ITEMP), WEG(IBETA) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLCT4 ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, 
     &           DLT5, DLT6, XTR, WT, RU, PATMOS,
     &           TC, CHIT,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC,
     &           CINT, ETA, CXI, LIN,
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLTD1 (NP, PRES, IPRES, T, Y, WW, WEG, IWEG, 
     &                    PTC, THETA, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
...
C        THETA(NS,NP) thermal diffusion vector
C        D(NS,NS,NP)  flux diffusion matrix
C
C
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLTD1 ( NP, NS, PRES, IPRES, T, WEG(IDLT1), WEG(IDLT2),
     &        WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     ...
     &        WEG(ITEMP), WEG(IBETA), WEG(IAUX), WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLTD1 ( NP, NS, PRES, IPRES, 
     ...
     &           LIN, G, DMI, AN, ZN, RN, TEMP, BETA, AUX, AAA, BBB )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      CALL EGMEMLE ( NP, NS, TEMPER, DLT1, DLT2, DLT3,  
...
C.....
      ITERMX = 2
      CALL EGMSI2 ( NP, NS, NG, G, DMI, TEMP, YTR, SUMTR, D, ITERMX, 
     &              AUX, AAA )
C-----------------------------------------------------------------------
C        Evaluate the transport coefficients
C-----------------------------------------------------------------------
      CALL EGMEVL ( NP, NG, PTC, AN, BETA, PATMOS, TEMPER )
      CALL EGMEVT ( NP, NS, THETA, AN, YTR, SUMTR, AAA )
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      IF ( IPRES .EQ. 0 ) THEN
         ...
         CALL DSCAL ( np*NS*NS, AA, D, 1 )
      ELSE
         CALL EGMSCATD ( NP, NS, PATMOS, PRES, THETA, D, AAA )
      ENDIF
C-----------------------------------------------------------------------
      RETURN
      END
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLTD2 (NP, PRES, IPRES, T, Y, WW, WEG, IWEG, 
     &                    PTC, THETA, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLTD2 ( NP, NS, PRES, IPRES, T, WEG(IDLT1), WEG(IDLT2),
     &        WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &        WEG(IXTR), WEG(IYTR), WEG(IEGWT), WEG(ISUMTR), 
     &        WEG(IEGRU), WEG(IEGPA), PTC, THETA, D,
     &        WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &        WEG(IAN), WEG(IAAA),  WEG(ITEMP), WEG(IBETA) )
      RETURN
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLTD2 ( NP, NS, PRES, IPRES, 
     &           TEMPER, DLT1, DLT2, DLT3, DLT4, DLT5, DLT6,  
     &           XTR, YTR, WT, SUMTR, RU, PATMOS, PTC, THETA, D,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, CINT, ETA, CXI,
...
C-----------------------------------------------------------------------
      ...
      CALL EGMEVT ( NP, NS, THETA, AN, YTR, SUMTR, AAA )
C-----------------------------------------------------------------------
      CALL EGMEVD ( NP, NS, NG, AN, YTR, G, SUMTR, D, TEMP, AAA )
C-----------------------------------------------------------------------
C     Project the matrix D
C-----------------------------------------------------------------------
      CALL EGMPDP ( NP, NS, YTR, TEMP, SUMTR, D, AAA )
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLTD3 (NP, PRES, IPRES, T, Y, WW, WEG, IWEG, 
     &                    PTC, THETA, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLTD3 ( NP, NS, PRES, IPRES, T, WEG(IDLT1), WEG(IDLT2),
     &         WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &         WEG(IXTR), WEG(IYTR), WEG(IEGWT), WEG(ISUMTR), 
     &         WEG(IEGRU), WEG(IEGPA), PTC, THETA, D,
     &         WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &         WEG(ITEMP), WEG(IBETA), WEG(IAUX), WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLTD3 ( NP, NS, PRES, IPRES, 
     &           TEMPER, DLT1, DLT2, DLT3, DLT4, DLT5, DLT6,  
     &           XTR, YTR, WT, SUMTR, RU, PATMOS, PTC, THETA, D,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, CINT, ETA, CXI,
     &           LIN, G, DMI, AN, ZN, RN, TEMP, BETA, AUX, AAA, BBB )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      CALL EGMEML ( NP, NS, TEMPER, DLT1, DLT2, DLT3,  
     &           DLT4, DLT5, DLT6, XTR, WT, RU, PATMOS,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, CINT, ETA, CXI,
     &           LIN, G, BETA )
C-----------------------------------------------------------------------
...
C.....
      ITERMX = 3
      CALL DCOPY  ( np*NG, BETA, 1, RN, 1 )
      CALL EGMCG3 ( NP, NS, NG, G, DMI, AN, ZN, RN, TEMP, 
     &              AAA, BBB, ITERMX )
C-----------------------------------------------------------------------
C        Evaluate the transport coefficients
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLTD4 ( NP, PRES, IPRES, T, Y, WW, WEG, IWEG, 
     &                     PTC, THETA, D )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLTD4 ( NP, NS, PRES, IPRES, T, WEG(IDLT1), WEG(IDLT2),
     &         WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &         WEG(IXTR), WEG(IYTR), WEG(IEGWT), WEG(ISUMTR), 
     &         WEG(IEGRU), WEG(IEGPA), PTC, THETA, D,
     &         WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &         WEG(IG), WEG(IAN), WEG(IAAA), WEG(ITEMP), WEG(IBETA) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLTD4 ( NP, NS, PRES, IPRES, 
     &           TEMPER, DLT1, DLT2, DLT3, DLT4, DLT5, DLT6,  
     &           XTR, YTR, WT, SUMTR, RU, PATMOS, PTC, THETA, D,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, CINT, ETA, CXI,
     &           LIN, G, AN, AAA, TEMP, BETA )
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLTD5 (NP, PRES, IPRES, T, Y, WW, WEG, IWEG, 
     &                    PTC, THETA, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLTD5 ( NP, NS, PRES, IPRES, T, WEG(IDLT1), WEG(IDLT2),
     &         WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &         WEG(IXTR), WEG(IYTR), WEG(IEGWT), WEG(ISUMTR), 
     &         WEG(IEGRU), WEG(IEGPA), PTC, THETA, D,
     &         WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &         WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLTD5 ( NP, NS, PRES, IPRES, 
     &           TEMPER, DLT1, DLT2, DLT3, DLT4, DLT5, DLT6,  
     &           XTR, YTR, WT, SUMTR, RU, PATMOS, PTC, THETA, D,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, CINT, ETA, CXI,
     &           LIN, G, DMI, AN, ZN, RN, TEMP, BETA, AUX, AAA, BBB )
...
C.....
      ITERMX = 2
      CALL EGMSI3 ( NP, NS, NG, G, DMI, TEMP, YTR, SUMTR, D, ITERMX, 
     &              AUX, AAA )
C-----------------------------------------------------------------------
C        Evaluate the transport coefficients
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLTD6 ( NP, PRES, IPRES, T, Y, WW, WEG, IWEG, 
     &                     PTC, THETA, D )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLTD6 ( NP, NS, PRES, IPRES, T, WEG(IDLT1), WEG(IDLT2),
     &         WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &         WEG(IXTR), WEG(IYTR), WEG(IEGWT), WEG(ISUMTR), 
     &         WEG(IEGRU), WEG(IEGPA), PTC, THETA, D,
     &         WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &         WEG(IG), WEG(IAN), WEG(IAAA), WEG(ITEMP), WEG(IBETA) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLTD6 ( NP, NS, PRES, IPRES, 
     &           TEMPER, DLT1, DLT2, DLT3, DLT4, DLT5, DLT6,  
     &           XTR, YTR, WT, SUMTR, RU, PATMOS, PTC, THETA, D,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, CINT, ETA, CXI,
     &           LIN, G, AN, AAA, TEMP, BETA )
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLTDR1 (NP, T, Y, WW, WEG, IWEG, PTC, THETA, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C     ------
C        PTC(NP)      partial thermal conductivity
C        THETA(NS,NP) rho * thermal diffusion vector
C        D(NS,NS,NP)  rho * flux diffusion matrix
C
C
C     Two CG iterations are performed on the matrix L_[e] in order
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLTDR1 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2),
     &        WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     ...
     &        WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLTDR1 ( NP, NS, TEMPER, DLT1, DLT2, DLT3,  
...
C-----------------------------------------------------------------------
      ...
      CALL EGMEVT ( NP, NS, THETA, AN, YTR, SUMTR, AAA )
C-----------------------------------------------------------------------
      CALL EGMSCATDR ( NP, NS, PATMOS, WWTR, RU, TEMPER, THETA, D, AAA )
C-----------------------------------------------------------------------
      RETURN
      END
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLTDR2 (NP, T, Y, WW, WEG, IWEG, PTC, THETA, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLTDR2 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2),
     &        WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &        WEG(IXTR), WEG(IYTR), WEG(IEGWT), WEG(IWWTR), WEG(ISUMTR), 
     &        WEG(IEGRU), WEG(IEGPA), PTC, THETA, D,
     &        WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &        WEG(IG), WEG(IAN), WEG(IAAA), WEG(ITEMP), WEG(IBETA) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLTDR2 ( NP, NS, TEMPER, DLT1, DLT2, DLT3,  
     &           DLT4, DLT5, DLT6, XTR, YTR, WT, WWTR, SUMTR,
     &           RU, PATMOS, PTC, THETA, D,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, CINT, ETA, CXI,
     &           LIN, G, AN, AAA, TEMP, BETA )
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLTDR3 (NP, T, Y, WW, WEG, IWEG, PTC, THETA, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLTDR3 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2),
     &        WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &        WEG(IXTR), WEG(IYTR), WEG(IEGWT), WEG(IWWTR), WEG(ISUMTR), 
     &        WEG(IEGRU), WEG(IEGPA), PTC, THETA, D,
     &        WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &        WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLTDR3 ( NP, NS, TEMPER, DLT1, DLT2, DLT3,  
     &           DLT4, DLT5, DLT6, XTR, YTR, WT, WWTR, SUMTR,
     &           RU, PATMOS, PTC, THETA, D,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, CINT, ETA, CXI,
     &           LIN, G, DMI, AN, ZN, RN, TEMP, BETA, AUX, AAA, BBB )
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLTDR4 ( NP, T, Y, WW, WEG, IWEG, PTC, THETA, D )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLTDR4 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2),
     &        WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &        WEG(IXTR), WEG(IYTR), WEG(IEGWT), WEG(IWWTR), WEG(ISUMTR), 
     &        WEG(IEGRU), WEG(IEGPA), PTC, THETA, D,
     &        WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &        WEG(IG), WEG(IAN), WEG(IAAA), WEG(ITEMP), WEG(IBETA) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLTDR4 ( NP, NS, TEMPER, DLT1, DLT2, DLT3,  
     &           DLT4, DLT5, DLT6, XTR, YTR, WT, WWTR, SUMTR,
     &           RU, PATMOS, PTC, THETA, D,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, CINT, ETA, CXI,
     &           LIN, G, AN, AAA, TEMP, BETA )
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLTDR5 (NP, T, Y, WW, WEG, IWEG, PTC, THETA, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLTDR5 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2),
     &        WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &        WEG(IXTR), WEG(IYTR), WEG(IEGWT), WEG(IWWTR), WEG(ISUMTR), 
     &        WEG(IEGRU), WEG(IEGPA), PTC, THETA, D,
     &        WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &        WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLTDR5 ( NP, NS, TEMPER, DLT1, DLT2, DLT3,  
     &           DLT4, DLT5, DLT6, XTR, YTR, WT, WWTR, SUMTR,
     &           RU, PATMOS, PTC, THETA, D,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, CINT, ETA, CXI,
     &           LIN, G, DMI, AN, ZN, RN, TEMP, BETA, AUX, AAA, BBB )
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMLTDR6 ( NP, T, Y, WW, WEG, IWEG, PTC, THETA, D )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMLTDR6 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2),
     &        WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &        WEG(IXTR), WEG(IYTR), WEG(IEGWT), WEG(IWWTR), WEG(ISUMTR), 
     &        WEG(IEGRU), WEG(IEGPA), PTC, THETA, D,
     &        WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &        WEG(IG), WEG(IAN), WEG(IAAA), WEG(ITEMP), WEG(IBETA) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMLTDR6 ( NP, NS, TEMPER, DLT1, DLT2, DLT3,  
     &           DLT4, DLT5, DLT6, XTR, YTR, WT, WWTR, SUMTR,
     &           RU, PATMOS, PTC, THETA, D,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, CINT, ETA, CXI,
     &           LIN, G, AN, AAA, TEMP, BETA )
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMTD1 (NP, PRES, IPRES, T, Y, WW, WEG, IWEG, 
     &                   THETA, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMTD1 ( NP, NS, PRES, IPRES, T, WEG(IDLT1), WEG(IDLT2),
     &        WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &        WEG(IXTR), WEG(IYTR), WEG(IEGWT), WEG(IWWTR), WEG(ISUMTR), 
     ...
     &        WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMTD1 ( NP, NS, PRES, IPRES, 
     &           TEMPER, DLT1, DLT2, DLT3, DLT4, DLT5, DLT6,  
     ...
     &           G, DMI, AN, ZN, RN, TEMP, BETA, AUX, AAA, BBB )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      CALL EGMEML ( NP, NS, TEMPER, DLT1, DLT2, DLT3, DLT4, DLT5, DLT6,  
     &           XTR, WT, RU, PATMOS,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC, CINT, ETA, CXI,
     &           LIN, G, BETA )
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      CALL EGMEVT ( NP, NS, THETA, AN, YTR, SUMTR, AAA )
      CALL EGMEVDI1 ( NP, NS, D, WT, WWTR, TEMP, XTR, YTR, BIN, AUX )
C-----------------------------------------------------------------------
C     Project the matrix D
C-----------------------------------------------------------------------
      CALL EGMPDP ( NP, NS, YTR, TEMP, SUMTR, D, AAA )
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMTDR1 (NP, T, Y, WW, WEG, IWEG, THETA, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMTDR1 ( NP, NS, T, WEG(IDLT1), WEG(IDLT2),
     &       WEG(IDLT3), WEG(IDLT4), WEG(IDLT5), WEG(IDLT6),
     &       WEG(IXTR), WEG(IYTR), WEG(IEGWT), WEG(IWWTR), WEG(ISUMTR), 
     &       WEG(IEGRU), WEG(IEGPA), THETA, D,
     &       WEG(IBIN), WEG(IAIJ), WEG(IBIJ), WEG(ICIJ), 
     ...
     &       WEG(IAAA), WEG(IBBB) )
      ...
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMTDR1 ( NP, NS, TEMPER, DLT1, DLT2, DLT3,  
     &           DLT4, DLT5, DLT6, XTR, YTR, WT, WWTR, SUMTR, 
     &           RU, PATMOS, THETA, D,
     &           BIN, AIJ, BIJ, CIJ, FITA, FITB, FITC,
     &           CINT, ETA, CXI, LIN,
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMV1 (NP, PRES, IPRES, T, Y, WW, WEG, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C     Output
C     ------
C        D(NS,NP)     flux diffusion coefficients
C
C     
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMV1 ( NP, NS, PRES, IPRES, WEG(IXTR), WEG(IYTR), 
     &              WEG(IEGWT), WEG(IWWTR), WEG(IEGPA), D, WEG(IBIN), 
     &              WEG(IAUX), WEG(IAAA) )
      RETURN
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMV1 ( NP, NS, PRES, IPRES, XTR, YTR, WT, WWTR, 
     &                    PATMOS, D, BIN, AUX, AAA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION XTR(NP,NS), YTR(NP,NS), WT(NS), D(NS,NP), 
...
C-----------------------------------------------------------------------
      DO I = 1, NS
         do nn = 1, np
            D(I,nn) = WT(I) * AUX(nn,I) / ( D(I,nn) * WWTR(nn) )
         enddo
      ENDDO
C-----------------------------------------------------------------------
C        Correct for the pressure dependence of D
C-----------------------------------------------------------------------
      ...
      ELSE
         CALL EGMSCAV ( NP, NS, PATMOS, PRES, D, AAA )
      ENDIF
C-----------------------------------------------------------------------
      RETURN
      END
C=======================================================================
C=======================================================================
      SUBROUTINE EGMVR1 (NP, T, Y, WEG, D)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C     Output
C     ------
C        D(NS,NP)     rho * flux diffusion coefficients
C
C        where rho is the density.
C
C     
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMVR1 ( NP, NS, T, WEG(IXTR), WEG(IEGWT), 
     &               WEG(IEGRU), WEG(IEGPA), D, WEG(IBIN), 
     &               WEG(IAUX) )
      RETURN
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMVR1 ( NP, NS, TEMP, XTR, WT, RU, PATMOS, 
     &                     D, BIN, AUX )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION XTR(NP,NS), WT(NS), D(NS,NP), BIN(NP,*), AUX(NP,NS)
      DIMENSION TEMP(NP)
C-----------------------------------------------------------------------
      CALL EGZERO ( np*NS, D )
      DO I = 1, NS
...
C-----------------------------------------------------------------------
      DO I = 1, NS
         FAC = WT(I) * PATMOS / RU
         do nn = 1, np
            D(I,nn) = FAC * AUX(nn,I) / ( D(I,nn) * TEMP(nn) )
         enddo
      ENDDO
C-----------------------------------------------------------------------
      RETURN
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMYV (NP, PRES, IPRES, Y, WW, WEG, F, IDDEC)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C        WEG          double precision work array for EGLIB
C        F(NS,NP)     species diffusion driving forces
...
C     Output
C     ------
C        F(NS,NP)     species flux diffusion velocities
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMYV ( NP, NS, PRES, IPRES, WEG(IXTR), WEG(IYTR), 
     &              WEG(IEGPA), WEG(IG), WEG(ITEMP), 
     &              WEG(IBIN), WEG(IAAA), F, IDDEC )
      RETURN
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMYV ( NP, NS, PRES, IPRES, XTR, YTR, 
     &                    PATMOS, G, TEMP, BIN, AAA, F, IDDEC )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION F(NS,NP), YTR(NP,NS), PRES(*), AAA(NP)
C-----------------------------------------------------------------------
      IF ( IDDEC .NE. 0 ) THEN
         CALL EGMDDEC ( NP, NS, XTR, YTR, G, BIN, TEMP )
      ENDIF
      CALL EGMSOL1 ( NP, NS, G, F, AAA )
      IF ( IPRES .EQ. 0 ) THEN
         PPP = - PATMOS / PRES(1)
         DO I = 1, NS
            do nn = 1, np
               F(I,nn) = F(I,nn) * YTR(nn,I) * PPP
            enddo
         ENDDO
      ELSE
         do nn = 1, np
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMRYV (NP, T, Y, WEG, F, IDDEC)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Minimum flags required in EGINI
...
C     Output
C     ------
C        F(NS,NP)     rescaled species flux diffusion velocities
...
C-----------------------------------------------------------------------
      ...
      INCLUDE 'eg.cmn'
C-----------------------------------------------------------------------
      CALL LEGMRYV ( NP, NS, WEG(IXTR), WEG(IYTR), WEG(IEGWT),
     &               WEG(IEGPA), WEG(IEGRU), T, WEG(IG), WEG(ITEMP), 
     &               WEG(IBIN), WEG(IAAA), F, IDDEC )
      RETURN
      END
C-----------------------------------------------------------------------
      SUBROUTINE LEGMRYV ( NP, NS, XTR, YTR, WT, PATMOS, RU, TEMPER, 
     &                     G, TEMP, BIN, AAA, F, IDDEC )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION TEMPER(NP), F(NS,NP), WT(NS), AAA(NP), XTR(NP,NS)
C-----------------------------------------------------------------------
      IF ( IDDEC .NE. 0 ) THEN
         CALL EGMDDEC  ( NP, NS, XTR, YTR, G, BIN, TEMP )
      ENDIF
...
C-----------------------------------------------------------------------
      ...
      DO I = 1, NS
         do nn = 1, np
            F(I,nn) = F(I,nn) * WT(I) * XTR(nn,I) * AAA(nn)
         enddo
      ENDDO
C-----------------------------------------------------------------------
      RETURN
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMDOT ( NP, NG, DOT, AN, BETA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMEVCT ( NP, NS, CHIT, AN, XTR, BIN, CIJ, FITC,
     &                     DLT1, DLT2, DLT3, DLT4, DLT5, DLT6, WT )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION CHIT(NS,NP), AN(NP,NS), XTR(NP,NS), BIN(NP,*),
...
C........
     ...
     &             / ( WT(I)+WT(J) )
               CHIT(I,nn) = CHIT(I,nn) + XTR(nn,J) * TERM
               CHIT(J,nn) = CHIT(J,nn) - XTR(nn,I) * TERM
            enddo
         ENDDO
      ENDDO
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMEVD ( NP, NS, NG, AN, YTR, G, SUMTR, D, TEMP, AAA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMEVDI1 ( NP, NS, D, WT, WWTR, TEMP, 
     &                      XTR, YTR, BIN, AUX )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION WT(NS), WWTR(NP), TEMP(NP,NS), XTR(NP,NS), YTR(NP,NS),
     &          BIN(NP,*), AUX(NP,NS)
      DIMENSION D(NS,NS,NP)
C-----------------------------------------------------------------------
      ...
      DO I = 1, NS
         DO J = I+1, NS
            IJ = NS*(I-1) - (I*(I-1))/2 + J
            do nn = 1, np
               D(I,J,nn) = WT(I) / WWTR(nn) * TEMP(nn,I) * TEMP(nn,J) 
     &                                      * XTR(nn,I) * BIN(nn,IJ)
               D(J,I,nn) = WT(J) / WWTR(nn) * TEMP(nn,I) * TEMP(nn,J) 
     &                                      * XTR(nn,J) * BIN(nn,IJ)
            enddo
         ENDDO
         do nn = 1, np
            D(I,I,nn) = WT(I) / WWTR(nn) * TEMP(nn,I) 
     &                                   * ( 1.0D0 + YTR(nn,I) )
         enddo
      ENDDO
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMEVK ( NP, NS, VV, G, AN, BETA, TEMP, VV01, VVS )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMEVL ( NP, NG, TC, AN, BETA, PATMOS, TEMPER )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMEVT ( NP, NS, THETA, AN, YTR, SUMTR, AAA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION THETA(NS,NP), AN(NP,NS), YTR(NP,NS), AAA(NP), SUMTR(NP)
C-----------------------------------------------------------------------
      call egzero ( np, aaa )
      DO I = 1, NS
         do nn = 1, np
      ...
      DO I = 1, NS
         do nn = 1, np
            THETA(I,nn) =  aaa(nn) / sumtr(nn) - AN(nn,I)
         enddo
      ENDDO
C-----------------------------------------------------------------------
      RETURN
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMSCAD ( NP, NS, PATMOS, PRES, D, AAA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION D(NS,NS,NP), PRES(NP), AAA(NP)
C-----------------------------------------------------------------------
      do nn = 1, np
         aaa(nn) = patmos / pres(nn)
      enddo
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMSCAV ( NP, NS, PATMOS, PRES, D, AAA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION D(NS,NP), PRES(NP), AAA(NP)
C-----------------------------------------------------------------------
      do nn = 1, np
         aaa(nn) = patmos / pres(nn)
      enddo
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMSCATD ( NP, NS, PATMOS, PRES, THETA, D, AAA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION THETA(NS,NP), D(NS,NS,NP), PRES(NP), AAA(NP)
C-----------------------------------------------------------------------
      do nn = 1, np
         aaa(nn) = patmos / pres(nn)
      enddo
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMSCADR ( NP, NS, PATMOS, WWTR, RU, TEMPER, D, AAA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION WWTR(NP), TEMPER(NP), D(NS,NS,NP), AAA(NP)
C-----------------------------------------------------------------------
      do nn = 1, np
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMSCATDR ( NP, NS, PATMOS, WWTR, RU, TEMPER, 
     &                       THETA, D, AAA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      DIMENSION WWTR(NP), TEMPER(NP), THETA(NS,NP), D(NS,NS,NP), AAA(NP)
C-----------------------------------------------------------------------
      do nn = 1, np
         aaa(nn) = PATMOS * WWTR(nn) / ( RU * TEMPER(nn) )
      enddo
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMCG1 ( NP, NG, G, DMI, AN, ZN, RN, TEMP, 
     &                    AAA, BBB, ITERMX )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMCG2 ( NP, NS, NG, G, DMI, AN, ZN, RN, TEMP, 
     &                    AAA, BBB, ITERMX )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMCG3 ( NP, NS, NG, G, DMI, AN, ZN, RN, TEMP, 
     &                    AAA, BBB, ITERMX )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMSI1 ( NP, NS, G, DMI, TEMP, YTR, SUMTR,
     &                    D, ITERMX, AUX, AAA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION G(NP,*), DMI(NP,NS), TEMP(NP,NS), YTR(NP,NS), 
     &          D(NS,NS,NP), AAA(NP), SUMTR(NP), AUX(NP,NS)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      IF (ITERMX .GT. 1) THEN
         DO I = 1, NS
            do nn = 1, np
               D(I,I,nn) = DMI(nn,I) * ( 1.0D0 + YTR(nn,I) )
            enddo
            DO J = I+1, NS
               IND = NS*(I-1) - (I*(I-1))/2 + J
               do nn = 1, np
                  D(I,J,nn) = - DMI(nn,I) * DMI(nn,J) * G(nn,IND)
                  D(J,I,nn) = D(I,J,nn)
               enddo
            ENDDO
         ENDDO
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMSI2 ( NP, NS, NG, G, DMI, TEMP, YTR, SUMTR,
     &                    D, ITERMX, AUX, AAA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMSI3 ( NP, NS, NG, G, DMI, TEMP, YTR, SUMTR,
     &                    D, ITERMX, AUX, AAA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMPDP ( NP, NS, YTR, TEMP, SUMTR, D, AAA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION YTR(np,NS), TEMP(np,NS), D(NS,NS,np), SUMTR(np), AAA(np)
C-----------------------------------------------------------------------
C     Input : matrix D
...
C-----------------------------------------------------------------------
      ...
      DO J = 1, NS
         DO I = 1, NS
            do nn = 1, np
               TEMP(nn,I) = TEMP(nn,I) + D(I,J,nn) * YTR(nn,j)
            enddo
         ENDDO
      ENDDO
C.....
...
C.....
      ...
      DO I = 1, NS
         ...
         DO J = 1, NS
            do nn = 1, np
               D(I,J,nn) = D(I,J,nn) - AAA(nn) * TEMP(nn,J)
            enddo
         ENDDO
      ENDDO
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMDDEC ( NP, NS, XTR, YTR, G, BIN, TEMP )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C
C     Output
...
C-----------------------------------------------------------------------
      CALL EGMDEC ( NP, NG, G, TEMP, IER )
      IF ( IER .NE. 0 ) THEN
         WRITE(*,'(1X,''stopping in EGMDDEC with IER = '',i3)') IER
         STOP
      ENDIF
C-----------------------------------------------------------------------
      RETURN
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMDEC ( NP, N, A, W, IER )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
      ...
      DO  K = 1,N
         ...
         do nn = 1, np
            IF (A(nn,KK) .EQ. 0.0D0) THEN
               WRITE(6, '(''SINGULAR MATRIX IN EGMDEC'')' )
               WRITE(6, '(''NODE = '',I7)' ) nn
               IER = K
               stop
            ENDIF
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMSOL ( NP, N, A, B, FAC )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMSOL1 ( NP, N, A, B, FAC )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
C     The difference with EGMSOL is the ordering of indeces for B
C-----------------------------------------------------------------------
      DIMENSION A(NP,*), B(N,NP), FAC(NP)
      NM1 = N - 1
      DO J = 1, NM1
         JP1 = J + 1
         do nn = 1, np
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMEMHA ( NP, NS, T, X, WT, RU, PATMOS, 
     &                     BETA, G, AIJ, ETA, BIN )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMEMKA ( NP, NS, TEMPER, X, WT, RU, PATMOS,
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMEMKA01 ( NP, NS, TEMPER, X, WT, RU, PATMOS,
     &                       BIN, AIJ, CINT, ETA, CXI,
     &                       G, BETA, CCC, RCV )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMEMAA ( NP, NS, TEMPER, X, WT, RU, PATMOS,
     &                     BIN, AIJ, BIJ, CINT, ETA, CXI,
     &                     LIN, G, BETA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMEML00 ( NP, NS, X, BIN, G )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
...
C=======================================================================
C=======================================================================
      SUBROUTINE EGMEMAAE ( NP, NS, TEMPER, X, WT, RU, PATMOS,
     &                      BIN, AIJ, BIJ, CINT, ETA, CXI,
     &                      LIN, G, BETA )
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C-----------------------------------------------------------------------
