...
# for GPU).  Note that each (CPU/xe) node has 32 integer cores, every two of which share a
# single floating point core (GPU/xk nodes have 16 integer cores).  By specifying ppn=32, we 
# are saying we want to treat each integer core as a traditional 'core'.  This works well 
# for some applications, and not so well for others as there might be contention
...
#
# Each XK node has:
#   1 AMD 6276 'interlagos' processors (see above for details)
#   1 NVIDIA GK110 "Kepler" K20X accelerator
#     = 14 streaming multiprocessor units (SMXs) as implmented on Blue Waters
#     Each SMX has 192 single-precision CUDA cores, or 64 double-precision CUDA cores
# 
