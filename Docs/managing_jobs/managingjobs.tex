...
an error allocating memory in {\tt cluster\_f.f90}.  The NERSC team looked
into this an determined it is a compiler bug (affecting atleast 4.8.2 and 4.8.3):
\begin{quote}
The code runs without any problem if it is compiled with {\tt -O2 -ftree-vectorize -fno-range-check} (our default) expcept that {\tt cluster\_f.f90} is compiled with {\tt -O2 -ftree-vectorize -fno-range-check -fno-tree-pre}.   The ``{\tt fno-tree-pre}'' option turns off ``{\tt ftree-pre}'' that is turned on by ``{\tt O2}''.
