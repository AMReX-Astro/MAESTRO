...
\item Intel 16.x currently has issues with the linear solvers in \boxlib\
...
libraries in a directory, {\tt yt-conda}.  You need to make sure that
your start-up files (typically {\tt .bashrc.ext} at NERSC) don't {\tt
  module load} python or any python libraries, as this will interfere
with the conda installation.  The install script will direct you to
add the install location to your path.

At the moment, it seems that this does not actually build the yt source,
so a second step is needed, in {\tt yt-conda/bin/src/yt-hg}, do
\begin{verbatim}
