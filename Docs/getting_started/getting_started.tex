...
.
\item {\em Get a copy of \fboxlib}.

  \maestro\ requires the \fboxlib\ library to manage the grids and
  parallelization.  We also rely on the build system in \fboxlib\ to
  build a \maestro\ executable.  \fboxlib\ is also available on github
  and can be obtained via:
  \begin{verbatim}
git clone https://github.com/AMReX-Codes/FBoxLib.git
  \end{verbatim}

  You will periodic want to update \fboxlib\ by doing
  \begin{verbatim}
git pull
  \end{verbatim}
  in the {\tt fboxlib/} directory.


\item {\em Setup your shell environment}.

  \maestro\ needs to know where to find \fboxlib, by specifying the
  {\tt FBOXLIB\_HOME} environment variable, and where
  to find \microphysics, bt specifying the {\tt
  MICROPHYSICS\_HOME} environment variable.

  If your shell is {\tt Bash}, add
  \begin{verbatim}
export FBOXLIB_HOME="/path/to/FBoxLib/"
export MICROPHYSICS_HOME="/path/to/Microphysics/"
  ...
  \begin{verbatim}
setenv FBOXLIB_HOME /path/to/FBoxLib/
setenv MICROPHYSICS_HOME /path/to/Microphysics/
  \end{verbatim}
  to your {\tt .cshrc}.  

  Note: you must specify the full path to the {\tt FBoxLib/} and {\tt
...
\item {\em Setup the problem's {\tt GNUmakefile}}.
  ...
  \item {\tt COMP := gfortran}
    ...
    settings as specified in {\tt FBoxLib\_HOME/Tools/F\_mk/}.
...
\item {\em Examine the output}.
  ...
  outputted.  Similarly, the checkpoints are named {\tt chk}{\em
    nnnnn}.  \fboxlib\ plotfiles and checkpoints are actually
...
in-house and externally developed tools can work with \fboxlib-formatted
...
\item {\tt flame\_1d} \\[-3mm]
   ...
   elliptic solver in \fboxlib\ that only works for a single grid, so
...
\item There are two ways to specify boundary conditions---via integer flags
  ...
  by \fboxlib\ in the \code{bc\_module} at the time of initializing
...
including {\tt FBoxLib/}, {\tt Microphysics/}, and {\tt Castro/}.  
...
the {\tt GMake.MPI} file in the \fboxlib\ build files.  See
