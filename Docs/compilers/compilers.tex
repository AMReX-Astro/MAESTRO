...
\item Cray 8.1.3, 8.2.1, and 8.2.2 seem to have issues with OpenMP on hopper 
 ...
 A separate issue is that the ``volume'' reported by {\tt
 ...
 boxarray\_volume()}, which is called by \code{multifab\_volume()}.
 In Cray 8.2.4 the problem appears to be resolved, with  
 \code{\tt multifab\_volume()} returning the correct cell count.

\item Cray 8.2.6 on NERSC Edison seems to work well with OpenMP.
