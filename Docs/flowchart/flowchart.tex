...
\begin{equation}
\frac{\partial \rho X_k}{\partial t} + \nabla \cdot (\rho \Ub) = \rho \omegadot_k
\end{equation}
and
\begin{equation}
\sum_k X_k = 1
...
The equation of state is cast into an elliptic constraint on the velocity field 
by differentiating $p_0(\rho, s, X_k)$ along particle paths, giving:
\begin{equation}
\nabla \cdot (\beta_0 \Ub) = \beta_0 \left ( S - \frac{1}{\gammabar p_0} \frac{\partial p_0}{\partial t} \right )
\end{equation}
where
 $\beta_0$ is a density-like variable that carries background stratification, defined as
\begin{equation}
...
pressure (sometimes called the dynamic pressure) as $\pi = p -                                    
...
\end{equation}
This is the form of the momentum equation that we solved in papers I--III and in the multilevel
paper.
...
We decompose the full velocity field into a base
state velocity, $w_0$, that governs the base state dynamics, and a
local velocity, $\Ubt$, that governs the local dynamics, i.e.,
\begin{equation}
...
\begin{equation}
\nabla\cdot(\beta_0 w_0 \eb_r) = \beta_0\left(\Sbar - \frac{1}{\gammabar p_0}\frac{\partial p_0}{\partial t}\right).\label{eq:w0 divergence}
\end{equation}
The divergence constraint for $\Ubt$ can be found by subtracting (\ref{eq:w0 divergence})
into (\ref{eq:U divergence}), resulting in
\begin{equation}
...
\subsection{Base State and Perturbation Form Equations}
