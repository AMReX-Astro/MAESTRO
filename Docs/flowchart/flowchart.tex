...
{\bf React State}$[\rho^{\inp},(\rho                                                                   h)^{\inp},X_k^{\inp},T^{\inp}, (\rho\Hext)^{\inp},                                                      
...
{\bf Advect Base Density}$[\rho_0^\inp,w_0^\inp] \rightarrow
...
{\bf Enforce HSE}$[p_0^{\inp},\rho_0^{\inp}]
...
{\bf Advect Base Enthalpy}$[(\rho h)_0^\inp,w_0^\inp,\psi^\inp] \rightarrow [(\rho h)_0^\outp]$
...
\subsubsection{Computing $w_0$}\label{Sec:Computing w0}
...
The constraint we derive from requiring the pressure to be close to the background hydrostatic
pressure takes the form:
\begin{equation}
\nablab \cdotb \Ub + \frac{1}{\Gamma_1 p_0} \frac{Dp_0}{Dt} = S \enskip .
\end{equation}
The default \maestro\ algorithm replaces $\Gamma_1$ with $\gammabar$, allowing us
to write this as a divergence constraint.  In paper~III, we explored the effects of
localized variations in $\Gamma_1$ by writing $\Gamma_1 = \gammabar + \delta \Gamma_1$.
This gives us:
\begin{equation}
...
\overline{ \left ( \frac{\delta \Gamma_1}{\gammabar^2 p_0} \ut \cdotb \nablab
...
This constraint is not in a form that can be projected.  To solve this form, we 
need to use a lagged $\Ubt$ in the righthand side.
