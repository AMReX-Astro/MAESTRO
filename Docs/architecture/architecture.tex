...
The \maestro\ grids are managed by the \fboxlib\ library\footnote{This is a pure Fortran library that 
has a lot of similarities to the \cpp\ \amrex\ library}, which is
...
\caption[\maestro\ `ecosystem'] {\label{fig:arch:eco} The basic
  ...
  various Makefile variables ({\tt FBOXLIB\_HOME}, {\tt NETWORK\_DIR},
...
\fboxlib\ library (distributed separately, consisting of the {\tt FBoxLib/} directory).
Figure~\ref{fig:arch:eco}
...
Finally the {\tt FBoxLib/Tools/Postprocessing/F\_Src} package provides simple
...
\section{\fboxlib\ Data Structures}

\maestro's gridding is handled by the \fboxlib\ library, which
...
on the corners.  In \fboxlib, data that is on an edge is termed `nodal'
...
To simplify the description of the underlying AMR grid, \fboxlib\
provides a number of Fortran types.  We briefly summarize the major
data types below.  A more extensive introduction to \fboxlib\ is 
provided by the \fboxlib\ User's Guide, distributed with the library.
...
{\label{fig:boxes} Three boxes that comprise a single level.  At this
  ...
  indexing in \fboxlib\ starts with $0$.}
\end{figure}
...
\code{FBoxLib/Src/F\_BaseLib/multifab.f90} for the full list of
...
\section{\fboxlib\ Helper Modules}

There are a large number of modules in {\tt FBoxLib/} that provide
...
modules bring in the basic \fboxlib\ data types. Specifically, here,
...
as there is now a special \fboxlib\ subroutine, {\tt ml\_restrict\_and\_fill},
