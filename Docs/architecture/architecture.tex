...
\item {\tt MAESTRO/}
  ...
  \item {\tt Exec/}
    ...
    conditions via {\tt initdata.f90}.  See \S~\ref{sec:makefile} and
...
\maestro\ initial models are read in by the {\tt Util/model\_parser}
...
is to copy from one of the existing problems.  The file {\tt
  initveldata.f90} controls the velocity field initialization and {\tt
  initscaldata.f90} controls the initialization of the scalars
...
For a simulation setup with \runparam{n\_cellx = 32} and \runparam{n\_celly =
...
\fab\ is defined, {\tt pbx} represents the ``physical'' box in the sense
that it includes {\tt bx} plus ghost cells, and {\tt ibx} is the same as {\tt bx} unless 
the \fab\ is nodal.  As can be seen in Figure~\ref{fig:dataloc},
for the same grid nodal data requires one more array element than 
cell-centered data.  To address this {\tt ibx} is made by growing
{\tt bx} by one element along all nodal dimensions.

It's important to note that all state data is stored in a four-dimensional array
{\em regardless of the problem's dimensionality}.  The array is 
{\tt (nx,ny,nz,nc)} in size, where {\tt nc} is the number of components, for instance
representing different fluid variables, and {\tt (nx,ny,nz)} are the number of cells
in each respective spatial dimension.  For 2D problems, {\tt nz=1}.
...
All MPI calls are wrapped by functions in the {\tt parallel} module.  For
