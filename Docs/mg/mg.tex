...
Here we describe some of the general ideas behind the \boxlib\ multigrid.
...
There are two MG solvers: cell-centered and nodal.  
...
\begin{itemize}
\item {MG\_SMOOTHER\_GS\_RB}: a red-black Gauss-Seidel smoother

\item {MG\_SMOOTHER\_JACOBI}: a Jacobi smoother (not implemented for 
the dense nodal stencil)

\item {MG\_SMOOTHER\_MINION\_CROSS}

\item {MG\_SMOOTHER\_MINION\_FULL}

\item {MG\_SMOOTHER\_EFF\_RB}
\end{itemize}
...
\MyArrow[1em]{start1}{end1}{colorii,line width=2pt}
  \begin{itemize} 
  \item \tikzmark{end1} {\tt cc\_mg\_tower\_smoother}:

  \item {\tt compute\_defect}:

  \item\tikzmark{start1} {\tt ml\_restriction}:
  \end{itemize}
