module make_explicit_thermal_module
  ...
  public :: make_explicit_thermal, make_thermal_coeffs, &
            make_explicit_thermal_dt

contains 

  ! estimate the explicit diffusion timescale
  subroutine make_explicit_thermal_dt(mla,the_bc_tower,dx,s,dt)
  ...
  end subroutine make_explicit_thermal_dt

  subroutine make_explicit_thermal_dt_2d(lo,hi,        &
                                         s,ng_s,       &
                                         dx,dt)
  ...
  end subroutine make_explicit_thermal_dt_2d
       ...
       do i=1,s(n)%nboxes
          ...
          case (2)
             call make_thermal_coeffs_2d(lo,hi,sp(:,:,1,:),ng_s,Tcoeffp(:,:,1,1),ng_T, &
                                         ...
                                         pcoeffp(:,:,1,1),ng_p)
          case default
             call bl_error("make_thermal_coeffs not yet implemented for non-2d")   
          end select
       end do
    enddo
  ...
  subroutine make_thermal_coeffs_2d(lo,hi,s,ng_s,Tcoeff,ng_T,hcoeff,ng_h, &
    ...
    do j=lo(2)-1,hi(2)+1
       do i=lo(1)-1,hi(1)+1
          ...
          den_eos(1) = s(i,j,rho_comp)
          temp_eos(1) = s(i,j,temp_comp)
          xn_eos(1,1:nspec) = s(i,j,spec_comp:spec_comp+nspec-1)/den_eos(1)
