...
!         call scalar_diags_3d(istep, sop(:,:,:,:), lo, hi, ng, dx, s0)
  ...
  subroutine scalar_diags_2d (istep, s,lo,hi,ng,dx,s0,p0)
    ...
    write(unit=file_name2,fmt='("p0",i4.4)') istep
    ...
    do j = lo(2), hi(2)
      ...
      write(92,*) (dble(j)+HALF)*dx(2),p0(j)
      write(93,*) (dble(j)+HALF)*dx(2),phse(j)
      write(94,*) (dble(j)+HALF)*dx(2),s0(j,rho_comp)
      mass  = mass  + rhoavg(j)
    ...
    do j = lo(2), hi(2)
       ...
       do i = lo(1), hi(1)
          ...
          deltap_avg =  deltap_avg + abs(p_eos(1)-p0(j))
       ...
       write(70,*)  (dble(j)+HALF)*dx(2),phsevs0
    enddo
    write(68,*)  ' '
    write(69,*)  ' '
    write(70,*)  ' '
