...
111 format(78('-'))
  ...
  if (restart < 0) then
     ...
     if ( horave_int > 0 .or. horave_deltat > ZERO) then
        ...
        call fabio_mkdir("horizontal_averages")

        call make_horizontalaverage(plot_file_name,mla,sold,uold,rho0_old, &
                                    div_coeff_old,w0,dx,time)
...
1107    format(' ')
        ...
        if (horave_int > 0 .or.  horave_deltat > ZERO) then
           if ( (horave_int > 0 .and. mod(istep,horave_int) .eq. 0) .or. &
                (horave_deltat > ZERO .and. &
                 mod(time - dt,horave_deltat) > mod(time,horave_deltat)) ) then
              ...
              print *, "Going into horizontal average"

              call make_horizontalaverage(plot_file_name,mla,snew,unew,rho0_new, &
                                          div_coeff_old, w0, dx, time)
