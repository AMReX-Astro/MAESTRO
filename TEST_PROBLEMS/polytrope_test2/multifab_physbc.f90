...
contains
  ...
  subroutine physbc_2d(s,lo,hi,ng,bc,icomp)
    ...
    use bc_module
    use bl_constants_module
    use probin_module, only: Thigh, Tlow, dhigh, dlow
    use variables, only: rho_comp, temp_comp, rhoh_comp, spec_comp, press_comp
    ...
    if (bc(2,2) .eq. EXT_DIR) then
       ...
       if (icomp .eq. rho_comp+2) then ! density
          ...
          s(:,hi(2)+1:hi(2)+ng) = 2.5d0*Thigh*dhigh
       else if (icomp .eq. temp_comp+2) then ! temperature
          s(:,hi(2)+1:hi(2)+ng) = Thigh
       else if (icomp .eq. spec_comp+2) then ! scalar
          s(:,hi(2)+1:hi(2)+ng) = dhigh
       else if (icomp .eq. press_comp+2) then ! pressure
          s(:,hi(2)+1:hi(2)+ng) = dhigh*Thigh
       end if
