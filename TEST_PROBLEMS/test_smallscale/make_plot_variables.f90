...
contains
  ...
  subroutine make_tfromrho_2d(t,tpert,rhopert,machno,deltagamma,spert,s,u,lo,hi,ng,s0, &
                             tempbar,p0)
  ...
  subroutine make_tfromrho_3d_cart(t,tpert,rhopert,machno,deltagamma,spert,s,u,lo,hi,ng,s0, &
                                  tempbar,p0)
    ...
    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
             ...
             vel = sqrt(u(i,j,k,1)*u(i,j,k,1) + u(i,j,k,2)*u(i,j,k,2) &
                  + u(i,j,k,3)*u(i,j,k,3))
  ...
  subroutine make_XfromrhoX (plotdata,comp,s)
    ...
    do i = 1, s%nboxes
       ...
       select case (dm)
       case (2)
          call make_XfromrhoX_2d(pp(:,:,1,comp:),sp(:,:,1,1),sp(:,:,1,spec_comp:), lo, hi, ng)
       case (3)
          call make_XfromrhoX_3d(pp(:,:,:,comp:),sp(:,:,:,1),sp(:,:,:,spec_comp:), lo, hi, ng)
       end select
    end do
