...
1104 format(' ')
  ...
  contains
         ...
         do i = 1, nboxes(unew(n))
            ...
            case (2)
               call hg_update_2d(proj_type, upn(:,:,1,:), ng_un, uon(:,:,1,:), ng_uo, &
                                 ...
                                 ng_rh, pp(:,:,1,1), ng_p, ph(:,:,1,1), ng_h, dp(:,:,1,1), ng_d, dt)
            case (3)
    ...
    subroutine hg_update_2d(proj_type,unew,ng_un,uold,ng_uo,gpi,ng_gp,gphi,ng_gh, &
                            rhohalf,ng_rh,pi,ng_p,phi,ng_h,divcoeff,ng_d,dt)
      ...
      real(kind=dp_t), intent(in   ) ::    uold(-ng_uo:,-ng_uo:,:)
      real(kind=dp_t), intent(inout) ::   gpi(-ng_gp:,-ng_gp:,:)
      real(kind=dp_t), intent(inout) ::    gphi(-ng_gh:,-ng_gh:,:)
      real(kind=dp_t), intent(in   ) :: rhohalf(-ng_rh:,-ng_rh:)
      real(kind=dp_t), intent(inout) ::    pi(-ng_p :,-ng_p :)
      real(kind=dp_t), intent(in   ) ::     phi(-ng_h :,-ng_h :)
      real(kind=dp_t), intent(in   ) :: divcoeff(-ng_d:,-ng_d:)
      real(kind=dp_t), intent(in   ) :: dt
      ...
      !     Subtract off the density-weighted gradient.
      unew(0:nx,0:ny,1) = unew(0:nx,0:ny,1) - gphi(0:nx,0:ny,1)/rhohalf(0:nx,0:ny)*divcoeff(0:nx,0:ny) 
      unew(0:nx,0:ny,2) = unew(0:nx,0:ny,2) - gphi(0:nx,0:ny,2)/rhohalf(0:nx,0:ny)*divcoeff(0:nx,0:ny)
      ...
      if ( (proj_type .eq. initial_projection_comp) .or. &
         ...
         gphi(0:nx,0:ny,1) = gphi(0:nx,0:ny,1)*divcoeff(0:nx,0:ny)
         gphi(0:nx,0:ny,2) = gphi(0:nx,0:ny,2)*divcoeff(0:nx,0:ny)
