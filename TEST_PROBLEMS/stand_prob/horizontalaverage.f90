...
contains
  ...
  subroutine make_horizontalaverage(plot_file_name,mla,s,u,rho0,div_coeff,w0,dx,time)
    ...
    real(kind=dp_t), allocatable :: sum_proc(:,:,:)
    real(kind=dp_t), allocatable :: sum(:,:,:)
    real(kind=dp_t), allocatable :: ave(:,:,:)
    real(kind=dp_t), allocatable :: outputs(:,:)
    ...
    if (spherical .eq. 1) then
       ...
       allocate(sum_proc(0:nr_fine-1,nlevs,nsum))
       allocate(sum     (0:nr_fine-1,nlevs,nsum))
       allocate(ave     (nlevs,0:nr_fine-1,nsum))
       allocate(outputs (0:nr_fine-1,nout))
    ...
    if (spherical .eq. 0) then
       ...
       do n=1,nlevs
          ...
          do i=1,nsum
             call parallel_reduce(sum(:,n,i), sum_proc(:,n,i), MPI_SUM, &
                               proc = parallel_IOProcessorNode())
          end do

          ! compute ave by normalizing sum
          if (parallel_IOProcessor()) then
             do i=1,numdisjointchunks(n)
                do r=r_start_coord(n,i),r_end_coord(n,i)
                   ave(n,r,:) = sum(r,n,:) / dble(ncell(r,n))
                end do
             end do
          end if
...
1000 format(1x,16(g24.10,1x))
1001 format("# time: ",(g24.10,1x))
1002 format("#",16(a24,1x))
   ...
   if (parallel_IOProcessor()) then
      ...
      write (un, *) " "
      write (un, 1001) time
      write (un, 1002) "location","KE","PE","u","v","r","KE1","KE2","KE3","KE4","KE5","KE6","KE7","KE8","KE9","KE10"
      ...
      do i = 1, nmodes
         do r = 0, nr(1)-1
            outputs(r,5+i) = ave(1,r,5) * ( ave(1,r,5+i)*ave(1,r,5+i) &
                              + ave(1,r,5+nmodes+i)*ave(1,r,5+nmodes+i) &
                              + ave(1,r,5+2*nmodes+i)*ave(1,r,5+2*nmodes+i) &
                              + ave(1,r,5+3*nmodes+i)*ave(1,r,5+3*nmodes+i) )
         end do
      end do

      do r=0,nr(1)-1
         rloc=prob_lo(2)+(dble(r)+HALF)*dr(1)
         write (un, 1000) rloc, outputs(r,:)
      end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine sum_energy_2d(s,u,rhob,div_coeff,w0,dx,sum,lo,hi,ng_u,ng_s)

    use variables, only: rho_comp
    use bl_constants_module, only: HALF, ZERO, M_PI
    use probin_module, only: prob_lo, prob_hi, grav_const
    ...
    do j = lo(2),hi(2)
       ...
       do i = lo(1),hi(1)
          ...
          sum(j,1) = sum(j,1) + HALF * rhob(j) * velsqr
          sum(j,2) = sum(j,2) + HALF * grav_const**2 * rhob(j) / N2(j) &
                     * (rho1/rhob(j))**2
          sum(j,3) = sum(j,3) + u(i,j,1)
          sum(j,4) = sum(j,4) + u(i,j,2)
          sum(j,5) = sum(j,5) + s(i,j,rho_comp)
          do k = 1,nmodes
             sum(j,nother+k) = sum(j,nother+k) + u(i,j,1)*sin(2.0d0*M_PI*x*k/Lx)
             sum(j,nother+nmodes+k) = sum(j,nother+nmodes+k) + &
                                        u(i,j,1)*cos(2.0d0*M_PI*x*k/Lx)
             sum(j,nother+2*nmodes+k) = sum(j,nother+2*nmodes+k) + &
                                        u(i,j,2)*sin(2.0d0*M_PI*x*k/Lx)
             sum(j,nother+3*nmodes+k) = sum(j,nother+3*nmodes+k) + &
                                        u(i,j,2)*cos(2.0d0*M_PI*x*k/Lx)
             sum(j,nother+4*nmodes+k) = sum(j,nother+4*nmodes+k) + &
                                        s(i,j,rho_comp)*sin(2.0d0*M_PI*x*k/Lx)
             sum(j,nother+5*nmodes+k) = sum(j,nother+5*nmodes+k) + &
                                        s(i,j,rho_comp)*cos(2.0d0*M_PI*x*k/Lx) 
          end do
