...
!        .1250_dp_t * 0.5_dp_t * (1.0_dp_t + tanh((2.0-r2)))  ) )
  ...
  subroutine init_base_state (n_base,s0,temp0,p0,gam1,dx,prob_lo,prob_hi)
    ...
    if (n_base .le. 512)  then
      print *,'READING model.hse.lowerBC.512'
      open(99,file="model.hse.lowerBC.512")
    else if (n_base .eq. 1024) then
      print *,'READING model.hse.lowerBC.1024'
      open(99,file="model.hse.lowerBC.1024")
    else if (n_base .eq. 1024) then
      print *,'DONT KNOW WHICH FILE TO READ ',n_base
      stop
    end if
