...
contains
  ...
  subroutine react_state(mla,tempbar_init,sold,snew,rho_omegadot,rho_Hnuc, &
                         rho_Hext,p0, &
                         dt,dx,the_bc_level)

    use probin_module, only: use_tfromp, do_heating, do_burning, &
         drive_initial_convection
    use variables, only: foextrap_comp, rho_comp, temp_comp, spec_comp, &
         rhoh_comp, nscal, rhoh_comp
    ...
    real(kind=dp_t) :: delta_x(nspec), enuc
    ...
    ! copy the input to the output; things that need updated will be below
    do n = 1, nlevs
    ...
    if(do_heating) then
       ...
       ! FIXME: should we just in-place update the new energy?
    ...
    if (do_burning) then
       ...
       ! put tempbar_init on Cartesian grid
       if (spherical == 1) then
          do n=1, nlevs
             ! tempbar_init_cart will hold the initial tempbar on a
             ! Cartesian grid to be used if drive_initial_convection is true
       ...
       do n = 1, nlevs
          do m = 1, nfabs(sold(n))
             sop => dataptr(sold(n), m)
             snp => dataptr(snew(n), m)
             rp => dataptr(rho_omegadot(n), m)
             hnp => dataptr(rho_Hnuc(n), m)
             hep => dataptr(rho_Hext(n), m)
!             mp => dataptr(mla%mask(n), m)
             lo = lwb(get_box(sold(n), m))
             hi = upb(get_box(sold(n), m))
             ...
             do k = klo, khi
                do j = jlo, jhi
                   do i = lo(1), hi(1)
                      ...
                      ! temperature - note that drive_initial_convection
                      !               wants 1d lateral average, tempbar_init
                      if (drive_initial_convection) then
                         if (spherical == 1) then
                            state_in % T = tcp(i,j,k,1)
                         else
                            select case(dm)
                            case (1)
                               idx = i
                            ...
                            state_in % T = tempbar_init(n,idx)
                         end if
                      else
                         state_in % T = sop(i,j,k,temp_comp)
                      end if

                      ! density and composition
                      state_in % rho = sop(i,j,k,rho_comp)
                      state_in % xn(:) = &
                           sop(i,j,k,spec_comp:spec_comp+nspec-1) / &
                           sop(i,j,k,rho_comp)
                      ! enthalpy - only used to determine energy change
                      state_in % h = sop(i,j,k,rhoh_comp) / sop(i,j,k,rho_comp)
                      ! FIXME: burner wants time
                      ! FIXME: mask for covered data
                      ! FIXME: subgrid burner wants dx info
                      call burner(state_in, state_out, dt, ZERO)
                      
                      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                      ! unload state data
                      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

                      ! only things that change in burner are mass fractions
                      ! and energy
                      snp(i,j,k,spec_comp:spec_comp+nspec-1) = &
                           state_out % xn(:) * sop(i,j,k,rho_comp)

                      ! rho_omegadot
                      delta_x = state_out % xn(:) - state_in % xn(:)
                      rp(i,j,k,:) = sop(i,j,k,rho_comp) * delta_x / dt

                      ! energy change
                      enuc = state_out % h - state_in % h
                      hnp(i,j,k,1) = sop(i,j,k,rho_comp) * enuc / dt

                      ! update enthalpy: nuclear + external heating
                      snp(i,j,k,rhoh_comp) = sop(i,j,k,rhoh_comp) + &
                           dt * ( hnp(i,j,k,1) + hep(i,j,k,1) )
                   end do
                end do
             end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !   ! default copy the input to the output; things that need updated will be
  !   do n = 1, nlevs
  !      call multifab_copy(snew(n), sold(n), nghost(sold(n)))
  !   end do
  ...
  !         ! use state_* as an easy container to pass things
  ...
  !                  ! temperature - note that drive_initial_convection wants
  !                  !               the 1d lateral average, tempbar_init
  !                  ! FIXME: better placement for this?!?
  !                  !        might be better to make different routine for
  !                  !        drive_iniital_convection ?
  !                  if (drive_initial_convection) then
  !                     select case(dm)
  !                     case (1)
  !                        idx = i
  ...
  !                       sop(i,j,k,rho_comp)
  !                  ! enthalpy - only used to determine energy change
  !                  state_in % e = sop(i,j,k,rhoh_comp) 

  !                  ! FIXME: burner wants time
  !                  ! FIXME: mask for covered data
  !                  ! FIXME: subgrid burner wants dx info
  !                  call burner(state_in, state_out, dt, 0.0)
  ...
  !                  ! energy change
  !                  enuc = state_out % e - state_in % e
  !                  hnp(i,j,k) = sop(i,j,k,rho_comp) * enuc / dt

  !                  ! update enthalpy: nuclear + external heating
  !                  snp(i,j,k,rhoh_comp) = sop(i,j,k,rhoh_comp) + &
  !                       dt * ( hnp(i,j,k) + hep(i,j,k) )
  !               end do
  !            end do
  !         end do
