  ...
  subroutine fill_ghost_base(nlevs,s0,is_cell_centered)
    ...
    if (is_cell_centered) then
       ...
       do n=nlevs,2,-1
          ...
          if (r_start_coord(n,1) .ne. 0) then
             s0(n,r_start_coord(n,1)-1) = -(1.d0/5.d0)*s0(n,r_start_coord(n,1)+1) &
                  + (2.d0/3.d0)*s0(n,r_start_coord(n,1)) &
                  + (8.d0/15.d0)*s0(n-1,r_start_coord(n,1)/2-1)
          end if

          if (r_end_coord(n,1) .ne. nr(n)-1) then
             s0(n,r_end_coord(n,1)+1) = -(1.d0/5.d0)*s0(n,r_end_coord(n,1)-1) &
                  + (2.d0/3.d0)*s0(n,r_end_coord(n,1)) &
                  + (8.d0/15.d0)*s0(n-1,(r_end_coord(n,1)+1)/2)
          end if
       ...
       do n=nlevs,2,-1
          ...
          if (r_start_coord(n,1) .ne. 0) then
             s0(n,r_start_coord(n,1)-1) = -THIRD*s0(n,r_start_coord(n,1)+1) &
                  + s0(n,r_start_coord(n,1)) + THIRD*s0(n-1,r_start_coord(n,1)/2-1)
          end if
          
          if (r_end_coord(n,1)+1 .ne. nr(n)) then
             s0(n,r_end_coord(n,1)+2) = -THIRD*s0(n,r_end_coord(n,1)) &
                  + s0(n,r_end_coord(n,1)+1) + THIRD*s0(n-1,(r_end_coord(n,1)+2)/2)
          end if
