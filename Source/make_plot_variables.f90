...
!     Local variables
  ...
  subroutine make_deltaT (plotdata,comp,state,p0,temp0, dx)
    ...
    do i = 1, state%nboxes
       ...
       select case (dm)
       case (2)
          call makedeltaT_2d(tp(:,:,1,comp),sp(:,:,1,:), lo, hi, ng, p0, temp0)
       case (3)
       end select
    end do
  ...
  subroutine makedeltaT_2d (dT,state,lo,hi,ng,p0,temp0)
