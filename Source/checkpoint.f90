...
contains
  ...
  subroutine checkpoint_write(dirname, mfs, mfs_nodal, dSdt, S_cc_old, S_cc_new, &
    ...
    call fabio_ml_multifab_write_d(S_cc_old, rrs(:,1), sd_name, nOutFiles = nOutFiles, lUsingNFiles = lUsingNFiles)
    ...
    call fabio_ml_multifab_write_d(S_cc_new, rrs(:,1), sd_name, nOutFiles = nOutFiles, lUsingNFiles = lUsingNFiles)
...
!   Read the S_cc_old data into a multilevel multifab.
    write(unit=sd_name, fmt='(a,"/Source_old")') trim(dirname)
    call fabio_ml_multifab_read_d(S_cc_old, sd_name)

!   Read the S_cc_new data into a multilevel multifab.
    write(unit=sd_name, fmt='(a,"/Source_new")') trim(dirname)
    call fabio_ml_multifab_read_d(S_cc_new, sd_name)
