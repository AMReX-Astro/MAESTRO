...
contains
  ...
  subroutine update_velocity(uold,unew,umac,uedge,force,w0,w0_cart,w0_force,w0_force_cart, &
                             dx,dt,sponge,do_sponge)
    ...
    do i = 1, uold%nboxes
       ...
       case (2)
          call update_velocity_2d(uop(:,:,1,:), unp(:,:,1,:), &
                                  ...
                                  uepx(:,:,1,:), uepy(:,:,1,:), &
                                  fp(:,:,1,:), w0, w0_force, &
                                  lo, hi, ng, dx, dt, spp(:,:,1,1), &
                                  do_sponge)
       case (3)
          wmp   => dataptr(umac(3),i)
          uepz  => dataptr(uedge(3),i)
          w0p   => dataptr(w0_cart,i)
          w0fp  => dataptr(w0_force_cart,i)
