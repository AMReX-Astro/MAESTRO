...
contains
 ...
 subroutine mk_vel_force_2d_polar(vel_force,ng_f,is_final_update, &
    ...
    integer        , intent(in   ) :: lo(:),hi(:),ng_f,ng_gp,ng_s,ng_ue,ng_w,ng_n,ng_gw
    real(kind=dp_t), intent(inout) :: vel_force(lo(1)-ng_f :,lo(2)-ng_f :,:)
    logical        , intent(in   ) :: is_final_update
    real(kind=dp_t), intent(in   ) ::     normal(lo(1)-ng_n :,lo(2)-ng_n :,:)
    real(kind=dp_t), intent(in   ) ::       uedge(lo(1)-ng_ue:,lo(2)-ng_ue:)
    real(kind=dp_t), intent(in   ) ::       vedge(lo(1)-ng_ue:,lo(2)-ng_ue:)
       ...
       !$OMP PARALLEL DO PRIVATE(i,j,Ut_dot_er)
        do j=lo(2),hi(2)
            do i=lo(1),hi(1)
                ...
                Ut_dot_er = &
                        HALF*(uedge(i,j)+uedge(i+1,j  ))*normal(i,j,1) + &
                        HALF*(vedge(i,j)+vedge(i  ,j+1))*normal(i,j,2)
