...
contains
      ...
      if (spherical .eq. 1) then
         ...
         do i=1,rho0_cart%nboxes
            ...
            call put_1d_array_on_cart_3d_sphr(n,.false.,.false.,rhoh0_halftime,rhp, &
                                              lo,hi,dx(n,:),2,0)
            call put_1d_array_on_cart_3d_sphr(n,.false.,.false.,t0_halftime,tp, &
                                              lo,hi,dx(n,:),2,0)
         enddo
  ...
  subroutine makeRhoHfromTP(s,p0,bc,mla)
    ...
    do n=1,nlevs
       do i = 1, s(n)%nboxes
          ...
          select case (dm)
          case (2)
             call makeRhoHfromTP_2d(sop(:,:,1,:), ng_s, lo, hi, p0(n,:))
          case (3)
             call makeRhoHfromTP_3d(sop(:,:,:,:), ng_s, lo, hi, p0(n,:))             
          end select
       end do
    enddo
  ...
  end subroutine makeRhoHfromTP

  subroutine makeRhoHfromTP_2d(s,ng_s,lo,hi,p0)
  ...
  end subroutine makeRhoHfromTP_2d
  
  subroutine makeRhoHfromTP_3d(s,ng_s,lo,hi,p0)
