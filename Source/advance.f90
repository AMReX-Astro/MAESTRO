...
contains
  ...
  subroutine advance_timestep(init_mode,mla,uold,sold,unew,snew, &
    ...
    real(dp_t) ::        grav_cell_nph(nlevs,0:nr_fine-1)
    real(dp_t) ::        grav_cell_new(nlevs,0:nr_fine-1)
    real(dp_t) ::             rho0_nph(nlevs,0:nr_fine-1)
    real(dp_t) ::               p0_nph(nlevs,0:nr_fine-1)
    real(dp_t) ::   p0_minus_pthermbar(nlevs,0:nr_fine-1)
    real(dp_t) ::            pthermbar(nlevs,0:nr_fine-1)
    real(dp_t) ::             w0_force(nlevs,0:nr_fine-1)
    real(dp_t) ::                 Sbar(nlevs,0:nr_fine-1)
    real(dp_t) ::        div_coeff_nph(nlevs,0:nr_fine-1)
    real(dp_t) ::          rho_Hextbar(nlevs,0:nr_fine-1)
    real(dp_t) ::              rhoh0_1(nlevs,0:nr_fine-1)
    real(dp_t) ::              rhoh0_2(nlevs,0:nr_fine-1)
    real(dp_t) ::        gamma1bar_old(nlevs,0:nr_fine-1)
