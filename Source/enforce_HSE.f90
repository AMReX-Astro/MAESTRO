...
contains
  ...
  subroutine enforce_HSE(rho0,p0,grav_cell)
    ...
    ! we start at r=1, using the pressure at r=0 as a reference point,
    ! i.e., we are numerically intergrating from the "bottom"
    ! later, we will offset the entire pressure so we integrate from the "top"
    ...
    do n=2,nlevs_radial
       ...
       if (spherical .eq. 1) then
          call bl_error('enforce_HSE: should not have multiple base levels for spherical')
       end if
