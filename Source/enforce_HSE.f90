...
contains
  ...
  subroutine enforce_HSE(rho0,p0,grav_cell)
    ...
    ! now integrate upwards from the bottom
    ! later, we will offset the entire pressure so we have effectively 
    ! integrated from the "top"
    ...
    do n=2,nlevs_radial
       do i=1,numdisjointchunks(n)
          ...
          if (r_start_coord(n,i) .eq. 0) then
             ! if we are at the bottom of the domain, use the old pressure as reference
             p0(n,0) = p0_old(n,0)
          ...
          ! now we need to look at the first coarser cell above this disjoint chunk
          ! and look at the mismatch between the integration performed over the
          ! finer cells vs. the coarse cells.  Then we need to offset the coarse
          ! cell above this point to sync up.
          
          ! first, compute the value of the pressure in the coarse cell above the
          ! disjointchunk.
