...
contains
  ...
  subroutine initscalardata(s,s0_init,p0_init,dx,the_bc_level,mla)
    ...
    do n=1,nlevs
       do i = 1, nfabs(s(n))
          ...
          case (2)
             if (polar .eq. 1) then
                call initscalardata_2d_polar(sop(:,:,1,:), lo, hi, ng, dx(n,:), s0_init(1,:,:), &
                                    p0_init(1,:))
  ...
  subroutine initscalardata_2d(s,lo,hi,ng,dx,s0_init,p0_init)
    ...
    ! if we are polar, we want to make sure that p0 is good, since that is
    ...
    ! initialize p0_cart
    call put_1d_array_on_cart_2d_polar(.false.,.false.,p0_init(:), &
    ...
    do j = lo(2), hi(2)
        do i = lo(1), hi(1)
            s(i,j,rho_comp) = ZERO
            do comp = spec_comp, spec_comp+nspec-1
                s(i,j,rho_comp) = s(i,j,rho_comp) + s(i,j,comp)
            enddo
        enddo
    enddo
       ...
       ! add an optional perturbation
        ...
        do j = lo(2), hi(2)
            ...
            do i = lo(1), hi(1)
                ...
                call perturb_2d_polar(x, y, p0_cart(i,j,1), s(i,j,:), &
                                        dens_pert, rhoh_pert, rhoX_pert, temp_pert, trac_pert)
