...
contains
  ...
  subroutine get_rho_Hext(mla,tempbar_init,s,rho_Hext,the_bc_level,dx,dt)
    ...
    if (symmetric_heat) then
       ...
       if (.not.( spherical.eq.1 .or. mla%dim.eq.2 )) then 
          call bl_error('symmetric heat only written for spherical or 2D geometries')
       end if
  ...
  subroutine get_symm_rho_Hext(rho_Hext,rho0,T0,nr,dr)
    ...
    use probin_module, only: prob_hi, prob_lo
