...
!          total luminosity (Integral(rho*H_ext dV))
...
!              including only points R <= 7.62d10
!          velocity components (std. average & Favre average)
!          avgerage magnitude of velocity
!          peak total velocity
!          x/y/z location of peak vel 
    ...
    do n = 1, nlevs
       ...
       do i = 1, s(n)%nboxes
          ...
          case (3)
             ...
             call bl_error("ERROR: 3-d diag not implmented")
       ...
       sum_data_local(  (dm+1)+1:2*(dm+1)) = rhovtot_local(:)
...
!FIXME!!
! need to think about how this would work for multilevel
...
! FIXME !!
! need to think about how to compute mass_core for my 2D model
       ...
       if (firstCall) then
          ...
          write (un1,1001) "time", &
                           ...
                           "int{rhovtot_x}/mass", "int{rhovtot_y}/mass", &
                            "int{rhovtot}/mass", &
                           "max{|vtot|}", &
                           "x(max{V})", "y(max{V})", "R{max{V})", &
                           "<Utot_x>", "<Utot_y>", "<Utot>", &
                           "int{rhoUtot_x}/mass", "int{rhoUtot_y}/mass", &
                           "int{rhoUtot}/mass", &
                           "max{|U + w0|}",  &
                           "x(max{U})", "y(max{U})", "R{max{U})", &
                           "max{Mach #}",&
                           "dt"
