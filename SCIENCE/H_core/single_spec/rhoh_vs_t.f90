...
contains
  ...
  subroutine makeHfromRhoT_edge_1d(sx,ng_se, &
    ...
    use probin_module, only: enthalpy_pred_type, small_temp, prob_lo
    use pred_parameters
  ...
  subroutine makeTfromRhoH(state,mla,the_bc_level,dx)
    ...
    do n=1,nlevs
       ...
       do i=1,nboxes(state(n))
          ...
          select case (dm)
          case (1)
             call makeTfromRhoH_1d(sp(:,1,1,:), lo, hi, ng, dx(n,:))
          case (2)
             call makeTfromRhoH_2d(sp(:,:,1,:), lo, hi, ng, dx(n,:))
          case (3)
             call makeTfromRhoH_3d(sp(:,:,:,:), lo, hi, ng, dx(n,:))
          end select
       end do
  ...
  subroutine makeTfromRhoH_1d(state,lo,hi,ng,dx)
    ...
    do i = lo(1), hi(1)
       ...
       call eos(x, eos_input_rh, den_eos, temp_eos, &
                npts, &
                xn_eos, &
                p_eos, h_eos, e_eos, &
                cv_eos, cp_eos, xne_eos, eta_eos, pele_eos, &
  ...
  subroutine makeTfromRhoH_2d(state,lo,hi,ng, dx)
    ...
    do j = lo(2), hi(2)
       ...
       do i = lo(1), hi(1)
          ...
          call eos(y, eos_input_rh, den_eos, temp_eos, &
                   npts, &
                   xn_eos, &
                   p_eos, h_eos, e_eos, &
                   cv_eos, cp_eos, xne_eos, eta_eos, pele_eos, &
  ...
  subroutine makeTfromRhoH_3d(state,lo,hi,ng, dx)
    ...
    use fill_3d_module
    use probin_module, only: prob_lo
    use geometry,      only: center, spherical
    ...
    do k = lo(3), hi(3)
       ...
       do j = lo(2), hi(2)
          ...
          do i = lo(1), hi(1)
             ...
             call eos(r, eos_input_rh, den_eos, temp_eos, &
                      npts, &
                      xn_eos, &
                      p_eos, h_eos, e_eos, &
                      cv_eos, cp_eos, xne_eos, eta_eos, pele_eos, &
  ...
  subroutine makeTfromRhoP(state,p0,mla,the_bc_level,dx)
    ...
    do n=1,nlevs
       ...
       do i=1,nboxes(state(n))
          ...
          select case (dm)
          case (1)
             call makeTfromRhoP_1d(sp(:,1,1,:),lo,hi,ng,p0(n,:), dx(n,:))
          case (2)
             call makeTfromRhoP_2d(sp(:,:,1,:),lo,hi,ng,p0(n,:), dx(n,:))
          case (3)
             if (spherical .eq. 1) then
                call makeTfromRhoP_3d_sphr(sp(:,:,:,:),lo,hi,ng,p0(1,:),dx(n,:))
             else
                call makeTfromRhoP_3d(sp(:,:,:,:),lo,hi,ng,p0(n,:), dx(n,:))
             end if
          end select
       end do
  ...
  subroutine makeTfromRhoP_1d(state,lo,hi,ng,p0, dx)
  ...
  subroutine makeTfromRhoP_2d(state,lo,hi,ng,p0, dx)
  ...
  subroutine makeTfromRhoP_3d(state,lo,hi,ng,p0, dx)
  ...
  subroutine makePfromRhoH(state,sold,peos,mla,the_bc_level, dx)
  ...
  subroutine makePfromRhoH_1d(state,temp_old,peos,lo,hi,ng_s,ng_so,ng_p, dx)
  ...
  subroutine makePfromRhoH_2d(state,temp_old,peos,lo,hi,ng_s,ng_so,ng_p, dx)
  ...
  subroutine makePfromRhoH_3d(state,temp_old,peos,lo,hi,ng_s,ng_so,ng_p, dx)
  ...
  subroutine makeTHfromRhoP(s,p0,bc,mla,dx)
    ...
    do n=1,nlevs
       do i = 1, nboxes(s(n))
          ...
          select case (dm)
          case (1)
             call makeTHfromRhoP_1d(sop(:,1,1,:), ng_s, lo, hi, p0(n,:), dx(n,:))
          case (2)
             call makeTHfromRhoP_2d(sop(:,:,1,:), ng_s, lo, hi, p0(n,:), dx(n,:))
          case (3)
             if (spherical .eq. 1) then
                call makeTHfromRhoP_3d_sphr(sop(:,:,:,:), ng_s, lo, hi, p0(1,:), dx(n,:))
             else
                call makeTHfromRhoP_3d(sop(:,:,:,:), ng_s, lo, hi, p0(n,:), dx(n,:))
             end if
          end select
       end do
    enddo
  ...
  subroutine makeTHfromRhoP_1d(s,ng_s,lo,hi,p0, dx)
  ...
  subroutine makeTHfromRhoP_2d(s,ng_s,lo,hi,p0, dx)
  ...
  subroutine makeTHfromRhoP_3d(s,ng_s,lo,hi,p0, dx)
