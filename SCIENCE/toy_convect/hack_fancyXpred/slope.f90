...
contains
  ...
  subroutine slopex_nolimit_2d(s,slx,lo,hi,ng,nvar,bc)
    ...
    if (slope_order .eq. 0) then
       ...
       do comp=1,nvar 
          do j = js-1,je+1 
             ...
             do i = is-2,ie+2 
                dxscr(i,cen) = half*(s(i+1,j,comp)-s(i-1,j,comp))
                dxscr(i,fromm) = dxscr(i,cen)
             enddo

             do i = is-1,ie+1 
                slx(i,j,comp) = two * two3rd * dxscr(i,cen) - &
                     sixth * (dxscr(i+1,fromm) + dxscr(i-1,fromm)) 
             enddo
  ...
  end subroutine slopex_nolimit_2d
  ...
  subroutine slopey_nolimit_2d(s,sly,lo,hi,ng,nvar,bc)
  ...
  end subroutine slopey_nolimit_2d
