...
contains
  ...
  subroutine mk_rhoX_flux(mla,sflux,etarhoflux,sold,sedge,sedge0,umac,w0,w0mac, &
    ...
    type(multifab) , intent(in   ) :: sold(:),sedge(:,:),sedge0(:,:)
    ...
    real(kind=dp_t), pointer :: sex0p(:,:,:,:)
    real(kind=dp_t), pointer :: sey0p(:,:,:,:)
    ...
    do n=1,nlevs
       do i=1, nboxes(sold(n))
          ...
          case (2)
             ...
             sey0p => dataptr(sedge0(n,2),i)
             vmp  => dataptr(umac(n,2),i)
             call mk_rhoX_flux_2d(sfxp(:,:,1,:), sfyp(:,:,1,:), ng_sf, &
                                  efp(:,:,1,1), ng_ef, &
                                  sexp(:,:,1,:), seyp(:,:,1,:), &
                                  sex0p(:,:,1,:), sey0p(:,:,1,:), ng_se, &
  ...
  subroutine mk_rhoX_flux_2d(sfluxx,sfluxy,ng_sf,etarhoflux,ng_ef,sedgex,sedgey, &
                             sedgex0,sedgey0,ng_se, &
    ...
    real(kind=dp_t), intent(inout) ::    sedgex0(lo(1)-ng_se:,lo(2)-ng_se:,:)
    real(kind=dp_t), intent(inout) ::    sedgey0(lo(1)-ng_se:,lo(2)-ng_se:,:)
...
!                sfluxx(i,j,comp) = umac(i,j)*sedgex(i,j,comp)
                ...
                sum = 0.d0
                do m=0,nspec-1
                   sum = sum + sedgex(i,j,spec_comp+m)
                end do
                sum = sedgex(i,j,comp) / sum

                sfluxx(i,j,comp) = umac(i,j)*(rho0_edge*sum &
                     + sedgex(i,j,comp) - sedgex0(i,j,comp))
...
!                     (vmac(i,j)+w0(j))*sedgey(i,j,comp)
                ...
                sum = 0.d0
                do m=0,nspec-1
                   sum = sum + sedgey(i,j,spec_comp+m)
                end do
                sum = sedgey(i,j,comp) / sum

                sfluxy(i,j,comp) = (vmac(i,j)+w0(j))*(rho0_edge*sum &
                     + sedgey(i,j,comp) - sedgey0(i,j,comp))
