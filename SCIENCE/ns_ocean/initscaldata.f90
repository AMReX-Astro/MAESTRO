...
contains
  ...
  subroutine initscalardata(s,s0_init,p0_init,dx,bc,mla)
    ...
    use probin_module, only: prob_lo, prob_hi, perturb_model, ns_pert_height
       ...
       do i = 1, nfabs(s(n))
          ...
          case (2)
             ...
             if (perturb_model .and. &
                  (ns_pert_height < prob_lo(2) .or. &
                   ns_pert_height > prob_hi(2))) &
          ...
          case (3)
             ...
             ! make sure our perturbation is within the domain
             if (perturb_model .and. &
                  (ns_pert_height < prob_lo(3) .or. &
                   ns_pert_height > prob_hi(3))) &
                   call bl_error("perturbation outside of the domain")
  ...
  subroutine perturb(distance, p0_init, s0_init,  &
    ...
    use probin_module, ONLY: ns_pert_size, ns_pert_factor
    ! apply an optional perturbation to the initial composition field
    ...
    real(kind=dp_t) :: XO0,XO,rad_pert
    ...
    rad_pert = -ns_pert_size**2 / (FOUR*log(HALF))

    XO0 = s0_init(spec_comp+io16-1)/s0_init(rho_comp)
