...
contains
  ...
  subroutine get_rho_Hext(mla,s,rho_Hext,dx,dt)
    ...
    do n=1,nlevs
       ...
       do i = 1, nfabs(s(n))
          ...
          case (3)
             call get_rho_Hext_3d(hp(:,:,:,1),ng_h,sp(:,:,:,:),ng_s,lo,hi,dx(n,:),dt)
          end select
       end do
  ...
  subroutine get_rho_Hext_3d(rho_Hext,ng_h,s,ng_s,lo,hi,dx,dt)
