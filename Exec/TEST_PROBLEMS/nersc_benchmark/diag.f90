...
contains
  ...
  subroutine diag(time,dt,dx,s,rho_Hnuc,rho_Hext,thermal,rho_omegadot, &
    ...
    if ( parallel_IOProcessor() ) then
       ! file1 -- nersc_benchmark_radvel_diag.out
       ...
       ! file2 -- nersc_benchmark_temp_diag.out
       ...
       ! file3 -- nersc_benchmark_enuc_diag.out
       ...
       ! file4 -- nersc_benchmark_vel_diag.out
...
1002 format(1x,16(g17.7,1x))
1003 format("#",16(a17,1x))
...
802 format("# ",a,a)
    ...
    if ( parallel_IOProcessor() ) then
       ...
       ! an existing file
       un1 = unit_new()
       inquire(file="nersc_benchmark_radvel_diag.out", exist=lexist)
       if ( lexist ) then
          open(unit=un1, file="nersc_benchmark_radvel_diag.out", &
               status="old", position="append")
       else
          open(unit=un1, file="nersc_benchmark_radvel_diag.out", status="new")
       endif

       un2 = unit_new()
       inquire(file="nersc_benchmark_temp_diag.out", exist=lexist)
       if ( lexist ) then
          open(unit=un2, file="nersc_benchmark_temp_diag.out", &
               status="old", position="append")
       else
       ...
       if ( lexist ) then
          open(unit=un4, file="nersc_benchmark_vel_diag.out", &
               status="old", position="append")
       else
          open(unit=un4, file="nersc_benchmark_vel_diag.out", status="new")
       endif
