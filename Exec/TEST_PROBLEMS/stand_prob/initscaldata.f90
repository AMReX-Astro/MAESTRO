...
contains
  ...
  subroutine initscalardata_2d(s,lo,hi,ng,dx,s0_init,p0_init)
    ...
    use probin_module, only: prob_lo, prob_hi, perturb_model, perturb_type, yhigh, ylow, amp, k, kz
    ...
    if (perturb_type .eq. 1) then
       ...
       do j = lo(2), hi(2)
          ...
          do i = lo(1), hi(1)
             ...
             s(i,j,rho_comp) = s(i,j,rho_comp)*(1.d0+amp*sin(2.d0*M_PI*x/Lx*k)*exp(-(y-ymid)*(y-ymid)/ywidth/ywidth))
       ...
       do j = lo(2), hi(2)
          ...
          do i = lo(1), hi(1)
             ...
             s(i,j,rho_comp) = s(i,j,rho_comp)*(1.d0+amp*sin(2.d0*M_PI*x/Lx*5.d0)*sin(2.d0*M_PI*y/Ly))
    ...
    else if (perturb_type .eq. 5) then
    ...
    else if (perturb_type .eq. 6) then
