module plot_variables_module
  ...
  public :: make_stability, make_ad_excess
...
contains
  ...
  !---------------------------------------------------------------------------
  subroutine make_stability(plotdata,comp_stability,state,normal, dx)
  ...
  subroutine make_stability_1d(stability, ng_ad, state, ng_s, lo, hi,dx)
  ...
  subroutine make_stability_2d(stability, ng_ad, state, ng_s, lo, hi,dx)
    ...
    do j = lo(2), hi(2)
       ...
       do i = lo(1), hi(1)
          ...
          call eos(y, eos_input_ps, den_eos, temp_eos, &
                   npts, &
                   xn_eos, &
                   p_eos, h_eos, e_eos, &
                   cv_eos, cp_eos, xne_eos, eta_eos, pele_eos, &
  ...
  subroutine make_stability_3d(stability, ng_ad, state, ng_s, lo, hi, dx)
...
! use this a base to write the function
  ...
  ! make_ad_excess
  !---------------------------------------------------------------------------
  subroutine make_ad_excess(plotdata,comp_ad_excess,state,normal,dx)
  ...
  subroutine make_ad_excess_1d(ad_excess, ng_ad, state, ng_s, lo, hi, dx)
  ...
  subroutine make_ad_excess_2d(ad_excess, ng_ad, state, ng_s, lo, hi, dx)
  ...
  subroutine make_ad_excess_3d(ad_excess, ng_ad, state, ng_s, lo, hi,dx)
  ...
  subroutine make_tfromH_1d(T,tpert,deltaP,ng_p,state,ng_s,lo,hi,p0,tempbar, dx)
  ...
  subroutine make_tfromH_2d(T,tpert,deltaP,ng_p,state,ng_s,lo,hi,p0,tempbar, dx)
  ...
  subroutine make_tfromH_3d_cart(T,tpert,deltaP,ng_p,state,ng_s,lo,hi,p0,tempbar, dx)
    ...
    do k = lo(3), hi(3)
       ...
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
             ...
             call eos(z, eos_input_rh, den_eos, temp_eos, &
                      npts, &
                      xn_eos, &
                      p_eos, h_eos, e_eos, &
                      cv_eos, cp_eos, xne_eos, eta_eos, pele_eos, &
  ...
  subroutine make_tfromp_1d(t,tpert,machno,cs,deltagamma,entropy,magvel, &
    ...
    use probin_module, only: plot_cs, prob_lo
    use bl_constants_module
