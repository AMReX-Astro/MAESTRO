...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine make_gamma(mla,gamma,s,p0,dx)
    ...
    do n = 1, nlevs
       do i = 1, nboxes(s(n))
          ...
          select case (dm)
          case (1)
             call make_gamma_1d(lo,hi,gamp(:,1,1,1),ng_g,sp(:,1,1,:),ng_s,p0(n,:),dx(n,:))
          case (2)
             call make_gamma_2d(lo,hi,gamp(:,:,1,1),ng_g,sp(:,:,1,:),ng_s,p0(n,:),dx(n,:))
          case (3)
             if (spherical .eq. 1) then
                call make_gamma_3d_sphr(lo,hi,gamp(:,:,:,1),ng_g,sp(:,:,:,:),ng_s,p0(1,:), &
                                        dx(n,:))
             else
                call make_gamma_3d(lo,hi,gamp(:,:,:,1),ng_g,sp(:,:,:,:),ng_s,p0(n,:),dx(n,:))
             end if
          end select
       end do
    end do
  ...
  subroutine make_gamma_1d(lo,hi,gamma,ng_g,s,ng_s,p0,dx)
  ...
  subroutine make_gamma_2d(lo,hi,gamma,ng_g,s,ng_s,p0, dx)
  ...
  subroutine make_gamma_3d(lo,hi,gamma,ng_g,s,ng_s,p0,dx)
