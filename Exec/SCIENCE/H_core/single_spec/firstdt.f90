...
contains
  ...
  subroutine firstdt_1d(n,u,ng_u,s,ng_s,force,ng_f,divu,ng_dU,p0,gamma1bar,lo,hi,dx,dt, &
    ...
    use bl_constants_module
    use probin_module, only: use_soundspeed_firstdt, use_divu_firstdt, prob_lo
  ...
  subroutine firstdt_3d(n,u,ng_u,s,ng_s,force,ng_f,divU,ng_dU,p0,gamma1bar,lo,hi,dx,dt, &
    ...
    real (kind = dp_t)  :: eps,dt_divu,dt_sound,gradp0,denom,rho_min, z
    integer             :: i,j,k
  ...
  subroutine firstdt_3d_sphr(u,ng_u,s,ng_s,force,ng_f,divU,ng_dU,p0,gamma1bar,lo,hi,dx, &
    ...
    use geometry,  only: nr, dr, nr_fine, center
    use variables, only: rho_comp, temp_comp, spec_comp
    use eos_module
    use network, only: nspec
    use bl_constants_module
    ...
    do k = lo(3), hi(3)
       ...
       do j = lo(2), hi(2)
          ...
          do i = lo(1), hi(1)
             ...
             pt_index_eos(:) = (/i, j, k/)
             rloc = sqrt( (x -center(1))**2 + (y -center(2))**2 + &
                  (z -center(3))**2)
