...
contains
  ...
  subroutine initscalardata_3d_sphr(s,lo,hi,ng,dx,s0_init,p0_init)
    ...
    use probin_module, only: prob_lo, perturb_model, init_model_eos_rt
    use init_perturb_module
    use eos_module, only: eos_input_rp, eos_input_rt, eos
    use eos_type_module
    ...
    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
             ...
             if (init_model_eos_rt) then
                ! If init_model_eos_rt == .true. then set (rho h) and p0
                call eos(eos_input_rt, eos_state, pt_index)
                s(i,j,k,rhoh_comp) = eos_state%rho*eos_state%h
                p0_cart(i,j,k,1)   = eos_state%p
             else
                ! Otherwise, set (rho h) and T
