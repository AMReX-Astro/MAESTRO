module initialize_module
  ...
  use multifab_module
  use bc_module
  use variables, only: nscal, rho_comp, rhoh_comp, temp_comp, pi_comp, ntrac
...
contains
  ...
  subroutine initialize_from_restart(mla,restart,dt,pmask,dx,uold,sold,gpi,pi, &
    ...
    integer :: n,ng_s,nr_fine_old,r,dm,nlevs,chk_nscal
    logical :: is_piless
    ...
    ! This is a temporary hack to make it possible for the latest Maestro to
    ! read in the old chkfiles that had no pi in the state multifab.
    chk_nscal = chkdata(1)%nc 
    is_piless = chk_nscal /= (nscal+dm*2)
    ...
    do n=1,nlevs
       ...
       if (is_piless) then
          !Effectively set pi_comp to 0 in sold while copying in the other components
          !from chkdata.  This is a bad hack that makes assumption about how 
          !variables.f90 is written.  It's not a viable long-term solution to
          !the problem of older chkfiles having no pi_comp in their state
          !multifabs.
          call setval(          sold(n), ZERO, all=.true.)
          call multifab_copy_c( sold(n),1,        chkdata(n),rho_comp+dm, pi_comp-1)
          call multifab_copy_c( sold(n),pi_comp+1,chkdata(n),pi_comp+dm,  ntrac)
       else
          call multifab_copy_c( sold(n),1,chkdata(n),rho_comp+dm      ,nscal)
       endif
       !    multifab_copy_c(  mdst,  dstcomp, msrc,      srccomp,          nc, ng, filter, bndry_reg_to_other, ngsrc)
       if (is_piless) then
          call multifab_copy_c(  gpi(n),1,       chkdata(n),rho_comp+dm+nscal-1,dm)
       else
    ...
    if (restart_into_finer .and. spherical .eq. 1) then
       ...
       call regrid(restart,mla,uold,sold,gpi,pi,dSdt,S_cc_old,dx,the_bc_tower, &
       ...
       ! compute gamma1bar
       call make_gamma1bar(mla,sold,gamma1,p0_old,dx)
