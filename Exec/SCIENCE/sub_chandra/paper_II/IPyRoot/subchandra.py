...
class SCSimulation(object):
  ...
  def getpltdata(self, machine, max=None):
    ...
    if machine.lower().strip() == 'titan':
      HPSS_BASE = '/proj/ast006/sub_chandra'
    ...
    #If not, spawn the subprocess that will generate the RV plot with VisIt on lens.
      ...
      if poll:
        ...
        pltfile = join(self.parent._scratch_dir, self.parent._label, 'plotfiles', 
            self.parent._label + '_plt{0:0=5d}'.format(self.timestamp[0]), 'Header')
...
class SCHotspots(object):
  """The hot cells from a particular sub-Chandra simulation plotfile/timestamp."""
  ##Shared class data##
  TEMP_COL = 0
  RHO_COL = 1
  ...
  def _binData(self, data, color_data, dx, cmap):
    ...
    #Loop through parallel lists of radius and temperature, sorted by radius.
    #Calculate the counts for each radial bin and the average temp for each bin, 
...
class SCPlotter(object):
  ...
  def plotMinMass(self, core, shell, fontsize='medium'):
    ...
    wk_patch = Polygon(vertices, hatch='x', facecolor='1.0', edgecolor='0.0', label='W&K2011')
...
def display_parameters(stage_dir, run_label):
  ...
  htmlstr += param_row(labellist, stage_dir, r'R$_{\mathrm{max}}$ [cm]', 'xmax')
