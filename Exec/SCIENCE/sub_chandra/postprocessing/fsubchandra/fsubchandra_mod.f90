...
module subchandra
  ...
  type temp_hist
     ...
     real(kind=dp_t) :: min_temp = 4.0d6
     real(kind=dp_t) :: max_temp = 4.0d9
...
contains
  ...
  subroutine parse_args(slicefile, pltfile, globals_only, fullstar, hpcnt, pf)
    ...
    character(len=512), intent(out) :: slicefile, pltfile
    type(plotfile),     intent(out) :: pf

    !Local data
    integer :: pffd, narg, iarg, indslsh, ierr
    character(len=512) :: fname, hpstring
            ...
            do jj = lbound(p,dim=2), ubound(p,dim=2)
               ...
               do ii = lbound(p,dim=1), ubound(p,dim=1)
                  ...
                  if ( any(fmask(ii*r1:(ii+1)*r1-1, &
                     ...
                     if (b<1) then
                        print *, 'WARNING! Cell temp below', thist%min_temp,   &
                                 ':', p(ii,jj,kk,sc%temp_comp)
                        print *, 'location: ', ii, jj, kk, xx, yy, zz
                        b=1
                     endif
  ...
  subroutine writeout(pf, slicefile, geo, sc, thist, glb, radav, hh)
    ...
    !Args
    type(plotfile), intent(in) :: pf 
    character(len=512), intent(in) :: slicefile
    type(geometry), intent(in) :: geo
    type(state_comps), intent(in) :: sc
    ...
    character(len=512) :: hotfile, histfile
