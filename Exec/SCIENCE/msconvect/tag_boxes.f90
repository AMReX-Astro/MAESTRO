...
contains
  ...
  subroutine tag_boxes(mf,tagboxes,dx,lev,aux_tag_mf)
   ...
   ! if (present(aux_tag_mf)) do nothing. Not implemented
    ...
    do while(next_tile(mfi,i))
       ...
       case  (3)
          call tag_boxes_3d(tp(:,:,:,1),tlo(1:3),mfp(:,:,:,rho_comp),mflo(1:3),lo,hi,dx,lev)
       end select
    end do
    !$omp end parallel
  ...
  subroutine tag_boxes_3d(tagbox,tlo,mf,mflo,lo,hi,dx,lev)
    ...
    select case(lev)
    case (1)
      ! tag all boxes where the density is between lo_dens_tag  and hi_dens_tag
	do k = lo(3),hi(3)
	    do j = lo(2),hi(2)
	      do i = lo(1),hi(1)
		if (mf(i,j,k) .gt. lo_dens_tag .and. mf(i,j,k) .lt. hi_dens_tag) then
		    tagbox(i,j,k) = .true.
		  end if
	      end do
	    end do
    ...
    case default
       ! tag all boxes where the density is between lo_dens_tag * ((1+steep_tag) ** lev) and hi_dens_tag * ((1-steep_tag) ** lev)
      lo_tag = (1. + (steep_tag ** lev)) * lo_dens_tag
      hi_tag = (1. - (steep_tag ** lev)) * hi_dens_tag
      if (lo_tag .lt. hi_tag) then 
	do k = lo(3),hi(3)
	    do j = lo(2),hi(2)
	      do i = lo(1),hi(1)
