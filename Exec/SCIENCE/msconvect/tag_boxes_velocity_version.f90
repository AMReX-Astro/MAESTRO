...
contains
  ...
  subroutine tag_boxes_2d(tagbox,tlo,up,uplo,maxvel,lo,hi,dx,lev)
    ...
    use probin_module, only: tag_val_steep,tag_vel_extreme,tag_vel_min
    ...
    tag_vel = max(tag_vel,min(tag_vel_min,maxvel))
  ...
  subroutine tag_boxes_init_2d(tagbox,tlo,mf_rho,mflo,maxdens,lo,hi,dx,lev)
    ...
    !we use lev + 2 here to reduce the number of initially tagged cells.
    !We redo this after initialisation anyway
    tag_dens = maxdens - maxdens/(dble(lev+2)*tag_val_steep)
