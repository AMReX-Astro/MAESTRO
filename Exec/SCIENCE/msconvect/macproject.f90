...
contains 
  ...
  subroutine macproject(mla,umac,phi,rho,dx,the_bc_tower, &
    ...
    use mg_eps_module, only: eps_mac, eps_mac_max, mac_level_factor, eps_mac_abs
...
!    real(dp_t)                   :: umac_norm(mla%nlevel)
    ...
    if ((spherical .eq. 1 .or. polar .eq. 1) .and. use_beta0_1d) then
       call bl_error('CANT HAVE SPHERICAL .or. POLAR .eq. 1 and 1D DIV_COEFF IN MACPROJECT')
    end if

    if ((spherical .eq. 0 .and. polar .eq. 0) .and. use_beta0_cart_edge) then
