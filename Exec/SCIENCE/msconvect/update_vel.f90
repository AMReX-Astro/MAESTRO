...
contains
  ...
  subroutine update_velocity(uold,unew,umac,uedge,force,w0,w0mac,dx,dt, &
    ...
    use probin_module, only: do_sponge, sponge_mode
  ...
  subroutine update_velocity_2d(uold,ng_uo,unew,ng_un,umac,vmac,ng_um,uedgex,uedgey,ng_ue, &
    ...
    use bl_constants_module
    use probin_module, only: do_sponge, sponge_mode, prob_lo
    ...
    if (polar .eq. 0) then
       ...
       !$OMP PRIVATE(w0_gradur,w0_gradvr,urad,x,y,r,cphi,sphi,vr,dvx,dvy)
          do j = lo(2), hi(2)
             do i = lo(1), hi(1)
                ...
                ! Add the sponge
                if (do_sponge) then
                    if (sponge_mode .eq. 1 .or. sponge_mode .eq. 2) then
                        unew(i,j,:) = unew(i,j,:) * sponge(i,j)
                        ...
                        !calculate radial velocity and the cartesian components
                        vr = cphi * unew(i,j,1) + sphi * unew(i,j,2)
                        dvx = vr * cphi
                        dvy = vr * sphi
                        
                        !apply sponge on radial velocity 
