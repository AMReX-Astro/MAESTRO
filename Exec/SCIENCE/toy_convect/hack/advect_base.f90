...
module advect_base_module
  ...
  public :: advect_base_dens, advect_base_enthalpy, advect_base_species
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine advect_base_species(w0,rhoX0_old,rhoX0_new,dt)
    ...
    real(kind=dp_t), intent(  out) ::            rhoX0_new(:,0:,:)
    real(kind=dp_t), intent(in   ) :: dt
    ...
    call build(bpt, "advect_base_species")

    if (spherical .eq. 0) then
       call advect_base_species_planar(w0,rhoX0_old,rhoX0_new,dt)
       do i = 1, nspec
          call restrict_base(rhoX0_new(:,:,i),.true.)
          call fill_ghost_base(rhoX0_new(:,:,i),.true.)
       enddo

    else
       call advect_base_species_spherical(w0,rhoX0_old,rhoX0_new,dt)
    end if

    call destroy(bpt)
       
  end subroutine advect_base_species

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  subroutine advect_base_species_planar(w0,rhoX0_old,rhoX0_new,dt)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do m = 1, nspec
       do n=1,nlevs
          do i=1,numdisjointchunks(n)
             do r=r_start_coord(n,i),r_end_coord(n,i)
                force(n,r) = -rhoX0_old(n,r,m) * (w0(n,r+1) - w0(n,r)) / dr(n)
             end do
          end do
       end do
       
       call make_edge_state_1d(rhoX0_old(:,:,m),edge,w0,force,dt)
        
       ! update rho_0
       do n=1,nlevs
          do i=1,numdisjointchunks(n)
             do r=r_start_coord(n,i),r_end_coord(n,i)
                rhoX0_new(n,r,m) = rhoX0_old(n,r,m) &
                     - dt / dr(n) * (edge(n,r+1) * w0(n,r+1) - edge(n,r) * w0(n,r)) 
             end do
          end do
       end do
  ...
  end subroutine advect_base_species_planar

  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  subroutine advect_base_species_spherical(w0,rhoX0_old,rhoX0_new,dt)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do m = 1, nspec
       ...
       !$OMP PARALLEL DO PRIVATE(r)
       do r=0,nr_fine-1
          force(1,r) = -rhoX0_old(1,r,m) * (w0(1,r+1) - w0(1,r)) / dr(1) - &
               2.0_dp_t*rhoX0_old(1,r,m)*HALF*(w0(1,r) + w0(1,r+1))/r_cc_loc(1,r)
       end do
       !$OMP END PARALLEL DO
       ...
       !$OMP PARALLEL DO PRIVATE(r)
       do r=0,nr_fine-1
          rhoX0_new(1,r,m) = rhoX0_old(1,r,m) - dtdr/r_cc_loc(1,r)**2 * &
               (r_edge_loc(1,r+1)**2 * edge(1,r+1) * w0(1,r+1) - &
                r_edge_loc(1,r  )**2 * edge(1,r  ) * w0(1,r  ))
       end do
       !$OMP END PARALLEL DO
  ...
  end subroutine advect_base_species_spherical
