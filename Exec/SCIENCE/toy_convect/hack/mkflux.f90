...
contains
  ...
  subroutine mk_rhoX_flux(mla,sflux,etarhoflux,sold,sedge,umac,w0,w0mac, &
    ...
    real(kind=dp_t), intent(in   ) :: rhoX0_old(:,0:,:),rhoX0_edge_old(:,0:,:)
    type(multifab) , intent(in   ) :: rho0mac_old(:,:)
    real(kind=dp_t), intent(in   ) :: rhoX0_new(:,0:,:),rhoX0_edge_new(:,0:,:)
       ...
       do i=1, nboxes(sold(n))
          ...
          case (2)
             ...
             call mk_rhoX_flux_2d(sfxp(:,:,1,:), sfyp(:,:,1,:), ng_sf, &
                                  ...
                                  rhoX0_old(n,:,:), rhoX0_edge_old(n,:,:), &
                                  rhoX0_new(n,:,:), rhoX0_edge_new(n,:,:), &
                                  rho0_predicted_edge(n,:), &
                                  w0(n,:),startcomp,endcomp,lo,hi)
          case (3)
             sfyp => dataptr(sflux(n,2),i)
             ...
             if(spherical .eq. 0) then
                call mk_rhoX_flux_3d_cart(sfxp(:,:,:,:), sfyp(:,:,:,:), sfzp(:,:,:,:), &
                                          ...
                                          ng_um, rhoX0_old(n,:,:), rhoX0_edge_old(n,:,:), &
                                          rhoX0_new(n,:,:), rhoX0_edge_new(n,:,:), &
                                          rho0_predicted_edge(n,:), &
                                          w0(n,:),startcomp,endcomp,lo,hi)
                ...
                call bl_error("ERROR: spherical (rho X)' not written")
  ...
  subroutine mk_rhoX_flux_1d(sfluxx,ng_sf,etarhoflux,ng_ef,sedgex,ng_se, &
                             umac,ng_um,rhoX0_edge_old,rhoX0_edge_new, &
    ...
    real(kind=dp_t), intent(in   ) :: rhoX0_edge_old(0:,:), rhoX0_edge_new(0:,:)
    real(kind=dp_t), intent(in   ) :: rho0_predicted_edge(0:)
    real(kind=dp_t), intent(in   ) :: w0(0:)
    integer        , intent(in   ) :: startcomp,endcomp
    ...
    do comp = startcomp, endcomp
       ...
       ! create x-fluxes
       do i = lo(1),hi(1)+1
          rhoX0_edge = HALF*(rhoX0_edge_old(i,comp-spec_comp+1)+rhoX0_edge_new(i,comp-spec_comp+1))
          sfluxx(i,comp) = &
               (umac(i)+w0(i))*(rhoX0_edge+sedgex(i,comp))
  ...
  subroutine mk_rhoX_flux_2d(sfluxx,sfluxy,ng_sf,etarhoflux,ng_ef,sedgex,sedgey,ng_se, &
                             umac,vmac,ng_um,rhoX0_old,rhoX0_edge_old,rhoX0_new,rhoX0_edge_new, &
                             rho0_predicted_edge,w0,startcomp,endcomp,lo,hi)
    ...
    do comp = startcomp, endcomp
       ...
       ! create x-fluxes
       do j=lo(2),hi(2)
          rhoX0_edge = HALF*(rhoX0_old(j,comp-spec_comp+1)+rhoX0_new(j,comp-spec_comp+1))
          do i=lo(1),hi(1)+1
             sfluxx(i,j,comp) = umac(i,j)* &
                  (rhoX0_edge + sedgex(i,j,comp))
          end do
       end do
             
       ! create y-fluxes
       do j = lo(2),hi(2)+1
          rhoX0_edge = HALF*(rhoX0_edge_old(j,comp-spec_comp+1)+rhoX0_edge_new(j,comp-spec_comp+1))
          do i = lo(1),hi(1)
             sfluxy(i,j,comp) = &
                  (vmac(i,j)+w0(j))*(rhoX0_edge+sedgey(i,j,comp))
  ...
  subroutine mk_rhoX_flux_3d_cart(sfluxx,sfluxy,sfluxz,ng_sf,etarhoflux,ng_ef, &
                                  ...
                                  rhoX0_old,rhoX0_edge_old,rhoX0_new,rhoX0_edge_new, &
                                  rho0_predicted_edge,w0,startcomp,endcomp,lo,hi)
   ...
   ! local
    ...
    do comp = startcomp, endcomp
       ...
       do k=lo(3),hi(3)
          rhoX0_edge = HALF*(rhoX0_old(k,comp-spec_comp+1)+rhoX0_new(k,comp-spec_comp+1))
          do j=lo(2),hi(2)
             do i=lo(1),hi(1)+1
                sfluxx(i,j,k,comp) = &
                     umac(i,j,k)*(rhoX0_edge+sedgex(i,j,k,comp))
             end do
          end do
          ...
          do j=lo(2),hi(2)+1
             do i=lo(1),hi(1)
                sfluxy(i,j,k,comp) = &
                     vmac(i,j,k)*(rhoX0_edge+sedgey(i,j,k,comp))
             end do
          end do
       end do
       ...
       ! create z-fluxes
       do k=lo(3),hi(3)+1
          rhoX0_edge = HALF*(rhoX0_edge_old(k,comp-spec_comp+1)+rhoX0_edge_new(k,comp-spec_comp+1))
          do j=lo(2),hi(2)
             do i=lo(1),hi(1)
                sfluxz(i,j,k,comp) = (wmac(i,j,k)+w0(k))* &
                     (rhoX0_edge+sedgez(i,j,k,comp))
