...
compute $\rho_0^{(2)}$ from $\rho^n$ using advect\_base, which advances equation 
(\ref{rho0 equation}) through $\Delta t$ in time.  The $(2)$ in the superscript indicates 
that we have not called correct\_base yet, which computes  $\rho^{n+1}$ from $\rho_0^{(2)}$. 
We use $\rho^{(2)}$ rather than $\rho^{n+1}$ to construct $\rho_0^{n+\half,{\rm avg}}$ to be 
