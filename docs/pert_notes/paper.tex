...
We first compute $\rho_0^{(2)}$ using advect\_base (but not correct\_base).  We compute 
$\rho$ on edges by adding $\rho_0^{n+\half} = (\rho_0^n + \rho_0^{(2)}) / 2$ to $\rho'$ at 
edges. For the non-radial faces, we can directly add $\rho_0^{n+\half}$ since it is a 
cell-centered quantity.  For the radial faces, we interpolate to obtain $\rho_0^{n+\half}$ 
at radial faces.
...
is set to zero, and therefore we set is\_conservative = T to alter the discretization 
in make\_edge\_scal.
...
underbraced term in (\ref{rho0 equation}) as the forcing.  The $\eta_\rho$ term is not 
...
&=& -\Ub\cdot\nabla(\rho h) - \rho h\nabla\cdot\Ub -\nabla\cdot(\rho h \Ub) 
+ \frac{Dp_0}{Dt} + \nabla\cdot\kappa\nabla T + \rho H_{\rm nuc} 
+ \rho H_{\rm ext}.\label{rhoh equation}
\end{eqnarray}
