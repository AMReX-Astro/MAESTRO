...
2 / {\tt pred\_rhoX}  &  $(\rho X_k)_\mathrm{edge}$ \\[2mm]
3 / {\tt pred\_rho\_and\_X}  &  $\rho_\mathrm{edge} (X_k)_\mathrm{edge}$ \\
\hline
\end{tabular}
\end{table}
We note the labels {\tt pred\_rhoprime\_and\_X}, {\tt pred\_rhoX}, and
{\tt pred\_rho\_and\_X} are provided by the {\tt pred\_parameters}
module.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{{\tt species\_pred\_type} = {\tt pred\_rho\_and\_X}}
...
\noindent {\tt species\_pred\_type = pred\_rho\_and\_X}:
\begin{equation}
\frac{\partial\rho X_k}{\partial t} = -\nabla\cdot(\rho X_k\Ub) = 
