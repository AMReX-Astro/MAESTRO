...
\end{eqnarray}
Note that the 2D and 3D algorithms are different - in 3D the transverse terms use upwinding analogous to equations (\ref{transverse upwinding 1})-(\ref{transverse upwinding 6}), using UAD instead of UEDGE.  Finally, upwind analogous to equation (\ref{ESTATE Upwind}) to get $\text{sedge}_{\ib-\half\eb_x}$, but use UEDGE instead of UAD.
