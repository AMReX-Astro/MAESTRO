...
with the possible exception of the $\partial w_0/\partial r$ term, which
is enabled in plane-parallel with {\tt use\_new\_godunov}.
\MarginPar{Need to discuss role of $\partial w_0/\partial r$ term}
...
\MarginPar{I think the upwinding needs to be permanent.}
\begin{eqnarray}
...
{\bf if} {\tt use\_new\_godunov = F} {\bf then}
\begin{eqnarray}
\wt_{L,\ib-\half\eb_r}^{\trans} &=& \wt_{\ib-\eb_r}^n + \left(\half - \frac{\dt}{2h}w_{\ib-\eb_r}^n\right)\Delta_r \wt_{\ib-\eb_r}^n, \\
\wt_{R,\ib-\half\eb_r}^{\trans} &=& \wt_{\ib}^n - \left(\half + \frac{\dt}{2h}w_{\ib}^n\right)\Delta_r \wt_{\ib}^n,
\end{eqnarray}
{\bf else if} {\tt use\_new\_godunov = T} {\bf then}
\MarginPar{Here is the optional $\partial w_0/\partial r$ term.}
\begin{eqnarray}
...
\end{eqnarray}
{\bf end if}\\ \\
To obtain $w_{\ib}^n$ at cell centers, simple averaging of $w_0$ is used:
\begin{equation}
...
\end{equation}
We pick the final trans states using a Riemann solver:
\MarginPar{Should we really upwind based on $\wt$ instead of $w$?}
\begin{equation}
...
$w_0^{\mac}$ only contains normal velocities at each face.
Then we construct $\Ub$ by using
\begin{equation}
u_{\ib} = \ut_{\ib} + \frac{w_{0,\ib+\half\eb_x}^{\mac} + w_{0,\mac,\ib+\half\eb_x}}{2},
\end{equation}
\begin{equation}
v_{\ib} = \vt_{\ib} + \frac{w_{0,\ib+\half\eb_y}^{\mac} + w_{0,\mac,\ib+\half\eb_y}}{2},
\end{equation}
\begin{equation}
w_{\ib} = \wt_{\ib} + \frac{w_{0,\ib+\half\eb_z}^{\mac} + w_{0,\mac,\ib+\half\eb_z}}{2}.
\end{equation}
This is a spherical generalization of equation
(\ref{Total Radial Velocity}).  Then we predict the normal 
components of velocity to the normal faces using a 1D extrapolation.  
...
\end{eqnarray}
{\bf else if} {\tt use\_new\_godunov = T} {\bf then}
\MarginPar{The code currently excludes the $\partial w_0/\partial r$ term in spherical}
\begin{eqnarray}
\ut_{L,\ib-\half\eb_x}^{\trans} &=& \ut_{\ib-\eb_x}^n + \left[\half - \frac{\dt}{2h}\max(0,u_{\ib-\eb_x}^n)\right]\Delta_x\ut_{\ib-\eb_x}^n \\
\ut_{R,\ib-\half\eb_x}^{\trans} &=& \ut_{\ib}^n - \left[\half + \frac{\dt}{2h}\min(0,u_{\ib}^n)\right]\Delta_x\ut_{\ib}^n.
\end{eqnarray}
...
obtain $\ut^{\trans}$.
\MarginPar{Again, should we really upwind based on $\ut/\vt/\wt$ instead of $u/v/w$?}
...
f_w \\
\end{array}\right)
= - \frac{1}{\rho}\nabla\pi + \underbrace{\frac{1}{\rho_0}\frac{\partial\pi_0}{\partial r}\eb_r} - \frac{(\rho-\rho_0)}{\rho}g\eb_r.
\end{equation}
The underbraced term is only included if {\tt use\_new\_godunov = T}.
Note that later this term is always included in the cell update.
...
\item Predict $\wt$ to r-faces using a full-dimensional extrapolation.
  Note the use of the $\partial w_0/\partial r$ term.
\end{enumerate}
...
{\bf end if}\\ \\
Solve a Riemann problem:
\MarginPar{Again, should we really upwind based on $\wt$ instead of $w$?}
\begin{equation}
...
\subsection{3D Spherical Case}
As before in the $\Ubt^{\trans}$, algorithm, we first map 
$w_0$ onto $w_0^{\mac}$ using {\tt put\_w0\_on\_edges.}\\ \\
We also map $\partial w_0/\partial r$ onto a Cartesian 
array using {\tt put\_1d\_array\_on\_cart\_3d\_sphr}.
This is a spherical generalization of
...
\MarginPar{Similar to the $\Ubt^{\trans}$ spherical algorithm, 
we are excluding the $\partial w_0/\partial r$ 
term in the 1D extrapolation.  We are also upwinding based 
on $\ut/\vt/\wt$ instead of $u/v/w$.}
...
\end{cases}
\end{equation}
We predict $s$ to r-faces using a full-dimensional extrapolation:
\begin{eqnarray}
...
\subsection{3D Cartesian Case}
Same as 2D with the older 3D corner coupling algorithm for the transverse terms.

\newpage

\subsection{3D Spherical Case}
Same as 3D Cartesian case except that we never include $\partial w_0/\partial r$ in any of the 1D extrapolations, and always include it in the full dimensional extrapolation.
