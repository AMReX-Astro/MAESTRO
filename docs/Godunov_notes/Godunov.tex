...
\item {\tt VARDEN} does not use the concept of $\Ub^{\trans}$ in the
  computation of $\Ub^{\mac,*}$, i.e., the function {\tt velpred}
...
direction of the extrapolation.  We also omit the underbraced source terms.
...
\begin{eqnarray}
\ut_{L,\ib-\half\eb_x}^{1D} &=& \ut_{\ib-\eb_x}^n + \left[\half - \frac{\dt}{2h}\max(0,u_{\ib-\eb_x}^n)\right]\Delta_x \ut_{\ib-\eb_x}^n, \label{u_L^1D}\\
\ut_{R,\ib-\half\eb_x}^{1D} &=& \ut_{\ib}^n - \left[\half + \frac{\dt}{2h}\min(0,u_{\ib}^n)\right]\Delta_x \ut_{\ib}^n.\label{u_R^1D}
\end{eqnarray}
We pick the final trans states using a Riemann solver:
\begin{equation}
\ut^{\trans}_{\ib-\half\eb_x} =
\begin{cases}
0, & \left(\ut_{L,\ib-\half\eb_x}^{1D} \le 0 ~~ {\rm AND} ~~ \ut_{R,\ib-\half\eb_x}^{1D} \ge 0\right) ~~ {\rm OR} ~~ \left|\ut_{L,\ib-\half\eb_x}^{1D} + \ut_{R,\ib-\half\eb_x}^{1D}\right| < \epsilon, \\
\ut_{L,\ib-\half\eb_x}^{1D}, & \ut_{L,\ib-\half\eb_x}^{1D} + \ut_{R,\ib-\half\eb_x}^{1D} > 0, \\
\ut_{R,\ib-\half\eb_x}^{1D}, & \ut_{L,\ib-\half\eb_x}^{1D} + \ut_{R,\ib-\half\eb_x}^{1D} < 0, \\
\end{cases}
\end{equation}
We predict $\wt$ to r-faces using a 1D extrapolation:
\begin{eqnarray}
\wt_{L,\ib-\half\eb_r}^{1D} &=& \wt_{\ib-\eb_r}^n + \left[\half - \frac{\dt}{2h}\max(0,w_{\ib-\eb_r}^n)\right]\Delta_r \wt_{\ib-\eb_r}^n,\\
\wt_{R,\ib-\half\eb_r}^{1D} &=& \wt_{\ib}^n - \left[\half + \frac{\dt}{2h}\min(0,w_{\ib}^n)\right]\Delta_r \wt_{\ib}^n.
\end{eqnarray}
We pick the final $\trans$ states using a Riemann solver, noting
that we upwind based on the full velocity, which is computed by adding
$w_0$ to the left and right $1D$ velocities.
\begin{equation}
\wt^{\trans}_{\ib-\half\eb_r} =
\begin{cases}
0, & \left(w_L^{1D} \le 0 ~~ {\rm AND} ~~ w_R^{1D} \ge 0\right) ~~ {\rm OR} ~~ \left|w_L^{1D} + w_R^{1D}\right| < \epsilon, \\
\wt_{L,\ib-\half\eb_r}^{1D}, & w_L^{1D} + w_R^{1D} > 0, \\
\wt_{R,\ib-\half\eb_r}^{1D}, & w_L^{1D} + w_R^{1D} < 0, \\
\end{cases}
\end{equation}
...
1D extrapolation:
\begin{eqnarray}
\vt_{L,\ib-\half\eb_y}^{1D} &=& \vt_{\ib-\eb_y}^n + \left[\half - \frac{\dt}{2h}\max(0,v_{\ib-\eb_y}^n)\right]\Delta_y \vt_{\ib-\eb_y}^n, \\
\vt_{R,\ib-\half\eb_y}^{1D} &=& \vt_{\ib}^n - \left[\half + \frac{\dt}{2h}\min(0,v_{\ib}^n)\right]\Delta_y \vt_{\ib}^n,
\end{eqnarray}
\begin{equation}
\vt^{\trans}_{\ib-\half\eb_y} =
\begin{cases}
0, & \left(v_{L,\ib-\half\eb_y}^{1D} \le 0 ~~ {\rm AND} ~~ v_{R,\ib-\half\eb_y}^{1D} \ge 0\right) ~~ {\rm OR} ~~ \left|v_{L,\ib-\half\eb_y}^{1D} + v_{R,\ib-\half\eb_y}^{1D}\right| < \epsilon, \\
\vt_{L,\ib-\half\eb_y}^{1D}, & v_{L,\ib-\half\eb_y}^{1D} + v_{R,\ib-\half\eb_y}^{1D} > 0, \\
\vt_{R,\ib-\half\eb_y}^{1D}, & v_{L,\ib-\half\eb_y}^{1D} + v_{R,\ib-\half\eb_y}^{1D} < 0. \\
\end{cases}
\end{equation}
...
sections.  Just make sure the advection velocities, as well as 
the upwinding, is done with the full velocity, not the 
...
\end{equation}
where the underbraced terms are the forcing.  Note that the 
$\partial w_0/\partial r$ term is treated like a source term, but
it is not actually part of $f_{\Ubt}$.
...
\item Predict $\wt$ to r-faces using a full-dimensional extrapolation.
  Note the inclusion of the $\partial w_0/\partial r$ term.
\end{enumerate}
...
where $\ut_{L/R,\ib-\half\eb_x}^{1D}$ comes from equations 
(\ref{u_L^1D}) and (\ref{u_R^1D}):
\begin{eqnarray}
\ut_{L,\ib-\half\eb_x}^{\mac,*} &=& \ut_{L,\ib-\half\eb_x}^{1D} - \frac{\dt}{4h}\left(w_{\ib-\eb_x+\half\eb_r}^{\trans}+w_{\ib-\eb_x-\half\eb_r}^{\trans}\right)\left(\ut_{\ib-\eb_x+\half\eb_r}^{1D} - \ut_{\ib-\eb_x-\half\eb_r}^{1D}\right) + \frac{\dt}{2}f_{\ut,\ib-\eb_x}, \nonumber \\
&& \\
\ut_{R,\ib-\half\eb_x}^{\mac,*} &=& \ut_{R,\ib-\half\eb_x}^{1D} - \frac{\dt}{4h}\left(w_{\ib+\half\eb_r}^{\trans}+w_{\ib-\half\eb_r}^{\trans}\right)\left(\ut_{\ib+\half\eb_r}^{1D} - \ut_{\ib-\half\eb_r}^{1D}\right) + \frac{\dt}{2}f_{\ut,\ib}.
\end{eqnarray}
...
Note the inclusion of the $\partial w_0/\partial r$ term:
\begin{eqnarray}
\wt_{L,\ib-\half\eb_r}^{\mac,*} &=& \wt_{L,\ib-\half\eb_r}^{1D} - \frac{\dt}{4h}\left(u_{\ib+\half\eb_x-\eb_r}^{\trans}+u_{\ib-\half\eb_x-\eb_r}^{\trans}\right)\left(\wt_{\ib+\half\eb_x-\eb_r}^{1D} - \wt_{\ib-\half\eb_x-\eb_r}^{1D}\right) \nonumber \\
...
\wt_{R,\ib-\half\eb_r}^{\mac,*} &=& \wt_{R,\ib-\half\eb_r} - \frac{\dt}{4h}\left(u_{\ib+\half\eb_x}^{\trans}+u_{\ib-\half\eb_x}^{\trans}\right)\left(\wt_{\ib+\half\eb_x}^{1D} - \wt_{\ib-\half\eb_x}^{1D}\right) \nonumber \\
...
Also, we make sure to upwind based on the full velocity, not the
perturbational velocity.
...
\end{itemize}
We are using {\tt enthalpy\_pred\_type} = 1 for now.  The force terms are:
\begin{eqnarray}
...
&=& -\frac{1}{\rho}\nabla\pi + \frac{1}{\rho_0}\frac{\partial\pi_0}{\partial r}\eb_r - \frac{(\rho-\rho_0)}{\rho}g\eb_r.
\end{eqnarray}
Note that the $\partial w_0/\partial r$ term is treated 
like a source term, but it is not actually part of $f_{\Ubt}$.
...
\frac{\dt}{2h}\left[\left(w^{\mac}s^{1D}\right)_{\ib-\eb_x+\half\eb_r} - \left(w^{\mac}s^{1D}\right)_{\ib-\eb_x-\half\eb_r}\right]\nonumber\\
&&- \frac{\dt}{2h}s_{\ib-\eb_x}^{n}\left(u_{\ib-\half\eb_x}^{\mac}-u_{\ib-\frac{3}{2}\eb_x}^{\mac}\right)\\
s_{R,\ib-\half\eb_x}^{\edge} &=& s_{R,\ib-\half\eb_x}^{\edge} -
\frac{\dt}{2h}\left[\left(w^{\mac}s^{1D}\right)_{\ib+\half\eb_r} - \left(w^{\mac}s^{1D}\right)_{\ib-\half\eb_r}\right]\nonumber\\
&&- \frac{\dt}{2h}s_{\ib}^{n}\left(u_{\ib+\half\eb_x}^{\mac}-u_{\ib-\half\eb_x}^{\mac}\right)\\
\end{eqnarray}
{\bf else}
\begin{eqnarray}
s_{L,\ib-\half\eb_x}^{\edge} &=& s_{L,\ib-\half\eb_x}^{\edge} -
\frac{\dt}{4h}\left(w^{\mac}_{\ib-\eb_x+\half\eb_r} + w^{\mac}_{\ib-\eb_x-\half\eb_r}\right)\left(s^{1D}_{\ib-\eb_x+\half\eb_r} - s^{1D}_{\ib-\eb_x-\half\eb_r}\right)\\
s_{R,\ib-\half\eb_x}^{\edge} &=& s_{R,\ib-\half\eb_x}^{\edge} -
\frac{\dt}{4h}\left(w^{\mac}_{\ib+\half\eb_r} + w^{\mac}_{\ib-\half\eb_r}\right)\left(s^{1D}_{\ib+\half\eb_r} - s^{1D}_{\ib-\half\eb_r}\right)
\end{eqnarray}
{\bf end if}\\ \\
...
\frac{\dt}{2h}\left[\left(u^{\mac}s^{1D}\right)_{\ib-\eb_r+\half\eb_x} - \left(u^{\mac}s^{1D}\right)_{\ib-\eb_r-\half\eb_x}\right]\nonumber\\
&&- \frac{\dt}{2h}s_{\ib-\eb_r}^{n}\left(w_{\ib-\half\eb_r}^{\mac}-w_{\ib-\frac{3}{2}\eb_r}^{\mac}\right)\\
s_{R,\ib-\half\eb_r}^{\edge} &=& s_{R,\ib-\half\eb_r}^{\edge} -
\frac{\dt}{2h}\left[\left(u^{\mac}s^{1D}\right)_{\ib+\half\eb_x} - \left(u^{\mac}s^{1D}\right)_{\ib-\half\eb_x}\right]\nonumber\\
&&- \frac{\dt}{2h}s_{\ib}^{n}\left(w_{\ib+\half\eb_r}^{\mac}-w_{\ib-\half\eb_r}^{\mac}\right)\\
\end{eqnarray}
{\bf else}
\begin{eqnarray}
s_{L,\ib-\half\eb_r}^{\edge} &=& s_{L,\ib-\half\eb_r}^{\edge} -
\frac{\dt}{4h}\left(u^{\mac}_{\ib-\eb_r+\half\eb_x} + u^{\mac}_{\ib-\eb_r-\half\eb_x}\right)\left(s^{1D}_{\ib-\eb_r+\half\eb_x} - s^{1D}_{\ib-\eb_r-\half\eb_x}\right)\\
s_{R,\ib-\half\eb_r}^{\edge} &=& s_{R,\ib-\half\eb_r}^{\edge} -
\frac{\dt}{4h}\left(u^{\mac}_{\ib+\half\eb_x} + u^{\mac}_{\ib-\half\eb_x}\right)\left(s^{1D}_{\ib+\half\eb_x} - s^{1D}_{\ib-\half\eb_x}\right)
\end{eqnarray}
{\bf end if}\\ \\
...
\item Predict $s$ to r-faces using a full-dimensional extrapolation.
\end{enumerate}
We are not using corner coupling yet.

\newpage

\subsection{3D Spherical Case}
Same as 3D Cartesian case.
