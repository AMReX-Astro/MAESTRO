...
\end{eqnarray}
Be careful with $\text{sedge}_L^x$ because the conditions correspond to the starting point of the extrapolation, which is different from $\text{sedge}_R^x$.\\ \\
{\bf If} NOT ICONSERVE {\bf and} $\text{VEDGE}_{\ib+\half\eb_y}\cdot\text{VEDGE}_{\ib-\half\eb_y} < 0$ {\bf and} $\text{VBAR}_{\ib} \ge 0$ {\bf then:}
\begin{eqnarray}
\text{sedge}_R^x = s_{\ib} &-& \left(\half + \frac{\dt}{h_x}\text{UEDGE}_{\ib-\half\eb_x}\right)\Delta^x s_{\ib} + \frac{\dt}{2}\text{TFORCES}_{\ib} \nonumber\\
&& - \frac{\dt}{2}\left[\frac{\text{VBAR}_{\ib}(s_{\ib}-s_{\ib-\eb_y})}{h_y}\right]
\end{eqnarray}
Be careful with $\text{sedge}_L^x$ because the conditions correspond to the starting point of the extrapolation, which is different from $\text{sedge}_R^x$.\\ \\
{\bf If} NOT ICONSERVE {\bf and} $\text{VEDGE}_{\ib+\half\eb_y}\cdot\text{VEDGE}_{\ib-\half\eb_y} \ge 0$ {\bf then:}
\begin{eqnarray}
\text{sedge}_R^x = s_{\ib} &-& \left(\half + \frac{\dt}{h_x}\text{UEDGE}_{\ib-\half\eb_x}\right)\Delta^x s_{\ib} + \frac{\dt}{2}\text{TFORCES}_{\ib} \nonumber\\
...
\end{eqnarray}
Be careful with $\text{sedge}_L^x$ because the conditions correspond to the starting point of the extrapolation, which is different from $\text{sedge}_R^x$.\\ \\
Finally, upwind analogous to equation (\ref{ESTATE Upwind}) to get $\text{sedge}_{\ib-\half\eb_x}$, but use UEDGE instead of UAD,
\begin{equation}
\text{sedge}_{\ib-\half\eb_x}^x =
\begin{cases}
\text{sedge}_L^x, & \text{UEDGE}_{\ib-\half\eb_x} > 0, \\
\text{sedge}_R^x, & \text{else}. \\
\end{cases}
\end{equation}
