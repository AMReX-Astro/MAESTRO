...
The $\partial w_0/\partial r$ term only enters in a full-dimensional 
extrapolation of $\wt$ to r-faces..
\subsection{3D Spherical Case}
Same as 3D case, except the $\partial w_0/\partial r$ term enters 
in the full dimensional extrapolation for each direction.

%-----------------------------------------------------------------------------
% edge states
%-----------------------------------------------------------------------------
\section{Computing $\rho^{'\edge}, X_k^{\edge}$, and $(\rho h)^{'\edge}/h^{\edge}/T^{\edge}/h'^{\edge}/T'^{\edge}$ in {\tt MAESTRO}}
In {\tt scalar\_advance}, we call {\tt make\_edge\_scal} to compute
$\rho^{'\edge}, X_k^{\edge}$, and 
$(\rho h)^{'\edge}/h^{\edge}/T^{\edge}/h'^{\edge}/T'^{\edge}$ at each edge.
The procedure is the same for each scalar, so we shall simply denote 
...
If {\tt enthalpy\_pred\_type} = 6, we convert 
$T'^{\edge} \rightarrow h^{\edge}$.
