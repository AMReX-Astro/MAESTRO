...
$w_0$ to the left and right $\trans$ velocities.
\begin{equation}
...
\vt_{L,\ib-\half\eb_y}^{\trans}, & v_{L,\ib-\half\eb_y}^{\trans} + v_{R,\ib-\half\eb_y}^{\trans} > 0, \\
\vt_{R,\ib-\half\eb_y}^{\trans}, & v_{L,\ib-\half\eb_y}^{\trans} + v_{R,\ib-\half\eb_y}^{\trans} < 0. \\
\end{cases}
\end{equation}
...
\ut_{R,\ib-\half\eb_x}^{\trans} &=& \ut_{\ib}^n - \left[\half + \frac{\dt}{2h}\min(0,u_{\ib}^n)\right]\Delta_x\ut_{\ib}^n.
\end{eqnarray}
Then we use the same Riemann solver as the Cartesian case to obtain $\ut^{\trans}$,
making sure we upwind based on the full velocity.
