...
\end{cases}
\end{equation}
For the radial faces, the 1D extrapolation of $\wt$ is as follows:\\ \\
{\bf if} {\tt use\_new\_godunov = F} {\bf then}
\begin{eqnarray}
...
\end{cases}
\end{equation}
Finally, we call {\tt addw0} to compute $w^{\trans}$ from $\wt^{\trans}$.
...
\end{eqnarray}
{\bf end if}\\ \\
Then, we use the same Riemann solver as the Cartesian 
case to obtain $\ut^{\trans}$.
\MarginPar{Again, should we really upwind based on $\ut$ instead of $u$?}
Finally, we call {\tt addw0} to compute
