...
for the time-centered 1D edge states, $s_{L/R,\ib-\half}$.  Once these states 
...
\item The paper uses the inequality $\delta\mathcal{I}_{\rm ext} \ge s\delta a$.  Phil multiplies the $\delta\mathcal{I}_{\rm ext}$ by $s$, which is consistent with the preprint. {\bf Changed in code to match Phil}
