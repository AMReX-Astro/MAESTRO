...
\begin{itemize}
\item Before equation (25), the paper tests on the inequality, $|\alpha_{j,\pm}| \ge 2|\alpha_{j,\mp}|$.  Phil tests on a slightly different inequality, $|\alpha_{j,\pm}| > 2|\alpha_{j,\mp}|$.
\item In equation (25), the paper defines $\delta a = a_{j\pm 1}^n - a_j^n$.  Phil uses $\delta a = a_{j\mp 1}^n - a_j^n$.
\item During (25), Phil redefines $s = \text{sign}(\alpha_{j,\mp})$ instead of leaving it as $s = \text{sign}(a_{j+1}^n - a_{j-1}^n)$.
\item The paper uses the inequality $\delta\mathcal{I}_{\rm ext} \ge s\delta a$.  Phil multiplies the $\delta\mathcal{I}_{\rm ext}$ by $s$.
