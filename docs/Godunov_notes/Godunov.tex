...
and $\Ubt = (u,\wt)$ since $u = \ut$.  We will use the shorthand $\ib = (x,r)$.\\ \\
For the x-faces, the 1D extrapolation of $u$ is as follows:\\ \\
...
\begin{eqnarray}
u_{L,\ib-\half\eb_x}^{\trans} &=& u_{\ib-\eb_x}^n + \left[\half - \frac{\dt}{2h}\max(0,u_{\ib-\eb_x}^n)\right]\Delta_x u_{\ib-\eb_x}^n, \\
u_{R,\ib-\half\eb_x}^{\trans} &=& u_{\ib}^n - \left[\half + \frac{\dt}{2h}\min(0,u_{\ib}^n)\right]\Delta_x u_{\ib}^n.
\end{eqnarray}
{\bf end if}\\ \\
...
\end{cases}
\end{equation}
For the r-faces, the 1D extrapolation of $\wt$ is as follows:\\ \\
{\bf if} {\tt use\_new\_godunov = F} {\bf then}
\begin{eqnarray}
...
\end{eqnarray}
{\bf end if}\\ \\
To obtain $w_{\ib}^n$ at cell centers, simple averaging is used:\label{Total Radial Velocity}
\begin{equation}
...
and $\Ubt = (u,v,\wt)$ since $u = \ut$ and $v = \vt$.
We will use the shorthand $\ib = (x,y,r)$.\\ \\
We use the exact same procedure in 2D and 3D to compute $u^{\trans}$ and 
$\wt^{\trans}$. The procedure for computing $v^{\trans}$ is analogous to
...
{\bf else if} {\tt use\_new\_godunov = T} {\bf then}
\begin{eqnarray}
v_{L,\ib-\half\eb_y}^{\trans} &=& v_{\ib-\eb_y}^n + \left[\half - \frac{\dt}{2h}\max(0,v_{\ib-\eb_y}^n)\right]\Delta_y v_{\ib-\eb_y}^n, \\
v_{R,\ib-\half\eb_y}^{\trans} &=& v_{\ib}^n - \left[\half + \frac{\dt}{2h}\min(0,v_{\ib}^n)\right]\Delta_y v_{\ib}^n,
\end{eqnarray}
{\bf end if}
\begin{equation}
...
extrapolation, which is analogous for all three directions.
...
{\bf else if} {\tt use\_new\_godunov = T} {\bf then}
\begin{eqnarray}
u_{L,\ib-\half\eb_r}^{1D} &=& u_{\ib-\eb_r}^n + \left[\half - \frac{\dt}{2h}\max(0,w_{\ib-\eb_r}^n)\right]\Delta_r u_{\ib-\eb_r}^n, \\
u_{R,\ib-\half\eb_r}^{1D} &=& u_{\ib} - \left[\half + \frac{\dt}{2h}\min(0,w_{\ib}^n)\right]\Delta_r u_{\ib}^n.
\end{eqnarray}
{\bf end if}\\ \\
...
{\bf else if} {\tt use\_new\_godunov = T} {\bf then}
\begin{eqnarray}
u_{L,\ib-\half\eb_x}^{\mac,*} &=& u_{\ib-\eb_x}^n + \left(\half - \frac{\dt}{2h}\max(0,u_{\ib-\eb_x})\right)\Delta_x u_{\ib-\eb_x}^n \nonumber \\
&& -\frac{\dt}{4h}\left[w_{\ib-\eb_x+\half\eb_r}^{\trans}+w_{\ib-\eb_x-\half\eb_r}^{\trans}\right]\left(u_{\ib-\eb_x+\half\eb_r}^{1D} - u_{\ib-\eb_x-\half\eb_r}^{1D}\right) + \frac{\dt}{2}f_{u,\ib-\eb_x}, \\
u_{R,\ib-\half\eb_x}^{\mac,*} &=& u_{\ib}^n - \left[\half + \frac{\dt}{2h}\min(0,u_{\ib})\right]\Delta_x u_{\ib}^n \nonumber \\
&& -\frac{\dt}{4h}\left(w_{\ib+\half\eb_r}^{\trans}+w_{\ib-\half\eb_r}^{\trans}\right)\left(u_{\ib+\half\eb_r}^{1D} - u_{\ib-\half\eb_r}^{1D}\right) + \frac{\dt}{2}f_{u,\ib}.
\end{eqnarray}
{\bf end if}\\ \\
