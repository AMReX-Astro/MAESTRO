...
\begin{eqnarray}
\ut_{L,\ib-\half\eb_x} &=& \ut_{\ib-\eb_x}^n + \left[\half - \frac{\dt}{2h}\max(0,u_{\ib-\eb_x}^n)\right]\Delta_x \ut_{\ib-\eb_x}^n, \label{u_L^1D}\\
\ut_{R,\ib-\half\eb_x} &=& \ut_{\ib}^n - \left[\half + \frac{\dt}{2h}\min(0,u_{\ib}^n)\right]\Delta_x \ut_{\ib}^n.\label{u_R^1D}
\end{eqnarray}
...
where $\ut_{L/R,\ib-\half\eb_x}$ comes from equations (\ref{u_L^1D})
and (\ref{u_R^1D}):
\begin{eqnarray}
...
\MarginPar{Need to write out the full equations - this will also show which
terms are treated conservatively.  Also make sure it's clear that the $\partial w_0/\partial r$ term is not part of the velocity forcing.}
\begin{eqnarray}
