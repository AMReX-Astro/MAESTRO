...
\end{cases}
\end{equation}
For the radial faces, we must include the radial derivative terms proportional to $w_0$ and $(\Ubt\cdot\eb_r) = \wt$.  The term proportional to $w_0$ is included by using $w$ rather than $\wt$ as the advection speed.  The term proportional to $\wt$ is an additional term.  For the radial faces, the 1D extrapolation of $\wt$ is:
\begin{eqnarray}
\wt_L &=& \wt_{\ib-\eb_r}^n + \left(\half - \frac{\dt}2{h}w_{\ib-\eb_r}^n\right)\Delta_r \wt_{\ib-\eb_r}^n + \frac{\dt}{2}\wt_{\ib-\eb_r}^n\left(\frac{\partial w_0}{\partial r}\right)_{\ib-\eb_r},\label{Radial utrans_L} \\
...
\subsection{2D Cartesian Case}
We begin by computing a time-centered 1D extrapolation of $u$ to transverse faces:
\begin{eqnarray}
...
\end{cases}
\end{equation}
Then, we do a full-dimensional extrapolation of $u$ to normal faces:
\begin{eqnarray}
...
\end{cases}
\end{equation}
Then, we do a full-dimensional extrapolation of $\wt$ to normal faces, incorporating the extra term proportional to $w_0$ by using $w$ instead of $\wt$ as the advection velocity and the extra term proportional to $\partial w_0/\partial r$ as a separate term:
\begin{eqnarray}
...
u_{R,\ib-\half\eb_x}^{1D} &=& u_{\ib} - \left(\half + \frac{\dt}{2h}u_{\ib}^n\right)\Delta_r u_{\ib}^n.
\end{eqnarray}
Then we upwind based on $u^{\trans}$:
\begin{equation}
u_{\ib-\half\eb_x}^{1D} =
...
\end{cases}
\end{equation}
The extrapolation of $u$ to radial faces is:
\begin{eqnarray}
...
\wt_{R,\ib-\half\eb_x}^{1D} &=& \wt_{\ib} - \left(\half + \frac{\dt}{2h}u_{\ib}^n\right)\Delta_r \wt_{\ib}^n.
\end{eqnarray}
Then we upwind based on $u^{\trans}$:
\begin{equation}
\wt_{\ib-\half\eb_x}^{1D} =
...
\end{cases}
\end{equation}
The extrapolation of $\wt$ to radial faces is:
\begin{eqnarray}
...
\end{cases}
\end{equation}
The full-dimensional extrapolation of $u$ to radial faces is:
\begin{eqnarray}
u_{L,\ib-\half\eb_r}^{\edge} &=& u_{\ib-\eb_r}^n + \left(\half - \frac{\dt}{2h}w_{\ib-\half\eb_r}^{\mac}\right)\Delta_r u_{\ib-\eb_r}^n + \frac{\dt}{2}f_{u,\ib-\eb_x} \nonumber \\
...
u_{R,\ib-\half\eb_r}^{\edge} &=& u_{\ib}^n + \left(\half - \frac{\dt}{2h}w_{\ib-\half\eb_r}^{\mac}\right)\Delta_r u_{\ib}^n + \frac{\dt}{2}f_{u,\ib-\eb_x} \nonumber \\
...
&& - \frac{\dt}{4h}\left(\wt_{\ib-\eb_x+\half\eb_r}^{\trans}+\wt_{\ib-\eb_x-\half\eb_r}^{\trans}\right)\left(w_{0,\ib+\half\eb_x}-w_{0,\ib-\half\eb_x}\right), \\
...
\end{cases}
\end{equation}
The full-dimensional extrapolation of $\wt$ to radial faces is:
