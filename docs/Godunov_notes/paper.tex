...
\end{cases}
\end{equation}
where $w_L = \wt_L + w_{0,r-\half}$ and $w_R = \wt_R + w_{0,r-\half}$.  As mentioned before, we now call {\tt addw0} to compute $\wt^{\trans}$ from $w^{\trans}.$
...
\begin{eqnarray}
u_L^{1D} &=& u_{\ib-\eb_y}^n + \left(\half - \frac{\dt}{2h}w_{\ib-\eb_y}^n\right)\Delta_y u_{\ib-\eb_y}^n, \\
u_R^{1D} &=& u_{\ib} - \left(\half + \frac{\dt}{2h}w_{\ib}^n\right)\Delta_y u_{\ib}^n.
\end{eqnarray}
Then we solve a Riemann problem:
\begin{equation}
u_{\ib-\half\eb_y}^{1D} =
\begin{cases}
\half(u_L^{1D} + u_R^{1D}), & \left|w^{\trans}_{\ib-\half\eb_y}\right| < \epsilon \\
u_L^{1D}, & w^{\trans}_{\ib-\half\eb_y} > 0, \\
u_R^{1D}, & w^{\trans}_{\ib-\half\eb_y} < 0. \\
\end{cases}
\end{equation}
...
&& - \frac{\dt}{4h}\left(w_{\ib-\eb_x+\half\eb_y}^{\trans}+w_{\ib-\eb_x-\half\eb_y}^{\trans}\right)\left(u_{\ib-\eb_x+\half\eb_y}^{1D} - u_{\ib-\eb_x-\half\eb_y}^{1D}\right), \\
u_R &=& u_{\ib}^n + \left(\half - \frac{\dt}{2h}u_{\ib}\right)\Delta_x u_{\ib}^n + \frac{\dt}{2}f_{\ib} \nonumber \\
&& - \frac{\dt}{4h}\left(w_{\ib+\half\eb_y}^{\trans}+w_{\ib-\half\eb_y}^{\trans}\right)\left(u_{\ib+\half\eb_y}^{1D} - u_{\ib-\half\eb_y}^{1D}\right).
\end{eqnarray}
...
\begin{eqnarray}
\wt_L &=& \wt_{\ib-\eb_y}^n + \left(\half - \frac{\dt}{2h}w_{\ib-\eb_y}\right)\Delta_y \wt_{\ib-\eb_y}^n + \frac{\dt}{2}f_{\ib-\eb_y} \nonumber \\
&& - \frac{\dt}{4h}\left(u_{\ib+\half\eb_x-\eb_y}^{\trans}+u_{\ib-\half\eb_x-\eb_y}^{\trans}\right)\left(\wt_{\ib+\half\eb_x-\eb_y}^{1D} - \wt_{\ib-\half\eb_x-\eb_y}^{1D}\right) \nonumber \\
&& - \frac{\dt}{2h}\underbrace{\wt_{\ib-\eb_y}}_{?}(w_{0,j-\half} - w_{0,j-\frac{3}{2}}), \\
\wt_R &=& \wt_{\ib}^n + \left(\half - \frac{\dt}{2h}w_{\ib}\right)\Delta_y \wt_{\ib}^n + \frac{\dt}{2}f_{\ib} \nonumber \\
...
\end{cases}
\end{equation}
Finally, we call {\tt addw0} to compute $\Ub^{\mac}$ from $\Ubt^{\trans}$.
\subsection{3D Cartesian Case}
\subsection{3D Spherical Case}
