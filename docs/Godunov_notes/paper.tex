...
\begin{eqnarray}
\Ub_{L,\ib-\half\eb_x}^{1D} &=& \Ub_{\ib-\eb_x} + \left(\half - \frac{\dt}{2h}u_{\ib-\eb_x}\right){\rm max}(0,\Delta_x\Ub_{\ib-\eb_x}), \label{varden U_L^1D} \\
...
\subsection{3D Cartesian Case}
This is more complicated than the 2D case because we include corner coupling.  We compute $\Ub_{\ib-\half\eb_x}, \Ub_{\ib-\half\eb_y}$, and $\Ub_{\ib-\half\eb_z}$ in an analogous manner as equations (\ref{varden U_L^1D})-(\ref{Transverse Velocity Riemann Problem}).  Then, we compute an intermediate state, $u_{\ib-\half\eb_y}^{y|z}$, which is described as ``state $u_{\ib-\half\eb_y}^{1D}$ that has been updated to account for the transverse fluxes in the z direction'', using:
\begin{eqnarray}
...
\begin{eqnarray}
u_{L,\ib-\half\eb_x}^{\mac,*} &=& u_{L,\ib-\half\eb_x}^{1D} + \frac{\dt}{2}f_{u,\ib-\eb_x} \nonumber \\
&& - \frac{\dt}{4h}\left(v_{\ib-\eb_x+\half\eb_y}^{1D}+v_{\ib-\eb_x-\half\eb_y}^{1D}\right)\left(u_{\ib-\eb_x+\half\eb_y}^{y|z}-u_{\ib-\eb_x-\half\eb_y}^{y|z}\right) \nonumber \\
&& - \frac{\dt}{4h}\left(w_{\ib-\eb_x+\half\eb_z}^{1D}+v_{\ib-\eb_x-\half\eb_z}^{1D}\right)\left(u_{\ib-\eb_x+\half\eb_z}^{z|y}-u_{\ib-\eb_x-\half\eb_z}^{z|y}\right), \\
u_{R,\ib-\half\eb_x}^{\mac,*} &=& u_{R,\ib-\half\eb_x}^{1D} + \frac{\dt}{2}f_{u,\ib} \nonumber \\
&& - \frac{\dt}{4h}\left(v_{\ib+\half\eb_y}^{1D}+v_{\ib-\half\eb_y}^{1D}\right)\left(u_{\ib+\half\eb_y}^{y|z}-u_{\ib-\half\eb_y}^{y|z}\right) \nonumber \\
&& - \frac{\dt}{4h}\left(w_{\ib+\half\eb_z}^{1D}+v_{\ib-\half\eb_z}^{1D}\right)\left(u_{\ib+\half\eb_z}^{z|y}-u_{\ib-\half\eb_z}^{z|y}\right).
\end{eqnarray}
...
\begin{eqnarray}
\rho_{L,\ib-\half\eb_x}^{1D} &=& s_{\ib-\eb_x}^n + \left(\half - \frac{\dt}{2h}u_{\ib-\half\eb_x}^{\mac}\right)\Delta_x \rho_{\ib-\eb_x}^n, \label{varden rho_L^1D}\\
\rho_{R,\ib-\half\eb_x}^{1D} &=& s_{\ib} - \left(\half + \frac{\dt}{2h}u_{\ib-\half\eb_x}^{\mac}\right)\Delta_x \rho_{\ib}^n. \label{varden rho_R^1D}
\end{eqnarray}
Then we upwind based on $u^{\mac}$:
\begin{equation}
...
\end{cases}
\end{equation}
The procedure to obtain $\rho_{\ib-\half\eb_y}^{1D}$ is completely analogous.  Now we do a full-dimensional extrapolation of $\rho$ to each face.  The extrapolation of $\rho$ to x-faces is:
\begin{eqnarray}
...
\end{cases}
\end{equation}
The procedure to obtain $\rho_{\ib-\half\eb_y}^{\edge}$ is completely analogous.
\subsection{3D Cartesian Case}
This is more complicated than the 2D case because we include corner coupling.  We compute $\rho_{\ib-\half\eb_x}^{1D}$, $\rho_{\ib-\half\eb_y}^{1D}$, and $\rho_{\ib-\half\eb_z}^{1D}$ in a completely analogous manner to equations (\ref{varden rho_L^1D}) and (\ref{varden rho_R^1D}).
\end{document}
