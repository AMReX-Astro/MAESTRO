...
\begin{eqnarray}
u_L^{\trans} &=& u_{\ib-\eb_x}^n + \left(\half - \frac{\dt}{2h}u_{\ib-\eb_x}^n\right)\Delta_x u_{\ib-\eb_x}^n, \\
u_R^{\trans} &=& u_{\ib}^n - \left(\half + \frac{\dt}{2h}u_{\ib}^n\right)\Delta_x u_{\ib}^n.
\end{eqnarray}
We pick the final edge states using a Riemann solver:
\begin{equation}
u^{\trans}_{\ib-\half\eb_x} =
\begin{cases}
0, & \left(u_L^{\trans} \le 0 ~ {\rm AND} ~ u_R^{\trans} \ge 0\right) ~ {\rm OR} ~ \left|u_L^{\trans} + u_R^{\trans}\right| < \epsilon, \\
u_L^{\trans}, & u_L^{\trans} + u_R^{\trans} > 0, \\
u_R^{\trans}, & u_L^{\trans} + u_R^{\trans} < 0, \\
\end{cases}
\end{equation}
...
\begin{eqnarray}
\wt_L^{\trans} &=& \wt_{\ib-\eb_r}^n + \left(\half - \frac{\dt}{2h}w_{\ib-\eb_r}^n\right)\Delta_r \wt_{\ib-\eb_r}^n - \frac{\dt}{2}\wt_{\ib-\eb_r}^n\left(\frac{\partial w_0}{\partial r}\right)_{\ib-\eb_r},\label{Radial utrans_L} \\
\wt_R^{\trans} &=& \wt_{\ib}^n - \left(\half + \frac{\dt}{2h}w_{\ib}^n\right)\Delta_r \wt_{\ib}^n - \frac{\dt}{2}\wt_{\ib}^n\left(\frac{\partial w_0}{\partial r}\right)_{\ib}.\label{Radial utrans_R}
\end{eqnarray}
...
\wt_L^{\trans}, & w_L^{\trans} + w_R^{\trans} > 0, \\
\wt_R^{\trans}, & w_L^{\trans} + w_R^{\trans} < 0, \\
\end{cases}
\end{equation}
where $w_L^{\trans} = \wt_L^{\trans} + w_{0,r-\half}$ and $w_R^{\trans} = \wt_R^{\trans} + w_{0,r-\half}$.  Finally, we call {\tt addw0} to compute $w^{\trans}$ from $\wt^{\trans}$.
...
\begin{eqnarray}
v_L^{\trans} &=& v_{\ib-\eb_y}^n + \left(\half - \frac{\dt}{2h}v_{\ib-\eb_y}^n\right)\Delta_y v_{\ib-\eb_y}^n, \\
v_R^{\trans} &=& v_{\ib}^n - \left(\half + \frac{\dt}{2h}v_{\ib}^n\right)\Delta_y v_{\ib}^n,
\end{eqnarray}
\begin{equation}
v^{\trans}_{\ib-\half\eb_y} =
\begin{cases}
0, & \left(v_L^{\trans} \le 0 ~ {\rm AND} ~ v_R^{\trans} \ge 0\right) ~ {\rm OR} ~ \left|v_L^{\trans} + v_R^{\trans}\right| < \epsilon, \\
v_L^{\trans}, & v_L^{\trans} + v_R^{\trans} > 0, \\
v_R^{\trans}, & v_L^{\trans} + v_R^{\trans} < 0. \\
\end{cases}
\end{equation}
...
u_{\ib-\half\eb_r}^{1D} =
\begin{cases}
\half\left(u_L^{1D} + u_R^{1D}\right), & \left|w^{\trans}_{\ib-\half\eb_r}\right| < \epsilon \\
...
\begin{eqnarray}
u_L^{\mac} &=& u_{\ib-\eb_x}^n + \left(\half - \frac{\dt}{2h}u_{\ib-\eb_x}\right)\Delta_x u_{\ib-\eb_x}^n + \frac{\dt}{2}f_{u,\ib-\eb_x} \nonumber \\
...
u_R^{\mac} &=& u_{\ib}^n - \left(\half + \frac{\dt}{2h}u_{\ib}\right)\Delta_x u_{\ib}^n + \frac{\dt}{2}f_{u,\ib} \nonumber \\
...
u_{\ib-\half\eb_x}^{\mac,*} =
\begin{cases}
0, & \left(u_L^{\mac} \le 0 ~ {\rm AND} ~ u_R^{\mac} \ge 0\right) ~ {\rm OR} ~ \left|u_L^{\mac} + u_R^{\mac}\right| < \epsilon, \\
u_L^{\mac}, & u_L^{\mac} + u_R^{\mac} > 0, \\
u_R^{\mac}, & u_L^{\mac} + u_R^{\mac} < 0. 
\end{cases}
\end{equation}
...
\wt_{\ib-\half\eb_x}^{1D} =
\begin{cases}
\half\left(\wt_L^{1D} + \wt_R^{1D}\right), & \left|u^{\trans}_{\ib-\half\eb_x}\right| < \epsilon \\
...
\begin{eqnarray}
\wt_L^{\mac} &=& \wt_{\ib-\eb_r}^n + \left(\half - \frac{\dt}{2h}w_{\ib-\eb_r}\right)\Delta_r \wt_{\ib-\eb_r}^n + \frac{\dt}{2}f_{w,\ib-\eb_r} \nonumber \\
...
\wt_R^{\mac} &=& \wt_{\ib}^n - \left(\half + \frac{\dt}{2h}w_{\ib}\right)\Delta_r \wt_{\ib}^n + \frac{\dt}{2}f_{w,\ib} \nonumber \\
...
\wt_L^{\mac}, & w_L^{\mac} + w_R^{\mac} > 0, \\
\wt_R^{\mac}, & w_L^{\mac} + w_R^{\mac} < 0. 
\end{cases}
\end{equation}
...
\begin{eqnarray}
u_L^{\mac} &=& \Ub_{L,\ib-\half\eb_x}^{1D} + \frac{\dt}{2}f_{u,\ib-\eb_x} \nonumber \\
...
u_R^{\mac} &=& \Ub_{R,\ib-\half\eb_x}^{1D} + \frac{\dt}{2}f_{u,\ib} \nonumber \\
...
To compute $\Ub^{\edge}$, {\tt VARDEN} uses the exact same algorithm as the $s^{\edge}$ case in {\tt MAESTRO}.  To the algorithm for $\rho^{\edge}$ in {\tt VARDEN} is slightly different than the $s^{\edge}$ case in {\tt MAESTRO} since it uses a ``conservative'' formulation.  In particular:
\begin{eqnarray}
