...
\subsection{Correct Base}

The base state density correction now includes the area factors in the divergences,
\begin{equation}
\rho_{0,j}^{\outp} = \rho_{0,j}^{\inp}  - \frac{1}{r_j^2} \frac{\dt}{\dr} \left [ \left(r^2 \etarho\right)_{j+\myhalf} - \left(r^2 \etarho\right)_{j-\myhalf} \right ] \enskip .
\end{equation}
