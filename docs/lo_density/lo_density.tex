...
\item After we ``{\tt correct base}'' (by setting $\rho_0 = \overline{\rho}$ in
  plane-parallel and $\rho_0 = \rho_0 + {\bf{Avg}}(\rho - \rho_0)$).

\item At the beginning of the second-half of the algorithm (Step 6), we reset
  the coordinates using $\rho_0^n$.
...
\item In {\tt init\_base\_state}, we give a warning of {\tt anelastic\_cutoff}
...
\MarginPar{I think we want to change this to include {\tt anelatic\_cutoff\_coord}.}
...
\item In {\tt enforce\_HSE}, we set $p_0(r) = p_0(r-1)$ for 
  $r \ge {\tt base\_cutoff\_density\_coord}+1$.
