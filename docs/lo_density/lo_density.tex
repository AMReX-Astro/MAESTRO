...
\item In {\tt make\_hgrhs}, {\tt make\_macrhs}, and {\tt make\_w0}, 
  ...
  (in spherical).
  \MarginPar{For plane-parallel, I think we should use 
    $r \le {\tt base\_cutoff\_density\_coord}$.}
...
\item In {\tt update\_scal}, if $\rho < {\tt base\_cutoff\_density}$
  ...
  \MarginPar{I think we want $\rho \le {\tt base\_cutoff\_density}$}

\item In {\tt make\_grav} for spherical, we only add the enclosed mass if
  $\rho_0 > {\tt base\_cutoff\_density}$.
  \MarginPar{I think this needs to be reworked to include mass up to the 
    cell-centered location of {\tt base\_cutoff\_density\_coord} for both cell 
    and edge gravity.}
...
\item In {\tt make\_psi} for plane-parallel, we only compute $\psi$ for 
  $r < {\tt base\_cutoff\_density\_coord}$.
  \MarginPar{I think we want to change this to $r \le {\tt base\_cutoff\_density\_coord}$.}
