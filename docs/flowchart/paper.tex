...
\newcommand{\uadvstar}{\widetilde{\mathbf{U}}^{\mathrm{ADV,\star}}}
\newcommand{\uadvstarone}{\widetilde{\mathbf{U}}^{\mathrm{ADV,(1),\star}}}
\newcommand{\uadvstartwo}{\widetilde{\mathbf{U}}^{\mathrm{ADV,(2),\star}}}
\newcommand{\wadv}{w^{\mathrm{ADV}}}
\newcommand{\dt}{\Delta t}
\newcommand{\dr}{\Delta r}
\newcommand{\dx}{\Delta \x}
\newcommand{\dy}{\Delta r}
\newcommand{\etal}{\it et al.}

\newcommand{\etarho}{\eta_{\rho}}
\newcommand{\etaXk}{\eta_{\rho X_k}}
...
a)  For the density update, we discretize equation (ref{eq:rho0upd\_new}), neglecting the $\etarho$ term,
...
b)  For the pressure update, We discretize equation (ref{eq:p0upd\_new}), 
...
\end{equation}
%for plane-parallel geometries,  or the analogous equation (see eq. [ref{eq:B7}])
%for spherical configurations.  
For spherical geometries, this equation would be modified.
%In the case of spherical geometries use the current $p_0^n,$
%${\Gamma_1}^n_0 = \Gamma_1(p_0^n, \rho_0^n, X_k^n),$ and $g$ as computed from $\rho_0^n.$
...
%The construction of 
%$ \left[ \nabla \cdotb \left(  (\uadvone+w_0^{\nph,\star} \er)  (\rho X_k)^{(1)} \right) \right]^{\nph}$ \\
%and $ \left[ \nabla \cdotb \left( (\uadvone+w_0^{\nph,\star} \er)  (\rho h)^{(1)}\right)  \right]^{\nph}$
%also follows that described in paper II, Appendix A with $\V = \uadvone+w_0^{\nph,\star} \er.$ 
%We note that the time-centering of the $p_0$ terms in equations (ref{eq:step4_rhoh})
%matches as closely as possible the time-centering of the $\beta_0$ in 
%equation (ref{eq:uadvone_satisfies}). 
...
%be computed via equation (ref{eq:spherical_psi_num}).
...
%In spherical configurations we use the analogous equation (see eq. [ref{eq:B7}]). 
%In the case of spherical geometries use $p_0^{n+1,\star},$
%${\Gamma_1}^{n+1,\star}_0 = \Gamma_1(p_0^{n+1,\star}, \rho_0^{n+1,\star}, X_k^{n+1,\star}),$ and 
%$g$ as computed from $\rho_0^{n+1,\star}.$
...
%The construction of 
%$ \left[ \nabla \cdotb \left(  (\uadvtwo+w_0^{\nph} \er)  (\rho X_k)^{(1)} \right) \right]^{\nph}$ \\
%and $ \left[ \nabla \cdotb \left( (\uadvtwo+w_0^{\nph} \er)  (\rho h)^{(1)}\right)  \right]^{\nph}$
%is similar to that described in paper II, Appendix A with $\V = \uadvtwo+w_0^{\nph} \er.$ 
%For species advection we set $s = X_k,$ then multiply the edge states of $X_k$ by predicted
%edge states of $\rho.$  For the enthalpy update we predict temperature to the edges, convert
%$T$ to $h$ using the EOS, then multiply by $\rho.$
%We note that the time-centering of the $p_0$ terms in Equations (ref{eq:step8_rhoh})
%matches as closely as possible the time-centering of the $\beta_0$ in 
%Equation (ref{eq:uadvtwo_satisfies}).
...
\end{eqnarray}
%For spherical geometries, $\psi_j^{\nph}$ would be computed via equation (ref{eq:spherical_psi_num}).
\end{enumerate}
...
We solve the linear system of equation (ref{eq:nodal\_solve}) using multigrid V-cycles with Gauss-Seidel relaxation.
