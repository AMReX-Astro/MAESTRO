...
\newline
is the process by which we update the base state through $\dt$ in time \MarginPar{we need a separate section for spherical}
given the radial velocity $w_0^{\inp}.$ Here we discuss the algorithm
for plane-parallel geometries.  The base state arrays are all
...
defined halfway in-between.\\ \\ a) For the density update, we
discretize equation the base state density evolution equation
\begin{eqnarray}
...
later in the overall algorithm.\\ \\ b) For the pressure update, we
discretize the base state pressure evolution equation,
\begin{equation}
...
\MarginPar{this is plane-parallel only}
\MarginPar{Need dpdt\_factor stuff.}
\begin{equation}
