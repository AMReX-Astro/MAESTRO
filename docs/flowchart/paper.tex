...
large-scale mixing, through $\etarho$

\begin{description}

\item[plane-parallel:] We use
\begin{eqnarray}
\rho_{0,j}^{\outp} &=& \rho_{0,j}^{\inp} - \frac{\dt}{\dr} 
...
\MarginPar{need to add spherical}
...
\end{equation}
using the lagged $\psi^{\nmh},$ \MarginPar{note: this is true only for plane-parallel}
...
\begin{equation}
\int_{\Omega_H} \ut \cdotb \er \; dA = 0 \label{eq:udoterzero} 
\end{equation}
(paper III; eq.~[21])
and
\begin{equation}
...
\end{eqnarray}
\MarginPar{why $\rho^{(2'),\star}$ and not $\rho^{(2),\star}$?}
...
+ \frac{Dp_0}{Dt} + \rho \Hnuc + \rho \Hext \enskip , \label{eq:rhohupd} 
  \end{equation}
  neglecting the reaction terms. \MarginPar{need to add diffusion.}
  ...
  explicitly including the reaction terms, \MarginPar{shouldn't this be $(\rho h)^{(2'),\star}$?}
  \begin{eqnarray}
...
\item[plane-parallel:] We integrate equation (\ref{eq:divw0}),
...
\end{eqnarray}
\MarginPar{this needs to be updated---we are not lagging things anymore.} 
...
\MarginPar{we need to update the centering of $\psi$ in the enthalpy difference equation.}
