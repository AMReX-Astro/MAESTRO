...
\item {\bf Advect Base}$(\rho_0^{\inp}, p_0^{\inp}, \beta_0^{\inp}, w_0^{\inp}, \psi^{\inp} ) \rightarrow (\rho_0^{\outp}, \rho_0^{\outp,n+\myhalf}, p_0^{\outp})$ 
...
Define
\begin{eqnarray}
\gammabar^{(1)}        &=& {\rm{\bf Avg}} \left( \Gamma_1(\rho^{(1)}, p_0^{n}, X_k^{(1)}) 
\right) \enskip , \\
