...
\item Compute the species edge states, $(\rho X_k)^{(1),\nph}$, for
  the conservative update of $(\rho X_k)$. Here we
  predict $\rho'$ and $X_k$ to the edges, using a second-order Taylor
  expansion in space and time, as described in paper II, Appendix A,
  using $\V = \uadvtwo+w_0^{\nph} \er$.  We use the base state
  density, $\rho_0$, to convert these to edge states for $(\rho X_k)$
  and $(\rho h)$.  We define $\rho^{(1),\nph} = \sum_k (\rho
  X_k)^{(1),\nph}$.
...
\item Compute the enthalpy edge state, $(\rho h)^{(1),\nph}$, for the
  conservative update of $(\rho h)$. Here we predict $(\rho h)'$ to
  the edges, using a second-order Taylor expansion in space and time,
  as described in paper II, Appendix A, using $\V =
  \uadvtwo+w_0^{\nph} \er$.  Again, we do not include the reaction
  terms in the enthalpy prediction since we accounted for them already
  in {\bf React State}. We use the base state enthalpy, $(\rho h)_0$,
  to convert these to edge states for $(\rho h)$.
