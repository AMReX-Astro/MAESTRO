...
\item Update the species.  Here we do a difference approximation
  ...
  for the conservative update of $(\rho X_k)$.  Here we predict
  $\rho'$ and $X_k$ to the edges, using a second-order Taylor
  expansion in space and time, as described in paper II, Appendix A,
  using $\V = \uadvone+w_0^{\nph,\star} \er$.  We use the base state
  density, $\rho_0$, to convert these to edge states for $(\rho X_k)$.
  We define $\rho^{(1),\nph,\star} = \sum_k (\rho
  X_k)^{(1),\nph,\star}.$
