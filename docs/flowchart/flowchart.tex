...
\item We eliminated {\bf Advect Base Pressure} entirely, and instead directly
enforce HSE after {\bf Correct Base}.  {\bf Advect Base Pressure} was just a fancy
way of enforcing HSE anyway, so we just compute the new pressure directly.
...
\item Update $p_0$ to enforce hydrostatic equilbrium, $\nabla p_0^{n+1,\star} = -\rho_0^{n+1,\star} g$
