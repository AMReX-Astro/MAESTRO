...
\left [ \left( {{\rho}_0}^{\outp,\nph} {w_0}^{\inp}\right)_{j+\myhalf} -  
         ...
         \nonumber \enskip,
\end{eqnarray}
\MarginPar{why is the notation for the edge state ${{\rho}_0}^{\outp,\nph}$? shouldn't
           it be ${{\rho}_0}^{\inp,\nph}$?}
...
\item {\bf Advect Base Pressure}$[p_0^\inp,w_0^\inp,\psi^\inp] \rightarrow [p_0^\outp]$\\
is the process by which we update the base state pressure through $\dt$ in time.
\begin{description}

\item[plane-parallel:] We discretize the base state pressure
...
\enskip,
\end{equation}
\MarginPar{shouldn't the edge states for $p_0$ have a $\inp$ or $\outp$ superscript?}
...
+ \dt\psi_j^{\inp} \enskip ,
\end{equation}
\MarginPar{why is the notation for the edge state ${{\rho h}_0}^{\outp,\nph}$? shouldn't
           it be ${{\rho h}_0}^{\inp,\nph}$?}
where the interface states are again found via the procedure described
...
\item Update the species.  Here we do a difference approximation
  ...
  \MarginPar{why don't we do something higher order here for plane-parallel?}
\begin{equation}
\rho^{(1),\nph,\star} = 
...
\item (spherical only) {\bf Advect Base Pressure}$[p_0^{n}, w_0^{\nph,\star}, \psi^{\nmh}]$ 
$\rightarrow [p_0^{n+1,\star}]$.  Note that $\psi$ is not used in the spherical case.
...
\item (plane-parallel only only) {\bf Advect Base Pressure}$[p_0^{n}, w_0^{\nph,\star},$ 
$\psi^{\nph,\star}] \rightarrow [p_0^{n+1,\star}]$.
...
\MarginPar{is it really $p_0^n$ here and not $p_0^{\nph}$?}
\end{description}
...
\item (spherical only) {\bf Advect Base Pressure}$[p_0^{n}, w_0^{\nph}, \psi^{\nph,\star}]$ 
$\rightarrow [p_0^{n+1}]$.  Note that $\psi$ is not used in the spherical case.
...
\item  (plane-parallel only) {\bf Advect Base Pressure}$[p_0^{n}, w_0^{\nph}, \psi^{\nph}]$ 
$\rightarrow [p_0^{n+1}]$.
...
\item Why for plane parallel, when constructing the full edge state
      (eq.~\ref{eq:rhoXedgestate}) do we simply average to the interface
      instead of doing the 4th order interpolation that is used for the
      full state?
