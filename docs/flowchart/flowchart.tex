...
\item {\bf Advect Base Pressure Spherical}$[p_0^{\inp},w_0^{\inp},\gammabar^{\inp},\Sbar^{\inp}] 
\rightarrow [p_0^{\outp},\psi^{n,\outp},\psi^{n+\myhalf,\outp}]$:

\begin{color}{red}
Proposal: We need to pass in two different $\gammabar$ objects, $\gammabar^{\inp,n}$ and $\gammabar^{\inp,n+\myhalf}$.  The pressure update will use $\gammabar^{\inp,n+\myhalf}$.  To compute $\psi^{\outp,n}$, we use $\gammabar^{\inp,n}$.  To compute $\psi^{\outp,n+\myhalf}$, we use $\gammabar^{\inp,n+\myhalf}$.  In the first half of the algorithm, we pass in $\gammabar^n$ for both arguments.  For the second half of the algorithm, we pass in $\gammabar^n$ and $(\gammabar^n+\gammabar^{n+1,\star})/2$.  I don't think we ever need $\gammabar^{(1)}$, as the only place it's currently used is as an input to the first call to {\bf Advect Base Pressure Spherical}.
\end{color}

First use a predictor-corrector formulation for the pressure update 
...
with:
\begin{equation}
f = \frac{\dt}{2} \left [ \Sbar^{\inp} -  \frac{1}{r^2} \frac{\partial}{\partial r} \left (r^2 w_0^{\inp} \right) - 
...
differencing.  Now we begin the corrector step.  Using $p_0^\star$, we can compute a more 
accurate approximation to equation~(\ref{eq:frhs}),
\begin{equation}
\tilde{f} = \frac{\dt}{2} \left [ \Sbar^{\inp} -  \frac{1}{r^2} \frac{\partial}{\partial r} \left (r^2 w_0^{\inp} \right) - 
...
Next, we compute $\psi^{\outp,n}$ and $\psi^{\outp,n+\myhalf}$.  Rewriting Equation (\ref{eq:w01}), we can define
\begin{eqnarray}
...
\left \{ \Sbar_j^{\inp} -  \frac{1}{r_j^2} \left [ (r^2 w_0^{\inp})_{j+\myhalf} -
          (r^2 w_0^{\outp})_{j-\myhalf} \right ] \right \} \enskip ,
\end{equation}
\begin{equation}
\psi_j^{\outp,n+\myhalf} = 
\left[\gammabar^{\inp} \frac{\left( p_0^{\inp} + p_0^{\outp} \right)}{2}\right]_j
\left \{ \Sbar_j^{\inp} -  \frac{1}{r_j^2} \left [ (r^2 w_0^{\inp})_{j+\myhalf} -
          (r^2 w_0^{\outp})_{j-\myhalf} \right ] \right \} \enskip .
\end{equation}
\end{description}
...
\end{equation}
The other difference is that the interface states are computed using 
$\psi^{\inp,n}$ as the forcing.
...
large-scale mixing.  It is also used to adjust the base state enthalpy
...
We note that  $\uadvone$ satisfies the discrete versions of paper III equation (21),
\begin{equation}
...
\end{equation}
This gives a radial bin-centered $\etarho^{\nph,\star}$.  To get $\etarho^{\nph,\star}$ at 
...
\item For spherical problems, update gravity, $g^{n+1,\star} = g(\rho_0^{n+1,\star})$
...
\overline{S^{\nph,\star}}] \rightarrow [p_0^{n+1,\star},\psi^{n,\star},\psi^{\nph,\star}]$.
\end{description}
...
\begin{eqnarray}
 ...
 \beta_0^{n+1,\star}    &=& \beta   \left(\rho_0^{n+1,\star}, p_0^{n+1,\star}, \gammabar^{n+1,\star}\right) \enskip ,
\end{eqnarray}
...
\item Define:
\begin{equation}
\gammabar^{\nph,\star} = \frac{\gammabar^{n} + \gammabar^{n+1,\star}}{2}\enskip , ~~~
\rhozero^{\nph,\star} = \frac{\rhozero^{n} + \rhozero^{n+1,\star}}{2}\enskip , ~~~
p_0^{\nph,\star} = \frac{p_0^{n} + p_0^{n+1,\star}}{2}\enskip .
\end{equation}
\item Compute $w_0^{n+\myhalf}$:
\begin{description}
\item For plane-parallel,\\
{\bf Compute $w_0$ Planar}$[\overline{S^{\nph}},
\gammabar^{n+\myhalf,\star},p_0^{n+\myhalf,\star},\psi^{n+\myhalf,\star}]\rightarrow [w_0^{n+\myhalf,\star}]$.
\item For spherical,\\
{\bf Compute $w_0$ Spherical}$[\overline{S^{\nph}},\gammabar^{n+\myhalf,\star},
\rhozero^{n+\myhalf,\star},p_0^{n+\myhalf,\star},\etarho^{n+\myhalf,\star}] \rightarrow [w_0^{n+\myhalf}]$.
\end{description}
% VOLUME DISCREPANCY STUFF
%\begin{equation}
%\overline{p(\rho,h,X_k)^{\nph,\star}} = 
%\frac{\overline{p(\rho,h,X_k)^n} + \overline{p(\rho,h,X_k)^{n+1,\star}}}{2}\enskip .
%\end{equation}
%\begin{equation}
%\frac{\partial w_0^{\nph}}{\partial r} = 
%\overline{S^{\nph}} - \frac{1}{\gammabar^{\nph,\star} p_0^{\nph,\star}}
...
\item For spherical problems, update gravity, 
$g^{n+1} = g(\rho_0^{n+1})$; $g^{\nph} = g(\rho_0^\nph)$
...
\item For spherical,\\
{\bf Advect Base Pressure Spherical}$[p_0^n,w_0^{\nph},\gammabar^{n+1,\star},\overline{S^{\nph,\star}}] \rightarrow [p_0^{n+1},\psi^n,\psi^{\nph}]$.
\end{description}
...
\begin{eqnarray}
 ...
 \beta_0^{n+1}    &=& \beta   \left(\rho_0^{n+1}, p_0^{n+1},   \gammabar^{n+1}\right) \enskip ,
\end{eqnarray}
where $\beta_0$ is computed as described in paper III, Appendix C.
