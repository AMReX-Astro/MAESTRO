...
\end{equation}
\begin{description}
\item[planar:] We simply average cells at constant height.
...
%compute $\psi$.  The input arguments are different for planar and spherical geometries.
%\begin{description}
...
\begin{description}
\item[planar:] Set $\rho_0^{\outp} = {\bf Avg}(\rho^{\inp})$.
...
The arguments are different for planar and spherical geometries.
\begin{description}
...
This sets $(\rho h)_0$ to be the lateral average of $(\rho h)^{(1)}$
...
\begin{equation}
\frac{\partial (\rho X_k)}{\partial t} + \nablab \cdotb (\Ub \rho X_k) = 0 \enskip .
\label{eq:species}
\end{equation}
...
using $\Vb = \uadvtwo+w_0^{\nph}\er$.  Convert the perturbational edge 
state to a full state using 
\begin{equation}
