...
differencing.  Next, we compute a provisional updated $\gammabar$ using the
provisional updated pressure,
...
\end{equation}
Next, we compute $\psi^{\outp,n}$ and $\psi^{\outp,n+\myhalf}$.  First, we compute an updated $\gammabar$ using the update pressure, $\gammabar^\star = {\bf Avg}[\Gamma_1(\rho^{\inp},p_0^{\outp},X_k^{\inp})]$.  Then, rewriting equation (\ref{eq:w01}), we can define
\begin{eqnarray}
