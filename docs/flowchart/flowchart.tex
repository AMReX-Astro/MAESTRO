...
\item Should we eeplace {\bf Correct Base} with $\rho_0 = {\rm{\bf Avg}}(\rho)$ in 
plane-parallel and $\rho_0 = \rho_0 - {\rm{\bf Avg}}(\rho_0 - \rho)$ in spherical?
This eliminates the need for $\nabla\cdot\etarho$.

\item In {\bf Step 4} and {\bf Step 8}, should we move {\bf Correct Base} so it 
...
\item After {\bf React State}, should we set $(\rho h)_0 = {\rm{\bf Avg}}(\rho h)$ in 
plane-parallel and $(\rho h)_0 = (\rho h)_0 - {\rm{\bf Avg}}[(\rho h)_0 - (\rho h)]$ 
in spherical?  This can currently be done by making {\tt full\_rhoh0\_evolution = FALSE}.
This means that {\bf React Base} no longer has to exist, and the intermediate states, 
...
$w_0^0 = 0$ as an initial approximation.  Compute $\beta_0^{\initp}$ as a function of 
...
first time step.  As initial approximations, set $\etarho^{-\myhalf}=0$, $\psi^{-\myhalf}=0$,
$S^{1,0}=S^0$, and $\pi^{-\myhalf}=0.$
\begin{enumerate}
  \renewcommand{\theenumi}{{\bf \alph{enumi}}}
  \renewcommand{\labelenumii}{\roman{enumii}.}
