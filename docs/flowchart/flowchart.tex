...
differencing.  Now we begin the corrector step.
\begin{color}{red} We need to compute 
$\gammabar^\star = {\bf Avg}[\Gamma_1(\rho^{(2)},p_0^\star,X_k^{(2)})]$ 
and $\gammabar^\dagger = (\gammabar^\inp + \gammabar^\star)/2$, and then use 
$\gammabar^\dagger$ in the $f^\dagger$ equation. \end{color} 
Using $p_0^\star$, we can compute a more 
accurate approximation to equation~(\ref{eq:frhs}),
\begin{equation}
f^\dagger = \frac{\dt}{2} \left [ \Sbar^{\inp} -  \frac{1}{r^2} \frac{\partial}{\partial r} \left (r^2 w_0^{\inp} \right) - 
                                             \frac{1}{\gammabar^{\inp} p_0^\dagger} w_0^{\inp} \left ( \frac{\partial p_0^\dagger}{\partial r} \right ) \right ] \enskip ,
\end{equation}
where $p_0^\dagger = (p_0^\inp + p_0^\star)/2$, and the difference is again computed
by first putting $p_0^\dagger$ on edges.
...
\begin{description}
\item For plane-parallel, we hold the gravity constant in space and time.
...
\begin{equation}
g_{j-\myhalf} = \frac{Gm_{j-\myhalf}}{r_{j-\myhalf}^2}; ~~~~~ g_j = \frac{Gm_j}{r_j^2}\enskip .
\end{equation}
\end{description}
...
\item For spherical,\MarginPar{$\gammabar^{(1)}$ instead of $\gammabar^{n+1,\star}$?}\\
{\bf Advect Base Pressure Spherical}$[p_0^n,w_0^{\nph},\gammabar^{n+1,\star},\overline{S^{\nph}}] \rightarrow [p_0^{n+1},\psi^n,\psi^{\nph}]$.
\end{description}
