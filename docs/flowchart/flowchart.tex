...
\item We have added thermal diffusion with {\tt use\_thermal\_diffusion = T}.
...
\item We compute the full state T on cell-centers after the advection-diffusion step 
from $T=T(\rho,X_k,p_0)$ by setting {\tt use\_tfromp = T} rather than $T=T(\rho,h,X_k)$.
\item We have changed the discretization of $\Ub\cdot\nabla p_0$ in the enthalpy
update to instead be $\nabla\cdot(\Ub p_0) - p_0\nabla\cdot\Ub$.
\end{itemize}
...
compute $h$ with the EOS using {\tt do\_eos\_h\_above\_cutoff} = T.
...
\item We have added the flag, {\tt full\_rhoh0\_evolution}, when false, sets
$(\rho h)_0 = (\overline{\rho h})^n$ before {\bf Advect Base}.  {\bf React Base}
will not be called.  Note that we do not have a {\bf Correct Base} or 
{\bf Diffuse Base} for enthalpy.
