...
This step remains unchanged from Paper III.
...
\item Compute $S^{\nph,\star\star}$.  We compute an estimate for the 
  time-centered expansion term in the velocity 
  divergence constraint (eq. [\ref{eq:utilde divergence}]).  For the  
  first time step ($n=0$), we set
\begin{equation}
S^{n+\myhalf,\star\star} = \frac{S^0 + S^1}{2},
\end{equation}
  where $S^1$ is found during initialization.  For other time steps
  $(n \ne 0)$, following \cite{Bell:2004}, we extrapolate
...
\item Compute the provisional base state velocity forcing.
  Rearrange equation (\ref{eq:w0 evolution}),
\begin{equation}
...
Using equation (\ref{eq:utildeupd}), we compute time-centered edge 
...
\item Update $(\rho X_k)$ using a discretized version of equation 
(\ref{eq:species}) omitting the reaction terms, which were already 
...
\item Compute the time-centered species edge states, $(\rho X_k)^{\nph,\star,\pred}$,
  ...
  and $X_k^{\nph,\star,\pred}$.
  We convert the perturbational density full state density using
\begin{equation}
...
(\ref{eq:enthalpy}), again omitting the reaction and heating terms
...
{\label{Fig:flowchart48} A flowchart for {\bf Steps 4} and {\bf 8}.
  ...
  indicated in each step.  Red text indicates that q\ uantity was
