...
\item We are working on {\bf Advect Base}, {\bf React Base}, and 
{\bf Correct Base} details.
...
\item We have added the flag, {\tt full\_rhoh0\_evolution}, which defaults to 
{\tt TRUE} (note that we do not have a {\bf Correct Base} or 
{\bf Diffuse Base} for enthalpy).  When {\tt FALSE}, we do not call {\bf React Base},
but instead set $(\rho h)_0 = \overline{(\rho h)^n}$ before {\bf Advect Base}.
...
\item Replacing {\bf Correct Base} with $\rho_0 = {\rm{\bf Avg}}(\rho)$ in 
plane-parallel and $\rho_0 = \rho_0 - {\rm{\bf Avg}}(\rho_0 - \rho)$ in spherical.  
This eliminates the need for $\nabla\cdot\etarho$.

\item In {\bf Step 4} and {\bf Step 8}, move {\bf Correct Base} to immediately
before {\bf Advect Base Pressure}.

\item Should we use a predictor-corrector for updating the full-state density?
In particular, after {\bf Correct Base}, do a full-state density advance and
{\bf Correct Base} again?

\item After {\bf React State}, setting $(\rho h)_0 = {\rm{\bf Avg}}(\rho h)$ in 
plane-parallel and $(\rho h)_0 = (\rho h)_0 - {\rm{\bf Avg}}[(\rho h)_0 - (\rho h)]$ 
in spherical by making {\tt full\_rhoh0\_evolution = FALSE} by default.  This means
that {\bf Correct Base} no longer has to exist, and the intermediate states, 
$(\rho h)_0^{(1)}$ and $(\rho h)_0^{(2)}$, no longer have to exist.  
{\bf Advect Base Enthalpy} advances $(\rho h)_0^n \rightarrow (\rho h)_0^{n+1}$ directly.
This also means we don't have to deal with $\eta_{\rho h}$ or a {\bf Diffuse Base}.

\item Eliminating {\bf Advect Base Pressure} entirely, and instead enforcing HSE 
directly after {\bf Correct Base}.  {\bf Correct Base} should occur directly after 
creating $\etarho$, there is no need to delay the call as is currently being done.
...
\item[spherical:] 
\MarginPar{Need to write this up somewhere}
...
~~~ $(\rho h)_0^{(1)} = \overline{(\rho h)^{(1)}}$ in plane-parallel

~~~ $(\rho h)_0^{(1)} = (\rho h)_0^{n} - {\rm{\bf Avg}}\left[(\rho h)_0^{n} - (\rho h)^{(1)}\right] $ in spherical. \\
{\bf end if}
\item Define
\begin{eqnarray}
