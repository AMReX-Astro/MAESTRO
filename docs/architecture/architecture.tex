...
with MAESTRO, how to run the code, and how to look at the output.
...
rather enforces $\nabla \cdot U = S$ as discussed in \cite{SNe}.

\item {\tt spherical\_heat} \\[-3mm]

Spherical heat maps a spherical star (a Chandrasekhar-mass white
...
cause the star to expand.  This is the 3-d analogue of the {\tt
  test\_basestate} unit test.  This problem was discussed in
\cite{multilevel}.
...
\subsection{Common Runtime Parameters}

\begin{itemize}

\item {\tt model\_file }
\item {\tt spherical\_in }
\item {\tt ppm\_type }
\item {\tt max\_levs }
\item {\tt n\_cellx }, {\tt n\_celly }, {\tt n\_cellz }
\item {\tt max\_grid\_size }
\item {\tt prob\_lo\_x }, {\tt prob\_lo\_y }, {\tt prob\_lo\_z }
\item {\tt prob\_hi\_x }, {\tt prob\_hi\_y }, {\tt prob\_hi\_z }
\item {\tt anelastic\_cutoff }
\item {\tt base\_cutoff\_density }
\item {\tt do\_sponge }
\item {\tt grav\_const }
\item {\tt max\_step }
\item {\tt init\_iter }
...
\item {\tt bcx\_lo }, {\tt bcy\_lo }, {\tt bcz\_lo }, 
      {\tt bcx\_hi }, {\tt bcy\_hi }, {\tt bcz\_hi }
\item {\tt pmask\_x }, {\tt pmask\_y }, {\tt pmask\_z }
\item {\tt verbose }
\item {\tt mg\_verbose }
\item {\tt cg\_verbose }
\item {\tt do\_initial\_projection }
\item {\tt init\_divu\_iter }
\item {\tt perturb\_model }

\end{itemize}

\subsection{Working with the Output}

Visualization and analysis are done with on the plotfiles.  The
frequency of plotfile outputting can be changed using the runtime
parameters {\tt plot\_int}, which specifies the number of steps
between output, and {\tt plot\_deltat} which specifies the simulation
time between output.  Setting {\tt plot\_int} to {\tt -1} disables
step-based outputting; setting {\tt plot\_deltat} to {\tt 0.0}
disables time-based outputting.



\subsubsection{Amrvis}
...
runtime parameters, in which case, its parameter file should also
appear in {\tt PROBIN\_PARAMETERS}.
\begin{verbatim}
