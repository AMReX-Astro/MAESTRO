...
\caption{\label{fig:dataloc} Some of the different data-centerings:
  ...
  Not show is the case where data is nodal in the $y$-direction only.}
\end{figure}
...
\subsection{`{\tt s}' and `{\tt u}' \multifab s (fluid state and velocity)}

The fluid state (density, enthalpy, species) are stored together in a
multi-component \multifab, typically named {\tt sold}, {\tt s1}, {\tt
s2}, or {\tt snew} (depending on which time-level it represents).



\subsection{{\tt umac} (the MAC velocity)}

In creating the advective fluxes, we need the velocity through the
faces of the zone---the $x$-velocity on the $x$-edges, the
...
At each level of refinement, this is done by making a \multifab\
array where each of the {\tt dm} components is nodal in
its respective direction:
\begin{verbatim}
