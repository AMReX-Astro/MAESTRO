...
with support for parallelism (using MPI or OpenMPI), or compiled
...
Setting {\tt OMP := t} will use OpenMP.  If desired, these are used together,
with MPI distributing the grids across the processors and within a 
shared-memory node, OpenMP is used to allow many cores to operate
...
The final line in the {\tt GNUmakefile} include the rules to actually
...
MAESTRO initial models are read in by the {\tt init\_base\_state}
...
\item {\tt multifab\_fill\_ghost\_cells} is used for multilevel
  ...
  This call is usually preceded by a call to {\tt
    ml\_cc\_restriction\_c} which sets the level {\tt n-1} data to be
