...
transfers heat to the fuel, driving a burning front.

\item {\tt spherical\_heat} \\[-3mm]

Spherical heat maps a spherical star (a Chandrasekhar-mass white dwarf)
onto the Cartesian domain and uses an external heat source to cause
the star to expand.  This is the 3-d analogue of the {\tt test\_basestate}
unit test.  This problem was discussed in \cite{multilevel}.
...
This test problem was discussed in \cite{wdconvect}.
...
\end{verbatim}
As the code runs, it will output both plotfiles and checkpoint files.
By default, the plotfiles will be named {\tt plt}{\em nnnnn}, where
the number {\em nnnnn} is the timestep number when the file was
outputted.  Similarly, the checkpoint files are named {\tt chk}{\em
  nnnnn}.

To restart a calculation from a checkpoint file, either edit the inputs
file, setting
\begin{verbatim}
  restart = n
\end{verbatim}
where {\tt n} is the step number of the checkpoint file to restart
from.  Equivalently, you can override the value in the inputs file
by adding {\tt --restart n} to the commandline, as 
\begin{verbatim}
  main.Linux.Intel.exe inputs_2d --restart n
\end{verbatim}
...
\begin{verbatim}
  ...
  OMP    :=
\end{verbatim}
Here, {\tt NDEBUG} is true is we are building an optimized executable.  Otherwise,
the debug version is built---this typically uses less optimization and adds various
runtime checks through compiler flags.  {\tt MPI} and {\tt OMP} are set to true if
we want to use either MPI or OpenMP for parallelization.

The MAESTRO build system knows what options to use for various compiler families.
The {\tt COMP} flag specifies which compiler to us.  Allowed values include
{\tt Intel}, {\tt gfortran}, {\tt PGI}, and {\tt PathScale}.  The specific details
of these choices are defined in the {\tt MAESTRO/mk/} directory.
\begin{verbatim}
  COMP := Intel
  MKVERBOSE := t
\end{verbatim}
{\tt MKVERBOSE} set to true will echo the build commands to the terminal as the
are executed.
...
same name found elsewhere in the source tree.  The {\tt Fmincludes} variable
lists all those directories that contain include files that are inserted into
the Fortran source at compile time via the {\tt include} statement.  Presently,
the only instance of this is with the Helmholtz general equation of state
found in {\tt extern/EOS/helmeos/}.
...
compiled.  This is a Fortran module that holds the values of the runtime
parameters and makes them available to any routine.  The variable
{\tt PROBIN\_PARAMETERS} should list all of the files that contain
runtime parameters for the current problem.  You must always
include the main {\tt ../\_parameters} file (i.e.\ {\tt MAESTRO/\_parameters}).
Sometimes a problem defines their own runtime parameters, in which
case, its parameter file should also appear in {\tt PROBIN\_PARAMETERS}.
\begin{verbatim}
...
Several useful analysis scripts can be found in {\tt
fParallel/data\_processing}.  The makefile there needs
to be edited to indicate which of the tools to build.

\begin{itemize}

\item {\tt fextract3d}

  Extracts a 1-d slice from a plotfile, through the center of the
  domain in any coordinate direction.  Either all of the variables or
  a single variable is output.

\item {\tt fextrema} 

  Outputs the min/max of each of the quantities in the plotfile.

\item {\tt ftime}

  Outputs the simulation time for one or more plotfiles.
...
is simply stored in Fortran arrays.
...
Individual state variables should be indexed using the {\tt variables}
module, with {\tt rho\_comp} referring to the density, {\tt rhoh\_comp}
referring to the enthalpy quantity $(\rho h)$, and {\tt spec\_comp}
referring to the start of the species (partial densities).  
...
\caption{\label{fig:mac} The MAC grid for the velocity.  
...
most commonly used keys are are listed in table~\ref{tab:variable_keys}.
\begin{table}[h!]
\centering
\begin{tabular}{ll}
\hline
{\tt rho\_comp}  & density \\
{\tt rhoh\_comp} & density * specific enthalpy, $(\rho h)$ \\
{\tt spec\_comp} & starting index of the species \\
{\tt temp\_comp} & temperature \\
\hline
\end{tabular}
\caption{\label{tab:variable_keys} {\tt variables} module common keys}
\end{table}
