...
\item {\tt bcx\_lo }, {\tt bcy\_lo }, {\tt bcz\_lo }, 
   ...
   Note: for periodic boundaries, one must also set the corresponding
   {\tt pmask\_}\{{\em x,y,z}\} parameter.

\item {\tt pmask\_x }, {\tt pmask\_y }, {\tt pmask\_z } is set to 
   true ({\tt T}) to indicate that the boundary is periodic in the
   $x$, $y$, or $z$ coordinate directions.
