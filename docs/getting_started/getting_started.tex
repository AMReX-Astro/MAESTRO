...
\item Setup your shell environment.
...
\item Setup the problem's {\tt GNUmakefile}.

In MAESTRO, each problem gets their own sub-directory under {\tt
MAESTRO/}.  This sub-directory will contain any problem-specific files
as well as the {\tt GNUmakefile} that specifies how to build the executable.
Note: we rely on features of GNU {\tt make}.  Full details of the
{\tt GNUmakefile} can be found in \S~\ref{sec:adding_problems}.  Here
we will configure for a serial build.
