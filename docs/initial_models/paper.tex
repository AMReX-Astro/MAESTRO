...
\item Step 1
\begin{enumerate}
\item Copy the initial data from the dots directly into the circles.  For the squares, compute only $\rho$ and $X$ by using linear interpolation from the two nearest corresponding dots.\MarginPar{Maybe we should use quadratic interpolation?}
...
\begin{equation}
p_{(r+1)/2}^{l-1} = p_{r}^l + \frac{3\Delta r^l g}{4}\left(\rho_{r}^l+\rho_{(r+1)/2}^{l-1}\right).
\end{equation}
\end{document}
