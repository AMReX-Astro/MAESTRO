...
\item {\em MAC projection}  
  ...
  There are 2 tolerances here: a relative tolerance of $\epsilon =
  \mathtt{eps\_mac} \cdot \mathtt{mac\_level\_factor}^{(\mathtt{nlevs}-1)}$, and a bottom
  solver tolerance of $\epsilon_\mathrm{bottom} = \mathtt{eps\_mac\_bottom}$.  These
  parameters are set in {\tt mg\_eps.f90} and have the default values:
   \begin{center}
   \begin{tabular}{ll}
   {\tt eps\_mac}           &= $10^{-10}$ \\
...
\item {\em velocity projection}
  ...
  is set in {\tt hgproject} using the parameter values specified in {\tt mg\_eps.f90}.  The default 
