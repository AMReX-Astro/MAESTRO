...
to
\begin{equation}
\nabla\cdot\Ub = \frac{\gamma-1}{\gamma p_0}H \label{eq:div constraint for constant gamma}.
\end{equation}
Plugging this back into either of \eqref{eq:H:pressure evolution} or \eqref{eq:e:pressure evolution} gives
\begin{equation}
  \frac{Dp}{Dt} = \frac{\gamma-1}{\gamma}\left(1-\frac{p}{p_0}\right)H. \label{eq:pressure evolution constant gamma}
\end{equation}
...
can be rewritten as
\[
-\frac{Dp}{Dt} - \frac{\gamma}{\gamma-1}\left(1-\frac{p}{p_0}\right)H = 0,
\]
where the equality holds from \eqref{eq:pressure evolution constant gamma}.
