module feval
   ...
   use burner_data, only: irp_dens, irp_cp, irp_dhdx, irp_o16, irp_rate, &
                           ...
                           irp_xc12tmp, n_rpar_comps, neqs, burn_npts
   implicit none
contains
   subroutine f_rhs_vec(y, t, yd, upar)
      !$acc routine seq
      ...
      real(kind=dp_t), intent(  out) :: yd(neqs,burn_npts)
      real(kind=dp_t), intent(inout) :: upar(n_rpar_comps,burn_npts)
   
      integer :: k, n
      real(kind=dp_t) :: ymass(nspec,burn_npts)
      real(kind=dp_t) :: dens(burn_npts), c_p(burn_npts), dhdX(nspec,burn_npts), X_O16(burn_npts)
      real(kind=dp_t) :: temp(burn_npts), T9, T9a, dT9dt, dT9adt
   ...
   subroutine jac_vec(y, t, pd, upar)
      !$acc routine seq
