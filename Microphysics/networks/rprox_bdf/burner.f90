...
contains
  ...
  subroutine burner(dens, temp, Xin, dt, Xout, rho_omegadot, rho_Hnuc)
    ...
    real(kind=dp_t), dimension(neqs) :: y
    ...
    real(kind=dp_t), dimension(neqs) :: atol, rtol
    ...
    real(kind=dp_t) :: rpar(n_rpar_comps,burn_npts), y0(neqs, burn_npts), y1(neqs, burn_npts)
...
!    dT_crit = 1.e20_dp_t
    ...
    rpar(irp_dens,1) = dens
    rpar(irp_T9_eos,1) = t9
    rpar(irp_dTcrit,1) = 1e15_dp_t
    ...
    if ((t9 .gt. 0.2_dp_t .and. t9 .lt. 2.5_dp_t) .or. Xin(ih1) > 0.05d0) then
       ...
       do n = 1, nspec
          rho_omegadot(n) = dens*(Xout(n) - Xin(n))/dt
       enddo
